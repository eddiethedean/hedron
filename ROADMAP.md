...
<!-- ADOPTER_ROADMAP_BEGIN -->
This is the **single** Hedron roadmap (adopter phase table and maintainer detail). Pin
`hedron` for production; see [What's ready today](docs/guides/whats-ready.md) for capability maturity.
...
- No Hedron `1.0` phase is scheduled (D-038). `fastapi-workbench` 1.0.0 in phase 0.30 is an
  ...
  tier** is catalogued in [STABILITY.md](docs/api/STABILITY.md); most other public APIs remain `beta` or
  `experimental`.
- **0.23** (**Published**) expanded that tier for a locked Beginner/CRUD facade only —
  [STABLE_FACADE](docs/api/STABLE_FACADE.md) — not every What’s ready Supported row.
- **0.24** (**Published**) Accepted **`polling_only`**: live transports stay **experimental**;
  polling is the Supported production story — [LIVE_DISPOSITION](docs/api/LIVE_DISPOSITION.md).
- Trust-program priorities (human AT → CSRF → stable tier → live disposition → archetype):
  [Production-quality maturity](docs/guides/production-quality.md).
...
- Identity helpers are **opt-in wiring** (not a managed IdP) — [Authentication](docs/guides/authentication.md)
...
- Live transports remain **experimental**; polling is Supported — [What’s ready](docs/guides/whats-ready.md)
...
Details: [What's ready today](docs/guides/whats-ready.md) ·
...
[CHART_SPEC](docs/implementation/CHART_SPEC.md).
