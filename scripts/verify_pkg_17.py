#!/usr/bin/env python3
"""Verify phase 0.17 packaging evidence that can run without a public index."""
...
GATE_TESTS = [
    "tests/unit/test_phase17_interaction_graph.py",
    "tests/unit/test_phase17_patches.py",
    "tests/unit/test_phase17_cross_filter.py",
    "tests/unit/test_phase17_graph_replay.py",
    "tests/unit/test_phase17_notebook.py",
    "tests/unit/test_phase17_mcp.py",
    "tests/unit/test_phase17_shell.py",
    "tests/unit/test_phase17_markup_asserts.py",
    "tests/unit/test_phase17_migration_docs.py",
...
def _assert_extras_pins() -> None:
    ...
    for extra, pattern in (
        ("dev", r"hedron-explorer>=0\.17\.0,<0\.18"),
        ("jinja", r"hedron-jinja>=0\.17\.0,<0\.18"),
        ("native", r"hedron-native>=0\.1\.0,<0\.2"),
        ("conformance", r"hedron-conformance>=0\.17\.0,<0\.18"),
        ("extras", r"hedron-extras>=0\.17\.0,<0\.18"),
...
def main() -> int:
    ...
    commands = [
        ...
        [sys.executable, str(ROOT / "scripts" / "check_release_gate.py"), "0.17.0", "--skip-evidence"],
    ]
    for command in commands:
        print("+", *command)
        subprocess.check_call(command, cwd=ROOT)
    print("ok: PKG-017 local packaging evidence")
    return 0
