#!/usr/bin/env python3
"""Shared helpers for phase 0.32 gate checkers."""
...
def require_inventory_supported(package: str, keys: tuple[str, ...], errors: list[str]) -> None:
    if not INVENTORY.is_file():
        errors.append("missing production-grade-inventory-032.toml")
        return
...
def require_inventory_experimental(package: str, keys: tuple[str, ...], errors: list[str]) -> None:
    ...
    missing = [key for key in keys if key not in experimental]
    if missing:
        errors.append(f"{package} inventory missing Experimental keys: {missing}")
