...
| Script | When to run |
|---|---|
| `ci_checks.sh` | **Shared CI suites** used by `.github/workflows/ci.yml` and `release.yml`. Local: `bash scripts/ci_checks.sh test\|workbench\|docs\|typing\|quality\|browser\|evidence\|packaging\|openwiki\|all` — `typing` runs BasedPyright in `all` mode for every uv workspace package and rejects both explicit and inferred `Any`; `openwiki` verifies the local OpenWiki Claims/source checkpoint; `all` runs that local-only freshness gate before the full workflow mirror. Docs-only CI calls `docs` (no wheels). Independent checks inside a suite overlap. `--jobs N` or `HEDRON_CHECK_JOBS` caps concurrency (see `ci_checks.sh --help`) |
...
| `check_package_typing_inventory.py` | Require every uv workspace package source root to appear in `[tool.basedpyright].include`. CI: package typing inventory job |
...
| `check_061.py` / `verify_pkg_61.py` | Phase 0.61 action-state, async-boundary, surface, compatibility, and package evidence |
