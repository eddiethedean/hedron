...
docker run --rm -d \
  ...
  python:3.12-slim \
  sh -lc "pip install -q /src/packages/hedron-core /src/packages/hedron /src/packages/fastapi-workbench /src/packages/hedron-posit && python -m hedron_posit.cli run app_facade:app --mode on --host 0.0.0.0 --allow-external-bind --port 8050 --mount '${MOUNT}' --public-base-url 'https://wb.example${MOUNT}'" \
  >/dev/null
