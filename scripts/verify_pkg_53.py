#!/usr/bin/env python3
"""Verify the phase 0.53 application DX packet or in-tree cut."""
...
from _gate_053 import (  # noqa: E402
    API,
    EXPECTED_GATES,
    GATE,
    IMPLEMENTATION,
...
def _check_packet_files() -> None:
    ...
    if missing:
        raise SystemExit(f"missing 0.53 packet files: {missing}")
    print("ok: 0.53 contract packet files")
...
def _check_gates(*, allow_planned: bool) -> None:
    ...
    if str(_load(GATE).get("contract_refine", "")).strip() != "D-092":
        raise SystemExit(f"{GATE}: contract_refine must be D-092")
    if allow_planned:
        non_planned = [
    ...
    print("ok: release-gate-0.53.toml exact gate inventory")
...
def _check_inventory(*, allow_planned: bool) -> None:
    ...
    print("ok: application-dx-inventory-053.toml")


def _check_contract() -> None:
    if not accepted_contract_present():
        raise SystemExit("RFC-0080 and D-091 must remain Accepted")
    if not contract_refine_present():
        raise SystemExit("D-092 and the frozen 0.53 contract markers must remain present")
    for path in (ROADMAP, STATUS, API, IMPLEMENTATION):
        text = path.read_text(encoding="utf-8")
        for marker in ("0.53", "D-091", "D-092"):
            if path in (ROADMAP, STATUS):
                if marker not in text:
                    raise SystemExit(f"{path}: missing 0.53 traceability marker {marker}")
            elif path in (API, IMPLEMENTATION) and "0.53" not in text:
                raise SystemExit(f"{path}: missing 0.53 marker")
    if TRACKING_ISSUE not in STATUS.read_text(encoding="utf-8"):
        raise SystemExit(f"{STATUS}: missing tracking issue {TRACKING_ISSUE}")
    print("ok: RFC-0080 / D-091 / D-092 boundary and traceability")
...
def _check_versions(*, allow_planned: bool) -> None:
    ...
    if allow_planned:
        ...
        return
    if published.startswith("0.54."):
        print(f"ok: 0.53 historical under living published {published}")
        return
    if not published.startswith("0.53."):
        raise SystemExit(f"cut published version must be on 0.53.x; found {published!r}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.allow_planned:
        ...
        if published.startswith("0.54."):
            print("ok: 0.53 historical packet; skip execute-verified under living 0.54")
        else:
            command = [
                sys.executable,
    ...
    print(f"ok: verify_pkg_53 ({'allow-planned' if args.allow_planned else 'cut'})")
    return 0
