#!/usr/bin/env python3
"""Verify the phase 0.54 authoring-loop packet or in-tree cut."""
...
from _gate_054 import (  # noqa: E402
    API,
    EXPECTED_GATES,
    GATE,
    IMPLEMENTATION,
...
def _check_packet_files() -> None:
    ...
    if missing:
        raise SystemExit(f"missing 0.54 packet files: {missing}")
    print("ok: 0.54 contract packet files")
...
def _check_gates(*, allow_planned: bool) -> None:
    ...
    if str(_load(GATE).get("contract_refine", "")).strip() != "D-094":
        raise SystemExit(f"{GATE}: contract_refine must be D-094")
    if allow_planned:
        non_planned = [
    ...
    print("ok: release-gate-0.54.toml exact gate inventory")
...
def _check_inventory(*, allow_planned: bool) -> None:
    ...
    print("ok: authoring-loop-inventory-054.toml")


def _check_contract() -> None:
    if not accepted_contract_present():
        raise SystemExit("RFC-0081 and D-093 must remain Accepted")
    if not contract_refine_present():
        raise SystemExit("D-094 and the frozen 0.54 contract markers must remain present")
    for path in (ROADMAP, STATUS, API, IMPLEMENTATION):
        text = path.read_text(encoding="utf-8")
        for marker in ("0.54", "D-093", "D-094"):
            if path in (ROADMAP, STATUS):
                if marker not in text:
                    raise SystemExit(f"{path}: missing 0.54 traceability marker {marker}")
            elif path in (API, IMPLEMENTATION) and "0.54" not in text:
                raise SystemExit(f"{path}: missing 0.54 marker")
    if TRACKING_ISSUE not in STATUS.read_text(encoding="utf-8"):
        raise SystemExit(f"{STATUS}: missing tracking issue {TRACKING_ISSUE}")
    print("ok: RFC-0081 / D-093 / D-094 boundary and traceability")
...
def _check_versions(*, allow_planned: bool) -> None:
    ...
    if allow_planned:
        ...
        return
    if published.startswith("0.55."):
        print(f"ok: 0.54 historical under living published {published}")
        return
    if not published.startswith("0.54."):
        raise SystemExit(f"cut published version must be on 0.54.x; found {published!r}")
    ...
    if status == "uploaded":
        ...
        if not (
            pypi.startswith("0.51.")
            or pypi.startswith("0.52.")
            or pypi.startswith("0.53.")
        ):
            raise SystemExit(
                f"deferred pypi_version must stay on 0.51.x–0.53.x; found {pypi!r}"
            )
    else:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.allow_planned:
        ...
        if errors:
            raise SystemExit("\n".join(errors))
        print("ok: 0.54 planned gate shape")
    else:
        published = str(_load(RELEASE).get("release", {}).get("published_version", "")).strip()
        if published.startswith("0.55."):
            print("ok: 0.54 historical packet; skip execute-verified under living 0.55")
        else:
            command = [
                sys.executable,
    ...
    print(f"ok: verify_pkg_54 ({'allow-planned' if args.allow_planned else 'cut'})")
    return 0
