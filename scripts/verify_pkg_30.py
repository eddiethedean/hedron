#!/usr/bin/env python3
"""Verify phase 0.30 packaging / packet evidence for dual Workbench packages.

Does **not** publish or tag.

* ``--allow-planned``: validate the 0.30 evidence manifest shape while rows may
  still be Planned (packet refine).
* Omit ``--allow-planned`` at ``v0.30.0`` cut once every evidence row is
  ``Verified`` and package metadata matches the living **0.30.x** train.
"""
...
EVIDENCE = ROOT / "docs" / "acceptance" / "release-gate-0.30.toml"
INVENTORY = ROOT / "docs" / "acceptance" / "production-grade-inventory-030.toml"
LIVING_TRAIN = "0.30.0"
RELEASE_CANDIDATE = "0.30.0"
EXPECTED_PACKAGES = ("fastapi-workbench", "hedron-workbench")
...
def _check_inventory() -> None:
    ...
    if str(data.get("baseline", "")).strip() != "v0.29.0":
        raise SystemExit(f"{INVENTORY}: baseline must be v0.29.0")
    print("ok: production-grade-inventory-030.toml")


def _check_contract() -> None:
    cmd = [sys.executable, str(ROOT / "scripts" / "check_contract_030.py")]
    print("+", *cmd)
...
def _check_security_review_packet(*, allow_planned: bool) -> None:
    ...
    if allow_planned:
        print("ok: security-review-030 BRIEF (Planned packet)")
        return
    ...
    if disposition.get("critical_high_open") is not False:
        raise SystemExit("security-review-030 DISPOSITION critical_high_open must be false")
    print("ok: security-review-030 packet")
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("ok: PKG-030 packaging / packet evidence")
    return 0
