#!/usr/bin/env python3
"""PERF-030: Workbench middleware normalization budget (fastapi-workbench)."""
...
sys.path.insert(0, str(ROOT / "packages" / "fastapi-workbench" / "src"))

from fastapi_workbench.config import WorkbenchMode  # noqa: E402
from fastapi_workbench.middleware import WorkbenchPathMiddleware  # noqa: E402
...
def main() -> int:
    ...
    print(
        f"PERF-030 normalize_scope n={N} mean_ms={mean:.4f} "
        f"p95_ms={p95:.4f} budget_p95_ms={BUDGET_P95_MS}"
    )
    print(f"PERF-030 inactive_noop p95_ms={inactive_p95:.4f}")
    if max(p95, inactive_p95) > BUDGET_P95_MS:
        print(
            f"PERF-030 budget exceeded: p95 {max(p95, inactive_p95):.4f}ms > {BUDGET_P95_MS}ms",
            file=sys.stderr,
        )
        return 1
    print("ok: PERF-030")
