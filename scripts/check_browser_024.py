#!/usr/bin/env python3
"""BROWSER-024: evidence path or waive ledger consistent with DECIDE-024 disposition.
...
LEDGER = ROOT / "docs" / "acceptance" / "waive-browser-024.toml"
TERMINALS = frozenset({"waived", "superseded", "verified"})
OWNED_DEFAULT = ("BROWSER-10-001", "LIVE-011-BROWSER")
...
def _rows(data: dict[str, object]) -> list[dict[str, object]]:
    ...
    for item in raw:
        ...
        # Skip completely empty placeholder tables
        if not item:
            continue
        out.append(item)
    return out
...
def main(argv: list[str] | None = None) -> int:
    ...
    if ledger.get("disposition_gate") != "BROWSER-024":
        errors.append("waive-browser-024.toml disposition_gate must be BROWSER-024")
    ...
    if disposition == "undecided":
        ...
        # Shape-only: allow empty rows / empty evidence
        ...
        if not evidence_path.strip():
            errors.append("prove_ops requires non-empty evidence_path in waive-browser-024.toml")
        else:
    ...
    print(
        f"ok: BROWSER-024 disposition={disposition!r} rows={len(rows)} evidence={evidence_path!r}"
    )
    return 0
