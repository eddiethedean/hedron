...
def _check_versions(*, allow_planned: bool) -> None:
    ...
    if allow_planned:
        if not (
            ...
            raise SystemExit(
                f"unexpected workspace version {version!r}; Stage 0/implementation expects 0.38.x–0.42.x"
            )
        print(f"ok: living tip {version} (0.39 allow-planned)")
        return
    if version != RELEASE_CANDIDATE and not version.startswith(("0.40.", "0.41.", "0.42.")):
        raise SystemExit(f"cut requires workspace version {RELEASE_CANDIDATE}; found {version!r}")
    if version.startswith(("0.40.", "0.41.", "0.42.")):
