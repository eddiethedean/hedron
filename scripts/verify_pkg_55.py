#!/usr/bin/env python3
"""Verify the phase 0.55 workflow packet or in-tree cut."""
...
from _gate_055 import (  # noqa: E402
    API,
    EXPECTED_GATES,
    GATE,
    IMPLEMENTATION,
...
def _check_packet_files() -> None:
    ...
    if missing:
        raise SystemExit(f"missing 0.55 packet files: {missing}")
    print("ok: 0.55 contract packet files")
...
def _check_gates(*, allow_planned: bool) -> None:
    ...
    if str(_load(GATE).get("contract_refine", "")).strip() != "D-096":
        raise SystemExit(f"{GATE}: contract_refine must be D-096")
    if allow_planned:
        non_planned = [
    ...
    print("ok: release-gate-0.55.toml exact gate inventory")
...
def _check_inventory(*, allow_planned: bool) -> None:
    ...
    print("ok: workflow-inventory-055.toml")


def _check_contract() -> None:
    if not accepted_contract_present():
        raise SystemExit("RFC-0082 and D-095 must remain Accepted")
    if not contract_refine_present():
        raise SystemExit("D-096 and the frozen 0.55 contract markers must remain present")
    for path in (ROADMAP, STATUS, API, IMPLEMENTATION):
        text = path.read_text(encoding="utf-8")
        for marker in ("0.55", "D-095", "D-096"):
            if path in (ROADMAP, STATUS):
                if marker not in text:
                    raise SystemExit(f"{path}: missing 0.55 traceability marker {marker}")
            elif path in (API, IMPLEMENTATION) and "0.55" not in text:
                raise SystemExit(f"{path}: missing 0.55 marker")
    if TRACKING_ISSUE not in STATUS.read_text(encoding="utf-8"):
        raise SystemExit(f"{STATUS}: missing tracking issue {TRACKING_ISSUE}")
    print("ok: RFC-0082 / D-095 / D-096 boundary and traceability")
...
def _check_versions(*, allow_planned: bool) -> None:
    ...
    if allow_planned:
        ...
        return
    if published.startswith(("0.56.", "0.57.", "0.58.", "0.59.", "0.60.", "0.61.", "0.62.", "0.63.")):
        print(f"ok: 0.55 historical under living published {published}")
        return
    if not published.startswith("0.55."):
        raise SystemExit(f"cut published version must be on 0.55.x; found {published!r}")
    ...
    if status == "uploaded":
        ...
        if not pypi.startswith("0.54."):
            raise SystemExit(f"deferred pypi_version must stay on 0.54.x; found {pypi!r}")
    else:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.allow_planned:
        ...
        if errors:
            raise SystemExit("\n".join(errors))
        print("ok: 0.55 planned gate shape")
    else:
        published = str(_load(RELEASE).get("release", {}).get("published_version", "")).strip()
        if published.startswith(("0.56.", "0.57.", "0.58.", "0.59.", "0.60.", "0.61.", "0.62.", "0.63.")):
            print("ok: 0.55 historical packet; skip execute-verified under living tip")
        else:
            command = [
                sys.executable,
    ...
    print(f"ok: verify_pkg_55 ({'allow-planned' if args.allow_planned else 'cut'})")
    return 0
