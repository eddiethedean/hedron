#!/usr/bin/env python3
"""Verify phase 0.29 packaging / packet evidence for hedron-workbench.

Does **not** publish or tag.

* ``--allow-planned``: validate the 0.29 evidence manifest shape while rows may
  still be Planned (packet refine).
* Omit ``--allow-planned`` at ``v0.29.0`` cut once every evidence row is
  ``Verified`` and package metadata matches the living **0.29.x** train.
"""
...
EVIDENCE = ROOT / "docs" / "acceptance" / "release-gate-0.29.toml"
INVENTORY = ROOT / "docs" / "acceptance" / "production-grade-inventory-029.toml"
LIVING_TRAIN = "0.29.0"
RELEASE_CANDIDATE = "0.29.0"
EXPECTED_PACKAGES = ("hedron-workbench",)
...
def _check_inventory() -> None:
    ...
    if str(data.get("baseline", "")).strip() != "v0.28.2":
        raise SystemExit(f"{INVENTORY}: baseline must be v0.28.2")
    print("ok: production-grade-inventory-029.toml")


def _check_contract() -> None:
    cmd = [sys.executable, str(ROOT / "scripts" / "check_contract_029.py")]
    print("+", *cmd)
    subprocess.check_call(cmd, cwd=ROOT)


def _check_security_review_packet(*, allow_planned: bool) -> None:
    review = ROOT / "docs" / "acceptance" / "security-review-029"
    brief = review / "BRIEF.md"
    if not brief.is_file():
        raise SystemExit(f"missing security review artifact: {brief}")
    if allow_planned:
        print("ok: security-review-029 BRIEF (Planned packet)")
        return
    ...
    if disposition.get("critical_high_open") is not False:
        raise SystemExit("security-review-029 DISPOSITION critical_high_open must be false")
    print("ok: security-review-029 packet")
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("ok: PKG-029 packaging / packet evidence")
    return 0
