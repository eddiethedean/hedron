#!/usr/bin/env python3
"""Verify the phase 0.44 type-driven authoring planning packet or release cut.

This command never publishes or tags. Use ``--allow-planned`` only while the 0.44
rows are Planned and the published/development baseline is 0.43.0.
At the in-tree cut, omit ``--allow-planned`` and require published 0.44.0.
"""
...
def _check_packet_files() -> None:
    ...
    if missing:
        raise SystemExit(f"missing 0.44 packet files: {missing}")
    print("ok: 0.44 contract packet files")
...
def _check_gates(*, allow_planned: bool) -> None:
    ...
    print("ok: release-gate-0.44.toml exact gate inventory")
...
def _check_inventory(*, allow_planned: bool) -> None:
    ...
    print("ok: type-authoring-capability-inventory-044.toml complete requirement coverage")


def _check_contract() -> None:
    if not accepted_contract_present():
        raise SystemExit("RFC-0071 and D-072 must remain Accepted")
    if not cross_phase_refinement_present():
        raise SystemExit("D-076 and the frozen 0.44 contract markers must remain present")
    packet = (ROOT / "docs" / "acceptance" / "RELEASE_0_44.md").read_text(encoding="utf-8")
    if TRACKING_ISSUE not in packet:
        raise SystemExit(f"tracking issue {TRACKING_ISSUE} must be bound in RELEASE_0_44.md")
    for path in (ROADMAP, STATUS, API, IMPLEMENTATION):
        text = path.read_text(encoding="utf-8")
        for marker in ("0.43", "0.44", "D-072", "D-076"):
            if marker not in text:
                raise SystemExit(f"{path}: missing 0.44 traceability marker {marker}")
    print("ok: RFC-0071 / D-072 / D-076 boundary and traceability")
...
def _check_versions(*, allow_planned: bool) -> None:
    ...
    if allow_planned:
        ...
        ("0.45.", "0.46.", "0.47.", "0.48.", "0.49.", "0.50.", "0.51.", "0.52.", "0.53.", "0.54.", "0.55.", "0.56.", "0.57.", "0.58.", "0.59.", "0.58.", "0.59.", "0.58.", "0.59.", "0.58.", "0.59.", "0.58.", "0.59.", "0.58.", "0.59.", "0.58.", "0.59.", "0.58.", "0.59.", "0.58.", "0.59.", "0.58.", "0.59.")
    ):
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.allow_planned:
        ...
        if errors:
            raise SystemExit("\n".join(errors))
        print("ok: 0.44 verified historical packet")
    else:
        command = [
            sys.executable,
    ...
    print(f"ok: verify_pkg_44 ({'allow-planned' if args.allow_planned else 'cut'})")
    return 0
