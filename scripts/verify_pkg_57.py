#!/usr/bin/env python3
"""Verify the phase 0.57 unified presentation packet or in-tree cut."""
...
from _gate_057 import (  # noqa: E402
    API,
    EXPECTED_GATES,
    GATE,
    IMPLEMENTATION,
...
def _check_packet_files() -> None:
    ...
    if missing:
        raise SystemExit(f"missing 0.57 packet files: {missing}")
    print("ok: 0.57 contract packet files")
...
def _check_gates(*, allow_planned: bool) -> None:
    ...
    if str(_load(GATE).get("planning_baseline", "")).strip() != PLANNING_BASELINE:
        raise SystemExit(f"{GATE}: planning_baseline must be {PLANNING_BASELINE}")
    if str(_load(GATE).get("contract_refine", "")).strip() != "D-100":
        raise SystemExit(f"{GATE}: contract_refine must be D-100")
    if allow_planned:
        non_planned = [
    ...
    print("ok: release-gate-0.57.toml exact gate inventory")
...
def _check_inventory(*, allow_planned: bool) -> None:
    ...
    required = {
        "phase": "0.57",
        "published_predecessor": f"v{PREDECESSOR}",
        "refine_baseline": PLANNING_BASELINE,
        "target": f"v{RELEASE_CANDIDATE}",
        "decision": "D-099",
        "contract_refine": "D-100",
        "owning_rfc": "RFC-0084",
    }
    ...
    print("ok: presentation-inventory-057.toml")


def _check_contract() -> None:
    if not accepted_contract_present():
        raise SystemExit("RFC-0084 and D-099 must remain Accepted")
    if not contract_refine_present():
        raise SystemExit("D-100 and the frozen 0.57 contract markers must remain present")
    for path in (ROADMAP, STATUS, API, IMPLEMENTATION):
        text = path.read_text(encoding="utf-8")
        if "0.57" not in text:
            raise SystemExit(f"{path}: missing 0.57 traceability marker")
    if TRACKING_ISSUE not in STATUS.read_text(encoding="utf-8"):
        raise SystemExit(f"{STATUS}: missing tracking issue {TRACKING_ISSUE}")
    print("ok: RFC-0084 / D-099 / D-100 boundary and traceability")
...
def _check_versions(*, allow_planned: bool) -> None:
    ...
    if allow_planned:
        if not published.startswith("0.56."):
            raise SystemExit(f"published baseline must remain on 0.56.x; found {published!r}")
        return
    if published.startswith("0.58.") or published.startswith("0.59."):
        print(f"ok: 0.57 historical under living published {published}")
        return
    if not published.startswith("0.57."):
        raise SystemExit(f"cut published version must be on 0.57.x; found {published!r}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.allow_planned:
        ...
        if not contract_refine_present():
            raise SystemExit("D-100 and the frozen 0.57 contract markers must remain present")
        print("ok: RFC-0084 / D-099 / D-100 Stage 0 contract")
    ...
    if args.allow_planned:
        ...
        if errors:
            raise SystemExit("\n".join(errors))
        print("ok: 0.57 planned gate shape")
    else:
    ...
    print(f"ok: verify_pkg_57 ({'allow-planned' if args.allow_planned else 'cut'})")
    return 0
