...
def _check_versions(*, allow_planned: bool) -> None:
    ...
    if allow_planned:
        ...
        expected = "0.42.0"
    elif published.startswith(("0.44.", "0.45.", "0.46.", "0.47.", "0.48.", "0.49.")):
...
def main(argv: list[str] | None = None) -> int:
    ...
    elif str(_load(PYPROJECT).get("project", {}).get("version", "")).startswith(("0.44.", "0.45.", "0.46.", "0.47.", "0.48.", "0.49.")):
        errors = release_gate.check_evidence_manifest(GATE)
        if errors:
            raise SystemExit("\n".join(errors))
        print("ok: 0.43 verified historical packet")
    else:
        command = [
            sys.executable,
