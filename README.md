...
`0.59.0` patch on the `0.59.x` train. Application pins and extras:
...
Packages are **Beta** — pin a minor range (`>=0.58.0,<0.60`). For production fit, read
...
| [`hedron-data`](https://pypi.org/project/hedron-data/) | DataTable / DataEditor (also `pip install "hedron[data]>=0.58.0,<0.60"`) |
...
Charts: `pip install "hedron[charts]>=0.58.0,<0.60"`.
Maps: `pip install "hedron[maps]>=0.58.0,<0.60"`.
