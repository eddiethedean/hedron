...
uvx --from "hedron>=0.29.0,<0.30" hedron new my-hedron-app
...
Pin installs with `hedron>=0.29.0,<0.30`. Full walkthrough:
...
Hedron **0.28.x** is published (last `v0.29.0`; Beta packages — pin as above). Before
production, read
...
[What’s new in 0.28](https://hedron.readthedocs.io/en/latest/guides/whats-new-0.28/) ·
...
| [`hedron-data`](https://pypi.org/project/hedron-data/) | DataTable / DataEditor (also `pip install "hedron[data]>=0.29.0,<0.30"`) |
...
Charts require the fixed satellite floor: `pip install "hedron[charts]>=0.29.0,<0.30"`.
