...
Pin installs with `hedron>=0.45.0,<0.46`. Full walkthrough:
...
Hedron **0.45.x** is published as `v0.45.0` (Beta packages — pin as above). Before
production, read
...
| [`hedron-data`](https://pypi.org/project/hedron-data/) | DataTable / DataEditor (also `pip install "hedron[data]>=0.45.0,<0.46"`) |
...
Charts require the fixed satellite floor: `pip install "hedron[charts]>=0.45.0,<0.46"`.
