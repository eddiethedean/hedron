...
Pin installs with `hedron>=0.27.0,<0.28`. Full walkthrough:
...
Hedron **0.27.x** is published (last `v0.27.0`; Beta packages — pin as above). Before
production, read
...
[What’s new in 0.27](https://hedron.readthedocs.io/en/latest/guides/whats-new-0.27/) ·
...
| [`hedron-data`](https://pypi.org/project/hedron-data/) | DataTable / DataEditor (also `pip install "hedron[data]>=0.27.0,<0.28"`) |
...
Charts require the fixed satellite floor: `pip install "hedron[charts]>=0.27.0,<0.28"`.
Plugin authors can install `hedron-sample-kit>=0.1.7,<0.2`. Older satellite releases target
