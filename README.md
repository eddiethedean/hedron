...
The latest installable PyPI release is `0.60.2`; the repository train is `0.60.x`.
...
Packages are **Beta** — pin a minor range (`>=0.60.2,<0.61`). For production fit, read
...
| [`hedron-data`](https://pypi.org/project/hedron-data/) | DataTable / DataEditor (also `pip install "hedron[data]>=0.60.2,<0.61"`) |
...
Charts: `pip install "hedron[charts]>=0.60.2,<0.61"`.
Maps: `pip install "hedron[maps]>=0.60.2,<0.61"`.
