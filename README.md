...
uvx --from "hedron>=0.44.0,<0.45" hedron new my-hedron-app
...
Pin installs with `hedron>=0.44.0,<0.45`. Full walkthrough:
...
Hedron **0.44.x** is published as `v0.44.0` (Beta packages — pin as above). Before
production, read
...
| [`hedron-data`](https://pypi.org/project/hedron-data/) | DataTable / DataEditor (also `pip install "hedron[data]>=0.44.0,<0.45"`) |
...
Charts require the fixed satellite floor: `pip install "hedron[charts]>=0.44.0,<0.45"`.
