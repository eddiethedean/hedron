...
uvx --from "hedron>=0.30.0,<0.31" hedron new my-hedron-app
...
Pin installs with `hedron>=0.30.0,<0.31`. Full walkthrough:
...
Hedron **0.30.x** is published (last `v0.30.0`; Beta packages — pin as above). Before
production, read
...
[What’s new in 0.30](https://hedron.readthedocs.io/en/latest/guides/whats-new-0.30/) ·
...
| [`hedron-data`](https://pypi.org/project/hedron-data/) | DataTable / DataEditor (also `pip install "hedron[data]>=0.30.0,<0.31"`) |
...
Charts require the fixed satellite floor: `pip install "hedron[charts]>=0.30.0,<0.31"`.
