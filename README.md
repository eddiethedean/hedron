...
uvx --from "hedron>=0.33.0,<0.34" hedron new my-hedron-app
...
Pin installs with `hedron>=0.33.0,<0.34`. Full walkthrough:
...
Hedron **0.33.x** is published as `v0.33.0` (Beta packages — pin as above). Before
production, read
...
[What’s new in 0.32](https://hedron.readthedocs.io/en/latest/guides/whats-new-0.32/) ·
...
| [`hedron-data`](https://pypi.org/project/hedron-data/) | DataTable / DataEditor (also `pip install "hedron[data]>=0.33.0,<0.34"`) |
...
Charts require the fixed satellite floor: `pip install "hedron[charts]>=0.33.0,<0.34"`.
