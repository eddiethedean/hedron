...
> **Project status:** Phase 0.3 is published as `v0.3.0`
> (`pip install hedron`). HDN, scoped styles, themes, fingerprinted assets,
> `build`/`dev`/`inspect`/`eject`, and a minimal Web Component proof ship on top
> of the FastAPI MVP. The project is MIT-licensed. Next milestone: phase 0.4
...
The next implementation target is the developer platform and ecosystem contracts
at `v0.4.0`. See the [phase 0.4 roadmap scope](ROADMAP.md).
