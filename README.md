...
Packages are **Beta** — pin a minor range (`>=0.61.0,<0.62`). For production fit, read
...
| [`hedron-data`](https://pypi.org/project/hedron-data/) | DataTable / DataEditor (also `pip install "hedron[data]>=0.61.0,<0.62"`) |
...
Charts: `pip install "hedron[charts]>=0.61.0,<0.62"`.
Maps: `pip install "hedron[maps]>=0.61.0,<0.62"`.
