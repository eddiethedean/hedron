...
The latest installable PyPI release is `0.62.0`; the repository train is `0.62.x`.
...
Packages are **Beta** — pin a minor range (`>=0.62.0,<0.63`). For production fit, read
...
| [`hedron-data`](https://pypi.org/project/hedron-data/) | DataTable / DataEditor (also `pip install "hedron[data]>=0.62.0,<0.63"`) |
...
Charts: `pip install "hedron[charts]>=0.62.0,<0.63"`.
Maps: `pip install "hedron[maps]>=0.62.0,<0.63"`.
