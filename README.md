...
uvx --from "hedron>=0.34.0,<0.35" hedron new my-hedron-app
...
Pin installs with `hedron>=0.34.0,<0.35`. Full walkthrough:
...
Hedron **0.34.x** is published as `v0.34.0` (Beta packages — pin as above). Before
production, read
...
| [`hedron-data`](https://pypi.org/project/hedron-data/) | DataTable / DataEditor (also `pip install "hedron[data]>=0.34.0,<0.35"`) |
...
Charts require the fixed satellite floor: `pip install "hedron[charts]>=0.34.0,<0.35"`.
