...
Packages are **Beta** — pin a minor range (`>=0.59.0,<0.60`). For production fit, read
...
| [`hedron-data`](https://pypi.org/project/hedron-data/) | DataTable / DataEditor (also `pip install "hedron[data]>=0.59.0,<0.60"`) |
...
Charts: `pip install "hedron[charts]>=0.59.0,<0.60"`.
Maps: `pip install "hedron[maps]>=0.59.0,<0.60"`.
