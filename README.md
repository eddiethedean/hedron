...
Pin installs with `hedron>=0.28.0,<0.29`. Full walkthrough:
...
Hedron **0.28.x** is published (last `v0.28.0`; Beta packages — pin as above). Before
production, read
...
| [`hedron-data`](https://pypi.org/project/hedron-data/) | DataTable / DataEditor (also `pip install "hedron[data]>=0.28.0,<0.29"`) |
...
Charts require the fixed satellite floor: `pip install "hedron[charts]>=0.28.0,<0.29"`.
