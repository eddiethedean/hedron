...
Pin installs with `hedron>=0.37.0,<0.38`. Full walkthrough:
...
Hedron **0.37.x** is published as `v0.37.0` (Beta packages — pin as above). Before
production, read
...
| [`hedron-data`](https://pypi.org/project/hedron-data/) | DataTable / DataEditor (also `pip install "hedron[data]>=0.37.0,<0.38"`) |
...
Charts require the fixed satellite floor: `pip install "hedron[charts]>=0.37.0,<0.38"`.
