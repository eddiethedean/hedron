...
def test_phase044_manifest_commands_exist() -> None:
    path = ROOT / "docs" / "acceptance" / "release-gate-0.44.toml"
    data = tomllib.loads(path.read_text(encoding="utf-8"))
    rows = data["evidence"]
    found = tuple(row["id"] for row in rows)
    assert found == EXPECTED_GATES
    assert set(GATE_TESTS) == set(EXPECTED_GATES)
    assert GATE_TESTS["PKG-044"] == ["tests/unit/test_phase044_packet.py"]
    for gate_id, tests in GATE_TESTS.items():
        assert tests, gate_id
        if gate_id == "PKG-044":
            continue
        assert "tests/unit/test_phase044_packet.py" not in tests, gate_id
    for row in rows:
