"""Catalog / structural tests for the docs simulator islands."""
...
def test_every_sim_include_matches_builder_and_contract() -> None:
    ...
    # The two full showcases are generated from their real applications by
    # edron-sim / hedron-sim; the remaining islands are hand-authored contracts.
    generated_showcase_includes = {"edron-showcase", "showcase-dashboard"}
    ids = contract_ids()
    assert includes - generated_showcase_includes == ids, (
        f"missing={sorted((includes - generated_showcase_includes) - ids)} "
        f"extra={sorted(ids - (includes - generated_showcase_includes))}"
    )
