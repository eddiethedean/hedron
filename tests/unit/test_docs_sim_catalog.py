...
def test_every_sim_include_matches_builder_and_contract() -> None:
    ...
    # Edron's showcase is generated by edron-sim from examples/edron-showcase/app.py;
    # the remaining islands are the hand-authored Hedron demo contracts below.
    edron_sim_includes = {"edron-showcase"}
    ids = contract_ids()
    assert includes - edron_sim_includes == ids, (
        f"missing={sorted((includes - edron_sim_includes) - ids)} "
        f"extra={sorted(ids - (includes - edron_sim_includes))}"
    )
