"""Edron Phase 0.9 candidate packet and Hedron 0.67.0 train contract."""
...
def test_phase09_packet_is_candidate_on_hedron_067() -> None:
    ...
    assert "edron==0.9.0" in packet
    assert "Hedron `0.67.0`" in packet
    assert gates["phase"] == "0.9"
    assert gates["status"] == "Candidate"
    assert gates["version"] == "0.9.0"
    assert gates["hedron_train"] == "0.67.0"
    assert gates["hedron_requirement"] == ">=0.67.0,<0.68"
    assert gates["hedron_lock_target"] == "hedron==0.67.0"
    assert gates["hedron_forward_compatibility_target"] == "1.0.0"
    assert gates["hedron_1_0_dependency_policy"] == "do-not-declare-before-release-and-verification"
    ...
    assert all(row["state"] == "Planned" for row in gates["gate"])


def test_phase09_preserves_the_08_release_boundary() -> None:
    ...
    assert "hedron>=0.66.2,<0.67" in project["dependencies"]
    assert "hedron-data>=0.66.2,<0.67" in project["dependencies"]
