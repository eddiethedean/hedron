...
def test_phase046_manifest_commands_exist() -> None:
    path = ROOT / "docs" / "acceptance" / "release-gate-0.46.toml"
    data = tomllib.loads(path.read_text(encoding="utf-8"))
    rows = data["evidence"]
    found = tuple(row["id"] for row in rows)
    assert found == EXPECTED_GATES
    assert set(GATE_TESTS) == set(EXPECTED_GATES)
    assert GATE_TESTS["PKG-046"] == ["tests/unit/test_phase046_packet.py"]
    for gate_id, tests in GATE_TESTS.items():
        assert tests, gate_id
        if gate_id == "PKG-046":
            continue
        assert "tests/unit/test_phase046_packet.py" not in tests, gate_id
    for row in rows:
...
def test_phase046_tracking_and_contract() -> None:
    assert TRACKING_ISSUE == "#334"
    assert accepted_contract_present()
    assert contract_refine_present()
    packet = (ROOT / "docs" / "acceptance" / "RELEASE_0_46.md").read_text(encoding="utf-8")
    assert TRACKING_ISSUE in packet
    assert "0.46" in packet
