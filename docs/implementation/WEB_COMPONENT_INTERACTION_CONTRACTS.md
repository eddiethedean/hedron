...
Phase 0.37 may transfer eligible drafts across swaps/history using the bounds in the platform spec;
...
Optimistic support is first proven on bounded DataEditor/collection mutations in phase 0.35. Other
components remain server-confirmed until they pass the same evidence.

## 4. `GestureOverlayCatalog`

Phase 0.34 locks a catalog of reusable interaction behaviors so components do not independently
...
The matrix ships in phase 0.36 and becomes part of the 0.38 Supported-inventory honesty review.
...
State ownership is foundational in 0.33. Interaction and gesture/overlay contracts ship with forms
and primitives in 0.34. Optimistic mutation is proven on rich data surfaces in 0.35. The migration
matrix and optional bridge ship with authoring/interoperability in 0.36. Composition/state transfer
in 0.37 and production graduation in 0.38 test all five together.
