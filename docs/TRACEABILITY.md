...
| Developer / portable conformance tooling (RFC-0064; D-059) | Tooling-grade `hedron-conformance`, `hedron-sample-kit`, `hedron-sim`, `hedron-notebook`, Node/Java evaluators for stated development/conformance roles only | Planned phase 0.31 ([#87](https://github.com/eddiethedean/hedron/issues/87)); not application production servers | Planned `CONF-031`…`JAVA-031` + shared `REGRESS-031` / `PKG-031` — [release-gate-0.31.toml](acceptance/release-gate-0.31.toml) |
...
| Streamlit AST migration assistant (RFC-0061; D-059; extends RFC-0017 / 0024 / 0026) | `hedron migrate streamlit`, versioned mapping/report schema, source maps, stable findings, secure generated scaffold, and explicit non-parity | Planned phase 0.31 flagship CLI modules; non-executing AST resolver, migration IR/registry, and deterministic generator ([#88](https://github.com/eddiethedean/hedron/issues/88)) | Planned `MIGRATE-031` (+ shared `REGRESS-031` / `PKG-031`): no-execution/no-overwrite, no-drop mapping corpus, parity fixtures, security/a11y/perf/adversarial/packaging evidence — [release-gate-0.31.toml](acceptance/release-gate-0.31.toml) |
