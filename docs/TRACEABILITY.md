...
| Accessibility engineering / inclusive authoring (0051–0055; RFC-0023 umbrella) | `AccessibilityContract` / profile / scenarios / Explorer a11y / PE / landmarks / Page scripts / governance | Published as phase 0.19 (`v0.19.0`); available on the living `0.28.x` train | [release-gate-0.19.toml](acceptance/release-gate-0.19.toml) |
...
| Standalone FastAPI Workbench package (D-058; owning RFC required) | Monorepo-owned `fastapi-workbench` 1.0.0 with hands-off plain-FastAPI pre-import launch; `hedron-workbench` 0.30 depends on and specializes the shared generic implementation | Planned phase 0.30; independent package version does not declare Hedron 1.0 | Planned `CONTRACT-030`…`PKG-030`: package ownership, isolation, dependency inversion, 0.3.4 migration, security, real Workbench, and release evidence |
...
| Streamlit AST migration assistant (RFC-0061; extends RFC-0017 / 0024 / 0026) | `hedron migrate streamlit`, versioned mapping/report schema, source maps, stable findings, secure generated scaffold, and explicit non-parity | Planned phase 0.31 flagship CLI modules; non-executing AST resolver, migration IR/registry, and deterministic generator ([#88](https://github.com/eddiethedean/hedron/issues/88)) | Planned `MIGRATE-031`: no-execution/no-overwrite, no-drop mapping corpus, parity fixtures, security/a11y/perf/adversarial/packaging evidence |
