...
| Unified Posit deployment adapter (RFC-0066; D-061; extends RFC-0062 / 0063) | `hedron-posit`, one `HedronPosit` facade for ordinary Hedron, Workbench, and Connect; one-way `hedron-workbench` compatibility; native Connect; Supported authenticated bridge **dropped** after Stage 0 (`BRIDGE_DECISION=drop_supported`) | [Implementation plan](implementation/HEDRON_POSIT_033.md) — Planned phase 0.33; RFC-0066 **Accepted**; Stage 1 ready ([#167](https://github.com/eddiethedean/hedron/issues/167)) | [0.33 acceptance](acceptance/RELEASE_0_33.md) · [gate index](acceptance/release-gate-0.33.toml) — Planned `CONTRACT-033`…`REVIEW-033` / `DOCS-033` + shared `REGRESS-033` / `PKG-033` |
