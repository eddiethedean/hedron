...
| Production-grade MCP projection (RFC-0065; D-060; extends RFC-0043) | Deny-by-default authenticated Streamable HTTP projection; Supported inventory only; satellite `hedron-mcp` `0.2.0` Beta | Implemented — phase 0.32 **Published** in-tree (`v0.32.0`; git tag deferred; [#89](https://github.com/eddiethedean/hedron/issues/89)) | Verified `PROTOCOL-032`…`PKG-032` — [release-gate-0.32.toml](acceptance/release-gate-0.32.toml) · [RELEASE_0_32](acceptance/RELEASE_0_32.md) · [inventory-032](acceptance/production-grade-inventory-032.toml) |
| Unified Posit deployment adapter (RFC-0066; D-061; extends RFC-0062 / 0063) | `hedron-posit`, one `HedronPosit` facade for ordinary Hedron, Workbench, and Connect; one-way `hedron-workbench` compatibility; native Connect plus an explicit bounded authenticated bridge only after reproduced need | [Implementation plan](implementation/HEDRON_POSIT_033.md) — Planned phase 0.33 ([#167](https://github.com/eddiethedean/hedron/issues/167)); licensed contract probe precedes RFC acceptance; bridge off by default and Connect/application identity remain separate | [0.33 acceptance](acceptance/RELEASE_0_33.md) · [gate index](acceptance/release-gate-0.33.toml) — Planned `CONTRACT-033`…`REVIEW-033` / `DOCS-033` + shared `REGRESS-033` / `PKG-033` |
| Web Component platform (RFC-0060; extends RFC-0021 / 0025) | Draft ABI plus `ElementStateOwnership`, `InteractionState`, `OptimisticMutation`, `GestureOverlayCatalog`, `ReactMigrationMatrix`, SSR/HTMX lifecycle, rich surfaces, authoring, composition, and stable inventory | [Platform](implementation/WEB_COMPONENT_PLATFORM.md) + [interaction contracts](implementation/WEB_COMPONENT_INTERACTION_CONTRACTS.md) — draft phases 0.36–0.41 ([#92](https://github.com/eddiethedean/hedron/issues/92)–[#97](https://github.com/eddiethedean/hedron/issues/97)) | [Web Component platform acceptance](acceptance/WEB_COMPONENT_PLATFORM.md) — draft `STATE-036`, `ACTIONSTATE-037`, `INTERACT-037`, `OPTIMISTIC-038`, `MIGRATE-039`, and later gates |
