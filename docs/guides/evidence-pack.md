...
| Package verify (0.11) | CI / release checklist | [`scripts/verify_pkg_11.py`](https://github.com/eddiethedean/hedron/blob/main/scripts/verify_pkg_11.py) |
...
1. Open the GitHub Release matching your pinned train (for example `v0.11.0`).
2. Download SBOM / license / evidence-bundle assets when present.
3. Confirm package versions on PyPI match the release tag (`hedron==0.11.0`, …).
4. Optionally clone the tag and run `uv run python scripts/verify_pkg_11.py` (or the
