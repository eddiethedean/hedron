...
# or
uv add "hedron>=0.39.0,<0.40"
```

That is the current published line (`v0.39.x`). Package maturity is **Beta** — see
[How to read](../getting-started/how-to-read.md). Pin with an upper bound:
`hedron>=0.39.0,<0.40`. Using `>=0.24.0` alone (no upper bound) can resolve a future
...
For curated extras (`hedron-extras`), install `hedron[extras]>=0.39.0,<0.40`.
**Auto** (inspectable object rendering built into `hedron` — no extra) is included.
For DataTable/DataEditor, install `hedron[data]>=0.39.0,<0.40`. For charts, install
`hedron[charts]>=0.39.0,<0.40`
([Compatibility](../COMPATIBILITY.md#charts-and-sample-kit-compatibility-floor)).
For Flask/Django adapters:

```bash
pip install "hedron-flask>=0.39.0,<0.40"
pip install "hedron-django>=0.39.0,<0.40"   # requires Django >=5.2,<6
```
...
with eyes open: packages are **Beta**, pin `>=0.39.0,<0.40`, prefer polling for live
...
`uv add "hedron>=0.39.0,<0.40"`. Or use
`hedron new my-app` after `pip install "hedron>=0.39.0,<0.40"`.
...
Short version for builders: **pin `hedron>=0.39.0,<0.40`**. Packages are Beta; that does
...
automatically. Use `hedron>=0.39.0,<0.40` (and matching adapters/extras) so compatible
...
Charts install through `hedron[charts]>=0.39.0,<0.40`; the sample kit installs as
`hedron-sample-kit>=0.1.10,<0.2`. Earlier satellite versions target older cores. See
[What’s ready](whats-ready.md) and
[Compatibility](../COMPATIBILITY.md#charts-and-sample-kit-compatibility-floor).

```bash
pip install "hedron[data]>=0.39.0,<0.40"     # DataTable, DataEditor (Auto is already in hedron)
pip install "hedron[charts]>=0.39.0,<0.40"   # compatible chart satellite
```
...
```bash
pip install "hedron[jinja]>=0.39.0,<0.40"
# or
uv add "hedron[jinja]>=0.39.0,<0.40"
```
...
Prefer GitHub Release assets for the current train tag (`v0.39.0`), or regenerate from the
