...
`pip install "hedron>=0.62.0,<0.63"`. Contributors working from a git checkout use
...
```bash
pip install "hedron>=0.62.0,<0.63"
# or
uv add "hedron>=0.62.0,<0.63"
```
...
For DataTable/DataEditor, install `hedron[data]>=0.62.0,<0.63`. For charts, install
`hedron[charts]>=0.62.0,<0.63`
([Compatibility](../COMPATIBILITY.md#charts-and-sample-kit-compatibility-floor)).
Flask/Django adapters:

```bash
pip install "hedron-flask>=0.62.0,<0.63"
pip install "hedron-django>=0.62.0,<0.63"   # requires Django >=5.2,<6
```
...
with eyes open: packages are **Beta**, pin `>=0.62.0,<0.63`, prefer polling
...
`uv add "hedron>=0.62.0,<0.63"`. Or use
`hedron new my-app` after `pip install "hedron>=0.62.0,<0.63"`.
...
Short version for builders: **pin from PyPI** (`hedron>=0.62.0,<0.63`). The repository
...
Install with an upper bound — `hedron>=0.62.0,<0.63` — so you get patches automatically
...
automatically. Use `hedron>=0.62.0,<0.63` so compatible patches are accepted while the
...
Charts install through `hedron[charts]>=0.62.0,<0.63`; the sample kit installs as
`hedron-sample-kit>=0.2.1,<0.3`. Earlier satellite versions target older cores. See
[What’s ready](whats-ready.md) and
[Compatibility](../COMPATIBILITY.md#charts-and-sample-kit-compatibility-floor).

```bash
pip install "hedron[data]>=0.62.0,<0.63"     # DataTable, DataEditor (Auto is already in hedron)
pip install "hedron[charts]>=0.62.0,<0.63"   # compatible chart satellite
```
...
```bash
pip install "hedron[jinja]>=0.62.0,<0.63"
# or
uv add "hedron[jinja]>=0.62.0,<0.63"
```
