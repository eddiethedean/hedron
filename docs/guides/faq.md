...
Install the current train: `hedron>=0.50.0,<0.51`. Full pin table:
[Installation](../getting-started/installation.md).

```bash
pip install "hedron>=0.50.0,<0.51"
# or
uv add "hedron>=0.50.0,<0.51"
```
...
For DataTable/DataEditor, install `hedron[data]>=0.50.0,<0.51`. For charts, install
`hedron[charts]>=0.50.0,<0.51`
([Compatibility](../COMPATIBILITY.md#charts-and-sample-kit-compatibility-floor)).
Flask/Django adapters:

```bash
pip install "hedron-flask>=0.50.0,<0.51"
pip install "hedron-django>=0.50.0,<0.51"   # requires Django >=5.2,<6
```
...
with eyes open: packages are **Beta**, pin `>=0.50.0,<0.51`, prefer polling
...
`uv add "hedron>=0.50.0,<0.51"`. Or use
`hedron new my-app` after `pip install "hedron>=0.50.0,<0.51"`.
...
Short version for builders: **pin the current train** (`hedron>=0.50.0,<0.51`).
...
automatically. Use `hedron>=0.50.0,<0.51` so compatible patches are accepted while the
...
Charts install through `hedron[charts]>=0.50.0,<0.51`; the sample kit installs as
`hedron-sample-kit>=0.1.10,<0.2`. Earlier satellite versions target older cores. See
[What’s ready](whats-ready.md) and
[Compatibility](../COMPATIBILITY.md#charts-and-sample-kit-compatibility-floor).

```bash
pip install "hedron[data]>=0.50.0,<0.51"     # DataTable, DataEditor (Auto is already in hedron)
pip install "hedron[charts]>=0.50.0,<0.51"   # compatible chart satellite
```
...
# or
uv add "hedron[jinja]>=0.50.0,<0.51"
```
