...
# or
uv add "hedron>=0.14.0"
```

That installs the current published train from PyPI (**0.14.0**)—see
...
1. Prefer **`uv tool install "hedron>=0.14.0"`** (or `pipx install`), then **re-open the shell**.
...
`hedron new my-app` after `pip install "hedron>=0.14.0"`.
