...
```bash
pip install "hedron>=0.35.0,<0.36"
# or
...
For curated extras (`hedron-extras`), install `hedron[extras]>=0.35.0,<0.36`.
**Auto** (inspectable object rendering built into `hedron` — no extra) is included.
For DataTable/DataEditor, install `hedron[data]>=0.35.0,<0.36`. For charts, install
`hedron[charts]>=0.35.0,<0.36`
([Compatibility](../COMPATIBILITY.md#charts-and-sample-kit-compatibility-floor)).
For Flask/Django adapters:

```bash
pip install "hedron-flask>=0.35.0,<0.36"
pip install "hedron-django>=0.35.0,<0.36"   # requires Django >=5.2,<6
```
...
with eyes open: packages are **Beta**, pin `>=0.35.0,<0.36`, prefer polling for live
...
`uv add "hedron>=0.35.0,<0.36"`. Or use
`hedron new my-app` after `pip install "hedron>=0.35.0,<0.36"`.
...
Short version for builders: **pin `hedron>=0.35.0,<0.36`**. Packages are Beta; that does
...
automatically. Use `hedron>=0.35.0,<0.36` (and matching adapters/extras) so compatible
...
Charts install through `hedron[charts]>=0.35.0,<0.36`; the sample kit installs as
`hedron-sample-kit>=0.1.10,<0.2`. Earlier satellite versions target older cores. See
[What’s ready](whats-ready.md) and
[Compatibility](../COMPATIBILITY.md#charts-and-sample-kit-compatibility-floor).

```bash
pip install "hedron[data]>=0.35.0,<0.36"     # DataTable, DataEditor (Auto is already in hedron)
pip install "hedron[charts]>=0.35.0,<0.36"   # compatible chart satellite
```
...
```bash
pip install "hedron[jinja]>=0.35.0,<0.36"
# or
uv add "hedron[jinja]>=0.35.0,<0.36"
```
