...
```bash
pip install "hedron>=0.59.0,<0.60"
# or
uv add "hedron>=0.59.0,<0.60"
```
...
For DataTable/DataEditor, install `hedron[data]>=0.59.0,<0.60`. For charts, install
`hedron[charts]>=0.59.0,<0.60`
([Compatibility](../COMPATIBILITY.md#charts-and-sample-kit-compatibility-floor)).
Flask/Django adapters:

```bash
pip install "hedron-flask>=0.59.0,<0.60"
pip install "hedron-django>=0.59.0,<0.60"   # requires Django >=5.2,<6
```
...
with eyes open: packages are **Beta**, pin `>=0.59.0,<0.60`, prefer polling
...
`uv add "hedron>=0.59.0,<0.60"`. Or use
`hedron new my-app` after `pip install "hedron>=0.59.0,<0.60"`.
...
Short version for builders: **pin from PyPI** (`hedron>=0.59.0,<0.60`). The repository
...
automatically. Use `hedron>=0.59.0,<0.60` so compatible patches are accepted while the
...
Charts install through `hedron[charts]>=0.59.0,<0.60`; the sample kit installs as
`hedron-sample-kit>=0.2.0,<0.3`. Earlier satellite versions target older cores. See
[What’s ready](whats-ready.md) and
[Compatibility](../COMPATIBILITY.md#charts-and-sample-kit-compatibility-floor).

```bash
pip install "hedron[data]>=0.59.0,<0.60"     # DataTable, DataEditor (Auto is already in hedron)
pip install "hedron[charts]>=0.59.0,<0.60"   # compatible chart satellite
```
...
```bash
pip install "hedron[jinja]>=0.59.0,<0.60"
# or
uv add "hedron[jinja]>=0.59.0,<0.60"
```
