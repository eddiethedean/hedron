...
```bash
pip install "hedron>=0.41.0,<0.42"
# or
uv add "hedron>=0.41.0,<0.42"
```

That is the current published line (`v0.41.x`). Package maturity is **Beta** — see
...
For curated extras (`hedron-extras`), install `hedron[extras]>=0.41.0,<0.42`.
**Auto** (inspectable object rendering built into `hedron` — no extra) is included.
For DataTable/DataEditor, install `hedron[data]>=0.41.0,<0.42`. For charts, install
`hedron[charts]>=0.41.0,<0.42`
([Compatibility](../COMPATIBILITY.md#charts-and-sample-kit-compatibility-floor)).
For Flask/Django adapters:

```bash
pip install "hedron-flask>=0.41.0,<0.42"
pip install "hedron-django>=0.41.0,<0.42"   # requires Django >=5.2,<6
```
...
with eyes open: packages are **Beta**, pin `>=0.41.0,<0.42`, prefer polling for live
...
`uv add "hedron>=0.41.0,<0.42"`. Or use
`hedron new my-app` after `pip install "hedron>=0.41.0,<0.42"`.
...
Short version for builders: **pin `hedron>=0.41.0,<0.42`**. Packages are Beta; that does
...
automatically. Use `hedron>=0.41.0,<0.42` (and matching adapters/extras) so compatible
...
Charts install through `hedron[charts]>=0.41.0,<0.42`; the sample kit installs as
`hedron-sample-kit>=0.1.10,<0.2`. Earlier satellite versions target older cores. See
[What’s ready](whats-ready.md) and
[Compatibility](../COMPATIBILITY.md#charts-and-sample-kit-compatibility-floor).

```bash
pip install "hedron[data]>=0.41.0,<0.42"     # DataTable, DataEditor (Auto is already in hedron)
pip install "hedron[charts]>=0.41.0,<0.42"   # compatible chart satellite
```
...
Prefer GitHub Release assets for the current train tag (`v0.41.0`), or regenerate from the
