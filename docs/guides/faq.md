...
`pip install "hedron>=0.60.2,<0.61"`. Contributors working from a git checkout use
`uv sync` (workspace packages); the verified 0.60.2 checkout is not yet on PyPI.
The Git tag is cut only after hosted CI is green; PyPI upload remains a separate coordinated step. See
[Installation](../getting-started/installation.md).

```bash
pip install "hedron>=0.60.2,<0.61"
# or
uv add "hedron>=0.60.2,<0.61"
```
...
For DataTable/DataEditor, install `hedron[data]>=0.60.2,<0.61`. For charts, install
`hedron[charts]>=0.60.2,<0.61`
([Compatibility](../COMPATIBILITY.md#charts-and-sample-kit-compatibility-floor)).
Flask/Django adapters:

```bash
pip install "hedron-flask>=0.60.2,<0.61"
pip install "hedron-django>=0.60.2,<0.61"   # requires Django >=5.2,<6
```
...
with eyes open: packages are **Beta**, pin `>=0.60.2,<0.61`, prefer polling
...
`uv add "hedron>=0.60.2,<0.61"`. Or use
`hedron new my-app` after `pip install "hedron>=0.60.2,<0.61"`.
...
Short version for builders: **pin from PyPI** (`hedron>=0.60.2,<0.61`). The repository
...
Install with an upper bound — `hedron>=0.60.2,<0.61` — so you get patches automatically
...
automatically. Use `hedron>=0.60.2,<0.61` so compatible patches are accepted while the
...
Charts install through `hedron[charts]>=0.60.2,<0.61`; the sample kit installs as
`hedron-sample-kit>=0.2.0,<0.3`. Earlier satellite versions target older cores. See
[What’s ready](whats-ready.md) and
[Compatibility](../COMPATIBILITY.md#charts-and-sample-kit-compatibility-floor).

```bash
pip install "hedron[data]>=0.60.2,<0.61"     # DataTable, DataEditor (Auto is already in hedron)
pip install "hedron[charts]>=0.60.2,<0.61"   # compatible chart satellite
```
...
# or
uv add "hedron[jinja]>=0.60.2,<0.61"
```
