...
```bash
pip install "hedron>=0.27.0,<0.28"
# or
uv add "hedron>=0.27.0,<0.28"
```

That is the current published line (`v0.27.0`). Package maturity is **Beta** — see
[How to read](../getting-started/how-to-read.md). Pin with an upper bound:
`hedron>=0.27.0,<0.28`. Using `>=0.24.0` alone (no upper bound) can resolve a future
...
For curated extras (`hedron-extras`), install `hedron[extras]>=0.27.0,<0.28`.
**Auto** (inspectable object rendering built into `hedron` — no extra) is included.
For DataTable/DataEditor, install `hedron[data]>=0.27.0,<0.28`. For charts, install
`hedron[charts]>=0.27.0,<0.28`
([Compatibility](../COMPATIBILITY.md#charts-and-sample-kit-compatibility-floor)).
For Flask/Django adapters:

```bash
pip install "hedron-flask>=0.27.0,<0.28"
pip install "hedron-django>=0.27.0,<0.28"   # requires Django >=5.2,<6
```
...
with eyes open: packages are **Beta**, pin `>=0.27.0,<0.28`, prefer polling for live
...
`uv add "hedron>=0.27.0,<0.28"`. Or use
`hedron new my-app` after `pip install "hedron>=0.27.0,<0.28"`.

## Should I use `uv init` or `hedron new`?

Prefer **`hedron new`** for a ready scaffold on **0.27.x** (install Hedron first).
...
Short version for builders: **pin `hedron>=0.27.0,<0.28`**. Packages are Beta; that does
...
automatically. Use `hedron>=0.27.0,<0.28` (and matching adapters/extras) so compatible
...
```bash
pip install "hedron[data]>=0.27.0,<0.28"     # DataTable, DataEditor (Auto is already in hedron)
pip install "hedron[charts]>=0.27.0,<0.28"   # compatible chart satellite
```
...
# or
uv add "hedron[jinja]>=0.27.0,<0.28"
```
...
Prefer GitHub Release assets for the train tag (`v0.27.0`), or regenerate from the
