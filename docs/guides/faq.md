...
For curated extras (`hedron-extras`), install `hedron[extras]>=0.22.0,<0.23`.
**Auto** (inspectable object rendering built into `hedron` — no extra) is included.
For DataTable/DataEditor, install `hedron[data]>=0.22.0,<0.23`. For charts, install
`hedron[charts]>=0.1.0,<0.2` (Alpha). For Flask/Django adapters:

```bash
pip install "hedron-flask>=0.22.0,<0.23"
pip install "hedron-django>=0.22.0,<0.23"   # requires Django >=5.2,<6
```
...
`uv add "hedron>=0.22.0,<0.23"`. Or use
`hedron new my-app` after `pip install "hedron>=0.22.0,<0.23"`.

## Should I use `uv init` or `hedron new`?

Prefer **`hedron new`** for a ready scaffold on **0.22.x** (install Hedron first).
...
- **Beta / Alpha** — **package** maturity on PyPI; pin versions (`>=0.22.0,<0.23`).
...
```bash
pip install "hedron[data]>=0.22.0,<0.23"     # DataTable, DataEditor (Auto is already in hedron)
...
```bash
pip install "hedron[jinja]>=0.22.0,<0.23"
# or
uv add "hedron[jinja]>=0.22.0,<0.23"
```
