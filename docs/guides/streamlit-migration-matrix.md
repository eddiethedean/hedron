...
| `st.line_chart`, `st.area_chart`, `st.bar_chart`, `st.scatter_chart` | Hedron chart components | **Partial** | Install `hedron[charts]>=0.50.1,<0.51`; provide accessible titles/descriptions/fallbacks. |
...
!!! note "Hedron 0.27 chart floor"
    ...
    Use `hedron[charts]>=0.50.1,<0.51`, which enforces
