...
2. Prefer `uv tool install "hedron>=0.48.0,<0.49"` (or `pipx install "hedron>=0.48.0,<0.49"`) so the
...
(`hedron>=0.48.0,<0.49`), then add other dependencies. For first apps, prefer staying
...
**Fix:** Edit `pyproject.toml` to `hedron>=0.48.0,<0.49` and `uvicorn[standard]>=0.30`, then
reinstall. Current `hedron new` scaffolds `hedron>=0.48.0,<0.49` automatically.
...
Hedron (`>=0.48.0,<0.49` on PyPI today).

**Fix:**

```bash
# Auto needs no extra
pip install "hedron[data]>=0.48.0,<0.49"      # DataTable, DataEditor
pip install "hedron[charts]>=0.48.0,<0.49"   # chart components
```
