...
2. Prefer `uv tool install "hedron>=0.43.0,<0.44"` (or `pipx install "hedron>=0.43.0,<0.44"`) so the
...
(`hedron>=0.43.0,<0.44`), then add other dependencies. For first apps, prefer staying
...
`pip install -U "hedron>=0.43.0,<0.44"` (or `uv add "hedron>=0.43.0,<0.44"`). The current
train is **0.43.x** (Published; last published PyPI/git = `v0.43.0`)—see
...
relevant when upgrading from earlier 0.25 lines onto `>=0.43.0,<0.44`. If docs describe a
feature missing from your install, upgrade to the current `0.39.x` pin
(`hedron>=0.43.0,<0.44`) or switch the docs to the tag matching your installed release.
...
**Fix:** Edit `pyproject.toml` to `hedron>=0.43.0,<0.44` and `uvicorn[standard]>=0.30`, then
reinstall. Current `hedron new` scaffolds `hedron>=0.43.0,<0.44` automatically.
...
```bash
# Auto needs no extra
pip install "hedron[data]>=0.43.0,<0.44"      # DataTable, DataEditor
pip install "hedron[charts]>=0.43.0,<0.44"   # chart components
```
