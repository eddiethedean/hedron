...
2. Prefer `uv tool install "hedron>=0.63.0,<0.64"` (or `pipx install "hedron>=0.63.0,<0.64"`) so the
...
(`hedron>=0.63.0,<0.64`), then add other dependencies. For first apps, prefer staying
...
Expect **`0.63.0`** in both a PyPI-installed application and source checkouts.
Upgrade an application with `pip install -U "hedron>=0.63.0,<0.64"`; use `uv sync` only
...
**Fix:** Edit `pyproject.toml` to `hedron>=0.63.0,<0.64` and `uvicorn[standard]>=0.30`, then
reinstall. Current `hedron new` scaffolds `hedron>=0.63.0,<0.64` automatically.
...
```bash
# Auto needs no extra
pip install "hedron[data]>=0.63.0,<0.64"      # DataTable, DataEditor
pip install "hedron[charts]>=0.63.0,<0.64"   # chart components
```
