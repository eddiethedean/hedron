...
2. Prefer `uv tool install "hedron>=0.17.0"` (or `pipx install "hedron>=0.17.0"`) so the
...
`pip install -U "hedron>=0.17.0"` (or `uv add "hedron>=0.17.0"`). The current published
...
**Fix:** Edit `pyproject.toml` to `hedron>=0.17.0` and `uvicorn[standard]>=0.30`, then
reinstall. Current `hedron new` scaffolds `hedron>=0.17.0` automatically.
