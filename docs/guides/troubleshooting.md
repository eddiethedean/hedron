...
2. Prefer `uv tool install "hedron>=0.21.0,<0.22"` (or `pipx install "hedron>=0.21.0,<0.22"`) so the
...
`pip install -U "hedron>=0.21.0,<0.22"` (or `uv add "hedron>=0.21.0,<0.22"`). The current
train is **0.21.x** (Published; last published PyPI/git = `v0.21.0`)—see
...
(`hedron>=0.21.0,<0.22`) or use a git checkout of that work.
...
**Fix:** Edit `pyproject.toml` to `hedron>=0.21.0,<0.22` and `uvicorn[standard]>=0.30`, then
reinstall. Current `hedron new` scaffolds `hedron>=0.21.0,<0.22` automatically.
...
```bash
# Auto needs no extra
pip install "hedron[data]>=0.21.0,<0.22"      # DataTable, DataEditor
