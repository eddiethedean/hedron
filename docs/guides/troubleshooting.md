...
2. Prefer `uv tool install "hedron>=0.28.0,<0.29"` (or `pipx install "hedron>=0.28.0,<0.29"`) so the
...
(`hedron>=0.28.0,<0.29`), then add other dependencies. For first apps, prefer staying
...
`pip install -U "hedron>=0.28.0,<0.29"` (or `uv add "hedron>=0.28.0,<0.29"`). The current
train is **0.28.x** (Published; last published PyPI/git = `v0.28.0`)—see

Living tip is Published **`v0.28.0`**. The `v0.25.2` patch hardened mount `..` / `%2e`, RedisStatusStore / Celery–RQ cancel CAS, adapter prepare under a running loop, SSE/streaming `Cache-Control`, and root-relative SafeUrl/Hx attrs — still relevant when upgrading from earlier 0.25 lines onto `>=0.28.0,<0.29`.
...
(`hedron>=0.28.0,<0.29`) or use a tagged checkout of that release.
...
**Fix:** Edit `pyproject.toml` to `hedron>=0.28.0,<0.29` and `uvicorn[standard]>=0.30`, then
reinstall. Current `hedron new` scaffolds `hedron>=0.28.0,<0.29` automatically.
...
```bash
# Auto needs no extra
pip install "hedron[data]>=0.28.0,<0.29"      # DataTable, DataEditor
pip install "hedron[charts]>=0.28.0,<0.29"   # chart components
```
