...
2. Prefer `uv tool install "hedron>=0.62.0,<0.63"` (or `pipx install "hedron>=0.62.0,<0.63"`) so the
...
(`hedron>=0.62.0,<0.63`), then add other dependencies. For first apps, prefer staying
...
Expect **`0.62.0`** in both a PyPI-installed application and source checkouts.
Upgrade an application with `pip install -U "hedron>=0.62.0,<0.63"`; use `uv sync` only
...
```bash
# Auto needs no extra
pip install "hedron[data]>=0.62.0,<0.63"      # DataTable, DataEditor
pip install "hedron[charts]>=0.62.0,<0.63"   # chart components
```
