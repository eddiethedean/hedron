...
2. Prefer `uv tool install "hedron>=0.37.0,<0.38"` (or `pipx install "hedron>=0.37.0,<0.38"`) so the
...
(`hedron>=0.37.0,<0.38`), then add other dependencies. For first apps, prefer staying
...
`pip install -U "hedron>=0.37.0,<0.38"` (or `uv add "hedron>=0.37.0,<0.38"`). The current
train is **0.37.x** (Published; last published PyPI/git = `v0.37.0`)—see
...
relevant when upgrading from earlier 0.25 lines onto `>=0.37.0,<0.38`. If docs describe a
feature missing from your install, upgrade to a matching `0.30.x` pin
(`hedron>=0.37.0,<0.38`) or use a tagged checkout of that release.
...
**Fix:** Edit `pyproject.toml` to `hedron>=0.37.0,<0.38` and `uvicorn[standard]>=0.30`, then
reinstall. Current `hedron new` scaffolds `hedron>=0.37.0,<0.38` automatically.
...
```bash
# Auto needs no extra
pip install "hedron[data]>=0.37.0,<0.38"      # DataTable, DataEditor
pip install "hedron[charts]>=0.37.0,<0.38"   # chart components
```
