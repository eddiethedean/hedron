...
2. Prefer `uv tool install "hedron>=0.46.0,<0.47"` (or `pipx install "hedron>=0.46.0,<0.47"`) so the
...
(`hedron>=0.46.0,<0.47`), then add other dependencies. For first apps, prefer staying
...
**Fix:** Edit `pyproject.toml` to `hedron>=0.46.0,<0.47` and `uvicorn[standard]>=0.30`, then
reinstall. Current `hedron new` scaffolds `hedron>=0.46.0,<0.47` automatically.
...
```bash
# Auto needs no extra
pip install "hedron[data]>=0.46.0,<0.47"      # DataTable, DataEditor
