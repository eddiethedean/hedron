...
2. Prefer `uv tool install "hedron>=0.47.0,<0.48"` (or `pipx install "hedron>=0.47.0,<0.48"`) so the
...
(`hedron>=0.47.0,<0.48`), then add other dependencies. For first apps, prefer staying
...
`pip install -U "hedron>=0.47.0,<0.48"` (or `uv add "hedron>=0.47.0,<0.48"`).
...
**Fix:** Edit `pyproject.toml` to `hedron>=0.47.0,<0.48` and `uvicorn[standard]>=0.30`, then
reinstall. Current `hedron new` scaffolds `hedron>=0.47.0,<0.48` automatically.
...
Hedron (`>=0.47.0,<0.48` on PyPI today).

**Fix:**

```bash
# Auto needs no extra
pip install "hedron[data]>=0.47.0,<0.48"      # DataTable, DataEditor
pip install "hedron[charts]>=0.47.0,<0.48"   # chart components
```
