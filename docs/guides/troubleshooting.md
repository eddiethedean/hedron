...
2. Prefer `uv tool install "hedron>=0.64.0,<0.65"` (or `pipx install "hedron>=0.64.0,<0.65"`) so the
...
(`hedron>=0.64.0,<0.65`), then add other dependencies. For first apps, prefer staying
...
Upgrade an application with `pip install -U "hedron>=0.64.0,<0.65"`; use `uv sync` only
...
**Fix:** Edit `pyproject.toml` to `hedron>=0.64.0,<0.65` and `uvicorn[standard]>=0.30`, then
reinstall. Current `hedron new` scaffolds `hedron>=0.64.0,<0.65` automatically.
...
```bash
# Auto needs no extra
pip install "hedron[data]>=0.64.0,<0.65"      # DataTable, DataEditor
pip install "hedron[charts]>=0.64.0,<0.65"   # chart components
```
