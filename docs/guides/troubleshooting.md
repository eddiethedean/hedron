...
2. Prefer `uv tool install "hedron>=0.30.0,<0.31"` (or `pipx install "hedron>=0.30.0,<0.31"`) so the
...
(`hedron>=0.30.0,<0.31`), then add other dependencies. For first apps, prefer staying
...
**Fix:** Edit `pyproject.toml` to `hedron>=0.30.0,<0.31` and `uvicorn[standard]>=0.30`, then
reinstall. Current `hedron new` scaffolds `hedron>=0.30.0,<0.31` automatically.
...
```bash
# Auto needs no extra
pip install "hedron[data]>=0.30.0,<0.31"      # DataTable, DataEditor
pip install "hedron[charts]>=0.30.0,<0.31"   # chart components
```
