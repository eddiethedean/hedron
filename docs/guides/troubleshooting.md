...
2. Prefer `uv tool install "hedron>=0.60.2,<0.61"` (or `pipx install "hedron>=0.60.2,<0.61"`) so the
...
(`hedron>=0.60.2,<0.61`), then add other dependencies. For first apps, prefer staying
...
Expect **`0.60.2`** in both a PyPI-installed application and source checkouts.
Upgrade an application with `pip install -U "hedron>=0.60.2,<0.61"`; use `uv sync` only
...
**Fix:** Edit `pyproject.toml` to `hedron>=0.60.2,<0.61` and `uvicorn[standard]>=0.30`, then
reinstall. Current `hedron new` scaffolds `hedron>=0.60.2,<0.61` automatically.
...
```bash
# Auto needs no extra
pip install "hedron[data]>=0.60.2,<0.61"      # DataTable, DataEditor
pip install "hedron[charts]>=0.60.2,<0.61"   # chart components
```
