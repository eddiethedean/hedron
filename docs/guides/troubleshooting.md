...
2. Prefer `uv tool install "hedron>=0.38.0,<0.39"` (or `pipx install "hedron>=0.38.0,<0.39"`) so the
...
(`hedron>=0.38.0,<0.39`), then add other dependencies. For first apps, prefer staying
...
`pip install -U "hedron>=0.38.0,<0.39"` (or `uv add "hedron>=0.38.0,<0.39"`). The current
train is **0.38.x** (Published; last published PyPI/git = `v0.38.0`)—see
...
**Fix:** Edit `pyproject.toml` to `hedron>=0.38.0,<0.39` and `uvicorn[standard]>=0.30`, then
reinstall. Current `hedron new` scaffolds `hedron>=0.38.0,<0.39` automatically.
...
```bash
# Auto needs no extra
pip install "hedron[data]>=0.38.0,<0.39"      # DataTable, DataEditor
