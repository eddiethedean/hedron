...
2. Prefer `uv tool install "hedron>=0.56.0,<0.58"` (or `pipx install "hedron>=0.56.0,<0.58"`) so the
...
(`hedron>=0.56.0,<0.58`), then add other dependencies. For first apps, prefer staying
...
Upgrade an application with `pip install -U "hedron>=0.56.0,<0.58"`; use `uv sync` only
...
**Fix:** Edit `pyproject.toml` to `hedron>=0.56.0,<0.58` and `uvicorn[standard]>=0.30`, then
reinstall. Current `hedron new` scaffolds `hedron>=0.56.0,<0.58` automatically.
...
```bash
# Auto needs no extra
pip install "hedron[data]>=0.56.0,<0.58"      # DataTable, DataEditor
pip install "hedron[charts]>=0.56.0,<0.58"   # chart components
```
