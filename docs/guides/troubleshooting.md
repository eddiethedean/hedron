...
2. Prefer `uv tool install "hedron>=0.35.0,<0.36"` (or `pipx install "hedron>=0.35.0,<0.36"`) so the
...
(`hedron>=0.35.0,<0.36`), then add other dependencies. For first apps, prefer staying
...
`pip install -U "hedron>=0.35.0,<0.36"` (or `uv add "hedron>=0.35.0,<0.36"`). The current
train is **0.35.x** (Published; last published PyPI/git = `v0.35.0`)—see
...
relevant when upgrading from earlier 0.25 lines onto `>=0.35.0,<0.36`. If docs describe a
feature missing from your install, upgrade to a matching `0.30.x` pin
(`hedron>=0.35.0,<0.36`) or use a tagged checkout of that release.
...
**Fix:** Edit `pyproject.toml` to `hedron>=0.35.0,<0.36` and `uvicorn[standard]>=0.30`, then
reinstall. Current `hedron new` scaffolds `hedron>=0.35.0,<0.36` automatically.
...
```bash
# Auto needs no extra
pip install "hedron[data]>=0.35.0,<0.36"      # DataTable, DataEditor
pip install "hedron[charts]>=0.35.0,<0.36"   # chart components
```
