...
2. Prefer `uv tool install "hedron>=0.23.0,<0.24"` (or `pipx install "hedron>=0.23.0,<0.24"`) so the
...
`pip install -U "hedron>=0.23.0,<0.24"` (or `uv add "hedron>=0.23.0,<0.24"`). The current
train is **0.23.x** (Published; last published PyPI/git = `v0.23.0`)—see
[What's ready](whats-ready.md) and the [public roadmap](roadmap.md). If docs describe a
feature missing from your install, upgrade to a matching `0.23.x` pin
(`hedron>=0.23.0,<0.24`) or use a git checkout of that work.
...
**Fix:** Edit `pyproject.toml` to `hedron>=0.23.0,<0.24` and `uvicorn[standard]>=0.30`, then
reinstall. Current `hedron new` scaffolds `hedron>=0.23.0,<0.24` automatically.
...
```bash
# Auto needs no extra
pip install "hedron[data]>=0.23.0,<0.24"      # DataTable, DataEditor
