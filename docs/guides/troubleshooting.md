...
2. Prefer `uv tool install "hedron>=0.61.0,<0.62"` (or `pipx install "hedron>=0.61.0,<0.62"`) so the
...
(`hedron>=0.61.0,<0.62`), then add other dependencies. For first apps, prefer staying
...
Expect **`0.61.0`** in both a PyPI-installed application and source checkouts.
Upgrade an application with `pip install -U "hedron>=0.61.0,<0.62"`; use `uv sync` only
...
```bash
# Auto needs no extra
pip install "hedron[data]>=0.61.0,<0.62"      # DataTable, DataEditor
pip install "hedron[charts]>=0.61.0,<0.62"   # chart components
```
