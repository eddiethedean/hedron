...
2. Prefer `uv tool install "hedron>=0.24.0,<0.25"` (or `pipx install "hedron>=0.24.0,<0.25"`) so the
...
`pip install -U "hedron>=0.24.0,<0.25"` (or `uv add "hedron>=0.24.0,<0.25"`). The current
train is **0.24.x** (Published; last published PyPI/git = `v0.24.0`)—see
[What's ready](whats-ready.md) and the [public roadmap](roadmap.md). If docs describe a
feature missing from your install, upgrade to a matching `0.24.x` pin
(`hedron>=0.24.0,<0.25`) or use a git checkout of that work.
...
**Fix:** Edit `pyproject.toml` to `hedron>=0.24.0,<0.25` and `uvicorn[standard]>=0.30`, then
reinstall. Current `hedron new` scaffolds `hedron>=0.24.0,<0.25` automatically.
...
```bash
# Auto needs no extra
pip install "hedron[data]>=0.24.0,<0.25"      # DataTable, DataEditor
