...
2. Prefer `uv tool install "hedron>=0.22.0,<0.23"` (or `pipx install "hedron>=0.22.0,<0.23"`) so the
...
train is **0.22.x** (Published; last published PyPI/git = `v0.22.0`)—see
...
**Fix:** Edit `pyproject.toml` to `hedron>=0.22.0,<0.23` and `uvicorn[standard]>=0.30`, then
reinstall. Current `hedron new` scaffolds `hedron>=0.22.0,<0.23` automatically.
...
```bash
# Auto needs no extra
pip install "hedron[data]>=0.22.0,<0.23"      # DataTable, DataEditor
