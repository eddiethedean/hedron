...
2. Prefer `uv tool install "hedron>=0.58.0,<0.60"` (or `pipx install "hedron>=0.58.0,<0.60"`) so the
...
(`hedron>=0.58.0,<0.60`), then add other dependencies. For first apps, prefer staying
...
Upgrade an application with `pip install -U "hedron>=0.58.0,<0.60"`; use `uv sync` only
...
**Fix:** Edit `pyproject.toml` to `hedron>=0.58.0,<0.60` and `uvicorn[standard]>=0.30`, then
reinstall. Current `hedron new` scaffolds `hedron>=0.58.0,<0.60` automatically.
...
```bash
# Auto needs no extra
pip install "hedron[data]>=0.58.0,<0.60"      # DataTable, DataEditor
pip install "hedron[charts]>=0.58.0,<0.60"   # chart components
```
