...
| Charts | **Supported** first-party and Matplotlib/static paths on Beta package | Install `hedron[charts]>=0.64.0,<0.65`; `ChartSpec` / `hedron-chart` and Matplotlib/static are Supported; Plotly / Altair remain **Experimental** ([Chart API](../api/CHART.md)) |
...
| Application DX Stage 1 | **Supported** (API `beta`; repository train `v0.64.0`) | Assets, diagnostics, routes, workflows, testgen, theming, discovery, fleet — [Application DX API](../api/APPLICATION_DX.md) · [What’s new in 0.53](whats-new-0.53.md) |
| Authoring loop / chrome (0.54) | **Supported** tooling-grade for notebook/sim/sample-kit; chrome APIs `beta` (published release `v0.64.0` on PyPI) | Package doctor, sim subset/parity, notebook handles, AppShell chrome — [Authoring loop](../api/AUTHORING_LOOP.md) · [What’s new in 0.54](whats-new-0.54.md) |
| Progressive screens (`Hedron.screen`) | **Supported** on FastAPI (API `beta`; tip `v0.64.0`) | [Hedron API](../api/HEDRON.md) · [What’s new in 0.58](whats-new-0.58.md) |
...
!!! note "Package train vs capability"
    ...
    Flagship packages are **Beta** maturity — pin `hedron>=0.64.0,<0.65`. The table above is
...
Pin `hedron>=0.64.0,<0.65`. Public-index notes: [Installation](../getting-started/installation.md).

Extras: `"hedron[data]>=0.64.0,<0.65"`, `"hedron[extras]>=0.64.0,<0.65"`,
`"hedron[jinja]>=0.64.0,<0.65"`, `"hedron[dev]>=0.64.0,<0.65"`,
`"hedron[notebook]>=0.64.0,<0.65"` (tooling / localhost),
`"hedron[mcp]>=0.64.0,<0.62"` (Beta Supported inventory),
`"hedron[gradio]>=0.64.0,<0.65"` (Beta satellite; pin `hedron-gradio>=0.2.1,<0.3`),
`"hedron[charts]>=0.64.0,<0.65"`, `"hedron[maps]>=0.64.0,<0.65"`, `"hedron[workbench]>=0.64.0,<0.65"`,
