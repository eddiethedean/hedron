...
| Charts | **Supported** first-party and Matplotlib/static paths on Beta package | Install `hedron[charts]>=0.60.0,<0.61`; `ChartSpec` / `hedron-chart` and Matplotlib/static are Supported; Plotly / Altair remain **Experimental** ([Chart API](../api/CHART.md)) |
...
`"hedron[notebook]>=0.60.0,<0.61"` (tooling / localhost),
`"hedron[mcp]>=0.60.0,<0.61"` (Beta Supported inventory),
`"hedron[gradio]>=0.60.0,<0.61"` (Beta satellite; pin `hedron-gradio>=0.2.0,<0.3`),
`"hedron[charts]>=0.60.0,<0.61"`, `"hedron[maps]>=0.60.0,<0.61"`, `"hedron[workbench]>=0.60.0,<0.61"`,
`"hedron[posit]>=0.60.0,<0.61"`,
