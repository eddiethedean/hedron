...
|---|---|
| License | MIT |
| Current version | Published **v1.0.15**; recommended application range `hedron>=1.0.15,<1.1` |
| Publication / commercial SLA | Hedron `1.0.15` and Edron `1.0.15` are on PyPI; **no commercial SLA** |
...
- Coordinate on published trains. Use `hedron>=1.0.15,<1.1` for the current application
  range, then commit a lockfile; use `==1.0.15` for exact evidence reproduction.
...
```bash
python -m pip download --only-binary=:all: "hedron==1.0.15"
python -m pip hash hedron-1.0.15-*.whl
```
