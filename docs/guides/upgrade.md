...
1. Upgrade the application pin to `hedron>=0.58.0,<0.59`.
...
Pin `hedron>=0.58.0,<0.59` from PyPI (`v0.52.0` LANDED). See
...
Pin `hedron>=0.58.0,<0.59` from PyPI (or the in-tree 0.53 tip when developing Hedron
...
```bash
python -m pip install -U "hedron>=0.58.0,<0.59"
python -m pip install -U "hedron[extras]>=0.58.0,<0.59"
```

## Summary

Hedron **0.58.x** ships progressive feature and styling authoring on top of the 0.57
presentation train. Install `hedron>=0.58.0,<0.59` from PyPI.
...
conformance/Posit train. From PyPI, keep using `hedron>=0.58.0,<0.59` until the
0.53 wheel lands.
...
2. Confirm you are on a recent pin (`hedron>=0.29.0,<0.30` through `>=0.58.0,<0.59`,
...
```bash
python -m pip install -U "hedron>=0.58.0,<0.59"
python -m pip install -U "hedron[data]>=0.58.0,<0.59"
python -m pip install -U "hedron[charts]>=0.58.0,<0.59"
python -m pip install -U "hedron[maps]>=0.58.0,<0.59"
# independent charts satellite:
python -m pip install -U "hedron-charts>=0.2.0,<0.3"
# optional production-grade elements inventory:
python -m pip install -U "hedron[elements]>=0.58.0,<0.59"
```
