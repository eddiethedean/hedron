...
is `1.0.1` on the `v1.0` branch. Read [Current release and support](current-release.md) for the
...
Checkout tip `v1.0.1` uses `uv sync`. See [What's new in 0.62](whats-new-0.62.md),
[Hedron API](../api/HEDRON.md), and [Presentation](../api/PRESENTATION.md).

1. Upgrade the application pin to `hedron>=1.0.1,<1.1`.
...
Checkout tip `v1.0.1` uses `uv sync`. From the public index, install
`hedron>=1.0.1,<1.1`. See [What's new in 0.58](whats-new-0.58.md),
[Hedron API](../api/HEDRON.md), and [Presentation](../api/PRESENTATION.md).
...
Checkout tip `v1.0.1` uses `uv sync`. From PyPI, install
`hedron>=1.0.1,<1.1`. See
...
Pin `hedron>=1.0.1,<1.1` from PyPI (`v0.52.0` LANDED). See
...
Pin `hedron>=1.0.1,<1.1` from PyPI (or the in-tree 0.53 tip when developing Hedron
...
```bash
python -m pip install -U "hedron>=1.0.1,<1.1"
python -m pip install -U "hedron[extras]>=1.0.1,<1.1"
```

## Summary

Hedron **0.64.x** ships responsive navigation, bounded optimism, and localized failure isolation
on top of the progressive feature and styling authoring train. Install `hedron>=1.0.1,<1.1`
from PyPI.

Hedron **0.60.x** shipped progressive feature and styling authoring on top of the 0.57
presentation train. Install `hedron>=1.0.1,<1.1` from PyPI.
...
conformance/Posit train. From PyPI, keep using `hedron>=1.0.1,<1.1` until the
0.53 wheel lands.
...
2. Confirm you are on a recent pin (`hedron>=0.29.0,<0.30` through `>=1.0.1,<1.1`,
...
```bash
python -m pip install -U "hedron>=1.0.1,<1.1"
python -m pip install -U "hedron[data]>=1.0.1,<1.1"
python -m pip install -U "hedron[charts]>=1.0.1,<1.1"
python -m pip install -U "hedron[maps]>=1.0.1,<1.1"
# optional production-grade elements inventory:
python -m pip install -U "hedron[elements]>=1.0.1,<1.1"
```
...
7. **MCP authorize isolation.** Pin `hedron-mcp>=0.2.4,<0.3` with coordinated 1.0
   artifacts (or use the public `hedron[mcp]>=1.0.1,<1.1` pin) so a second
