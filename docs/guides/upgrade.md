...
(`v0.60.2` in-tree and on PyPI). Pin `hedron>=0.60.2,<0.61`.
...
Checkout tip `v0.60.2` uses `uv sync`. See [What's new in 0.60](whats-new-0.60.md),
[Hedron API](../api/HEDRON.md), and [Presentation](../api/PRESENTATION.md).

1. Upgrade the application pin to `hedron>=0.60.2,<0.61`.
...
Checkout tip `v0.60.2` uses `uv sync`. From the public index, install
`hedron>=0.60.2,<0.61`. See [What's new in 0.58](whats-new-0.58.md),
[Hedron API](../api/HEDRON.md), and [Presentation](../api/PRESENTATION.md).
...
Checkout tip `v0.60.2` uses `uv sync`. From PyPI, install
`hedron>=0.60.2,<0.61`. See
...
The published `v0.60.2` train supersedes the older 0.52/0.53 pins. Keep
`hedron>=0.60.2,<0.61`. See
...
Pin `hedron>=0.60.2,<0.61` from PyPI (`v0.52.0` LANDED). See
...
Pin `hedron>=0.60.2,<0.61` from PyPI (or the in-tree 0.53 tip when developing Hedron
...
```bash
python -m pip install -U "hedron>=0.60.2,<0.61"
python -m pip install -U "hedron[extras]>=0.60.2,<0.61"
```

## Summary

Hedron **0.60.x** ships progressive feature and styling authoring on top of the 0.57
presentation train. Install `hedron>=0.60.2,<0.61` from PyPI.
...
conformance/Posit train. From PyPI, keep using `hedron>=0.60.2,<0.61` until the
0.53 wheel lands.
...
2. Confirm you are on a recent pin (`hedron>=0.29.0,<0.30` through `>=0.60.2,<0.61`,
...
```bash
python -m pip install -U "hedron>=0.60.2,<0.61"
python -m pip install -U "hedron[data]>=0.60.2,<0.61"
python -m pip install -U "hedron[charts]>=0.60.2,<0.61"
python -m pip install -U "hedron[maps]>=0.60.2,<0.61"
# independent charts satellite:
python -m pip install -U "hedron-charts>=0.2.0,<0.3"
# optional production-grade elements inventory:
python -m pip install -U "hedron[elements]>=0.60.2,<0.61"
```
