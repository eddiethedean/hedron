...
Read [What’s new in 1.1](whats-new-1.1.md) and the [1.1.2 release notes](release-notes.md)
...
The `1.0.x` checkout tip uses `uv sync`. See [What's new in 0.62](whats-new-0.62.md),
...
`hedron>=1.0.0`. See [What's new in 0.58](whats-new-0.58.md),
[Hedron API](../api/HEDRON.md), and [Presentation](../api/PRESENTATION.md).
...
2. Confirm you are on a recent pin (`hedron>=0.29.0,<0.30` through `>=1.0.0`,
...
7. **MCP authorize isolation.** Pin `hedron-mcp>=0.2.4,<0.3` with coordinated 1.0
   artifacts (or use the public `hedron[mcp]>=1.0.0` pin) so a second
