...
This guide covers an application upgrade onto the published **0.62.x** train
(`v0.62.0` in-tree and on PyPI). Pin `hedron>=0.62.0,<0.63`.
...
Checkout tip `v0.62.0` uses `uv sync`. See [What's new in 0.62](whats-new-0.62.md),
[Hedron API](../api/HEDRON.md), and [Presentation](../api/PRESENTATION.md).

1. Upgrade the application pin to `hedron>=0.62.0,<0.63`.
...
`hedron>=0.62.0,<0.63`. See [What's new in 0.58](whats-new-0.58.md),
[Hedron API](../api/HEDRON.md), and [Presentation](../api/PRESENTATION.md).
...
The published `v0.62.0` train supersedes the older 0.52/0.53 pins. Keep
`hedron>=0.62.0,<0.63`. See
...
Pin `hedron>=0.62.0,<0.63` from PyPI (`v0.52.0` LANDED). See
...
Pin `hedron>=0.62.0,<0.63` from PyPI (or the in-tree 0.53 tip when developing Hedron
...
```bash
python -m pip install -U "hedron>=0.62.0,<0.63"
python -m pip install -U "hedron[extras]>=0.62.0,<0.63"
```

## Summary

Hedron **0.62.x** ships responsive navigation, bounded optimism, and localized failure isolation
on top of the progressive feature and styling authoring train. Install `hedron>=0.62.0,<0.63`
from PyPI.

Hedron **0.60.x** shipped progressive feature and styling authoring on top of the 0.57
presentation train. Install `hedron>=0.62.0,<0.63` from PyPI.
...
conformance/Posit train. From PyPI, keep using `hedron>=0.62.0,<0.63` until the
0.53 wheel lands.
...
2. Confirm you are on a recent pin (`hedron>=0.29.0,<0.30` through `>=0.62.0,<0.63`,
...
```bash
python -m pip install -U "hedron>=0.62.0,<0.63"
python -m pip install -U "hedron[data]>=0.62.0,<0.63"
python -m pip install -U "hedron[charts]>=0.62.0,<0.63"
python -m pip install -U "hedron[maps]>=0.62.0,<0.63"
# independent charts satellite:
python -m pip install -U "hedron-charts>=0.2.1,<0.3"
# optional production-grade elements inventory:
python -m pip install -U "hedron[elements]>=0.62.0,<0.63"
```
