# Upgrade to Hedron 0.62

This guide covers an application upgrade onto the published **0.63.x** train
(`v0.63.0` in-tree and on PyPI). Pin `hedron>=0.63.0,<0.64`.
...
Checkout tip `v0.63.0` uses `uv sync`. See [What's new in 0.62](whats-new-0.62.md),
[Hedron API](../api/HEDRON.md), and [Presentation](../api/PRESENTATION.md).

1. Upgrade the application pin to `hedron>=0.63.0,<0.64`.
...
Checkout tip `v0.63.0` uses `uv sync`. From the public index, install
`hedron>=0.63.0,<0.64`. See [What's new in 0.58](whats-new-0.58.md),
[Hedron API](../api/HEDRON.md), and [Presentation](../api/PRESENTATION.md).
...
Checkout tip `v0.63.0` uses `uv sync`. From PyPI, install
`hedron>=0.63.0,<0.64`. See
...
The published `v0.63.0` train supersedes the older 0.52/0.53 pins. Keep
`hedron>=0.63.0,<0.64`. See
...
Pin `hedron>=0.63.0,<0.64` from PyPI (`v0.52.0` LANDED). See
...
Pin `hedron>=0.63.0,<0.64` from PyPI (or the in-tree 0.53 tip when developing Hedron
...
```bash
python -m pip install -U "hedron>=0.63.0,<0.64"
python -m pip install -U "hedron[extras]>=0.63.0,<0.64"
```

## Summary

Hedron **0.63.x** ships responsive navigation, bounded optimism, and localized failure isolation
on top of the progressive feature and styling authoring train. Install `hedron>=0.63.0,<0.64`
from PyPI.

Hedron **0.60.x** shipped progressive feature and styling authoring on top of the 0.57
presentation train. Install `hedron>=0.63.0,<0.64` from PyPI.
...
conformance/Posit train. From PyPI, keep using `hedron>=0.63.0,<0.64` until the
0.53 wheel lands.
...
2. Confirm you are on a recent pin (`hedron>=0.29.0,<0.30` through `>=0.63.0,<0.64`,
...
```bash
python -m pip install -U "hedron>=0.63.0,<0.64"
python -m pip install -U "hedron[data]>=0.63.0,<0.64"
python -m pip install -U "hedron[charts]>=0.63.0,<0.64"
python -m pip install -U "hedron[maps]>=0.63.0,<0.64"
# independent charts satellite:
python -m pip install -U "hedron-charts>=0.2.1,<0.3"
# optional production-grade elements inventory:
python -m pip install -U "hedron[elements]>=0.63.0,<0.64"
```
