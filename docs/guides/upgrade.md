...
(`v0.61.0` in-tree and on PyPI). Pin `hedron>=0.61.0,<0.62`.
...
Pin `hedron>=0.61.0,<0.62` from PyPI (`v0.52.0` LANDED). See
...
Pin `hedron>=0.61.0,<0.62` from PyPI (or the in-tree 0.53 tip when developing Hedron
...
```bash
python -m pip install -U "hedron>=0.61.0,<0.62"
python -m pip install -U "hedron[extras]>=0.61.0,<0.62"
```

## Summary

Hedron **0.60.x** ships progressive feature and styling authoring on top of the 0.57
presentation train. Install `hedron>=0.61.0,<0.62` from PyPI.
...
conformance/Posit train. From PyPI, keep using `hedron>=0.61.0,<0.62` until the
0.53 wheel lands.
...
2. Confirm you are on a recent pin (`hedron>=0.29.0,<0.30` through `>=0.61.0,<0.62`,
...
```bash
python -m pip install -U "hedron>=0.61.0,<0.62"
python -m pip install -U "hedron[data]>=0.61.0,<0.62"
python -m pip install -U "hedron[charts]>=0.61.0,<0.62"
python -m pip install -U "hedron[maps]>=0.61.0,<0.62"
# independent charts satellite:
python -m pip install -U "hedron-charts>=0.2.1,<0.3"
# optional production-grade elements inventory:
python -m pip install -U "hedron[elements]>=0.61.0,<0.62"
```
