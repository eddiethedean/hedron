...
```bash
pip install "hedron[data]>=0.28.1,<0.29"
# optional backends
pip install "hedron-data[pandas]>=0.28.1,<0.29"
```
...
```bash
pip install "hedron-data[sqlalchemy]>=0.28.1,<0.29"
```
...
Install `hedron[charts]>=0.28.1,<0.29` for the compatible `0.1.8` chart satellite.
