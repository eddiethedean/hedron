...
```bash
pip install "hedron[data]>=0.31.0,<0.32"
# optional backends
pip install "hedron-data[pandas]>=0.31.0,<0.32"
```
...
```bash
pip install "hedron-data[sqlalchemy]>=0.31.0,<0.32"
```
...
Install `hedron[charts]>=0.31.0,<0.32` for the compatible `0.1.10` chart satellite.
