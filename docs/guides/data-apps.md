...
```bash
pip install "hedron[data]>=0.50.0,<0.51"
# optional backends
pip install "hedron-data[pandas]>=0.50.0,<0.51"
```
...
```bash
pip install "hedron-data[sqlalchemy]>=0.50.0,<0.51"
```
...
Install `hedron[charts]>=0.50.0,<0.51`; the flagship extra resolves the compatible
