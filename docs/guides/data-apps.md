...
```bash
pip install "hedron[data]>=0.49.1,<0.50"
# optional backends
pip install "hedron-data[pandas]>=0.49.1,<0.50"
```
...
```bash
pip install "hedron-data[sqlalchemy]>=0.49.1,<0.50"
```
...
Install `hedron[charts]>=0.49.1,<0.50`; the flagship extra resolves the compatible
