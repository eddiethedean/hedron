...
```bash
pip install "hedron[data]>=0.35.0,<0.36"
# optional backends
pip install "hedron-data[pandas]>=0.35.0,<0.36"
```
...
```bash
pip install "hedron-data[sqlalchemy]>=0.35.0,<0.36"
```
...
Install `hedron[charts]>=0.35.0,<0.36` for the compatible `0.1.10` chart satellite.
