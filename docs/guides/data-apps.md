...
```bash
pip install "hedron[data]>=0.30.0,<0.31"
# optional backends
pip install "hedron-data[pandas]>=0.30.0,<0.31"
```
...
```bash
pip install "hedron-data[sqlalchemy]>=0.30.0,<0.31"
```
...
Install `hedron[charts]>=0.30.0,<0.31` for the compatible `0.1.10` chart satellite.
