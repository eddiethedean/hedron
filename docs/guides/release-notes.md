...
- Coordinated release publication now builds and publishes `edron` and `edron-sim` alongside the
  rest of the workspace, while `hedron-native` remains owned by its native-wheel workflow.
- Refreshed the coordinated package metadata, changelogs, release evidence, and reproducible
  publication manifest for 1.0.1.

## 1.0.0 — 2026-08-27

First SemVer-stable Hedron train, verified by all 17 release gates and published to PyPI.
...
roles and use `hedron>=1.0.1,<1.1`.

## 0.64.0 — 2026-08-24 (in-tree)

In-tree bounded release slice for Phase 0.64: finite presentation tokens, responsive scoped
recipes, a parts/state manifest, lifecycle facts, and opt-in local HTMX asset injection. The
broader phase inventory remains explicitly dispositioned in the acceptance packet.

For maintainers, the release gate is `python scripts/check_release_gate.py 0.64.0` and the phase
packet check is `python scripts/check_064.py --verify`. The GitHub tag and PyPI upload remain
deferred until the authorized publication step.
...
- Canonical navigation generations with ordinary full-navigation fallback and localized failure
  boundaries.
- Stable identity/target diagnostics and revisioned optimism for approved reversible edits.
- A 17-gate evidence packet covering navigation, fallback, optimism, conflicts, failure isolation,
  security, accessibility, browser behavior, performance, docs, upgrades, and packaging.
- Explicit omission of dashboard fan-out from the Supported 0.62 claim.

For maintainers, the release gate is `python scripts/check_release_gate.py 0.62.0` and the
phase packet check is `python scripts/check_062.py --gate CONTRACT-062 --verify`.

## 0.61.0 — 2026-08-24

Verified and published release for unified action state and server-first async boundaries.
The coordinated packages are versioned `0.61.0` in the repository and available on PyPI.

### Added

- Unified action lifecycle and operation identity across forms, jobs, fragments, and supported elements.
...
The 0.60.2 maintenance release is published. The coordinated packages are
versioned `0.60.2` in the repository and available on PyPI.
...
`hedron>=1.0.1,<1.1`. [What’s new in 0.58](whats-new-0.58.md).
...
`hedron>=1.0.1,<1.1`. [What’s new in 0.57](whats-new-0.57.md).
[Installation](../getting-started/installation.md).
...
`hedron>=1.0.1,<1.1`. [What’s new in 0.56](whats-new-0.56.md).
[Installation](../getting-started/installation.md).
...
(RFC-0082 / D-095 / D-096). Pin `hedron>=1.0.1,<1.1` from PyPI until the 0.55
...
(RFC-0080 / D-091 / D-092). Pin `hedron>=1.0.1,<1.1` from PyPI until the 0.53
...
Pin `hedron>=1.0.1,<1.1` from the public index (0.53 remains deferred).
...
Quality and typing patch on the 0.51 train. Pin `hedron>=1.0.1,<1.1` from PyPI until the
...
Bugfix patch on the 0.51 train. Pin `hedron>=1.0.1,<1.1` from PyPI until a later
...
Published curated extras cut. Install from PyPI with `hedron>=1.0.1,<1.1`.
...
from PyPI `hedron>=1.0.1,<1.1` — [Installation](../getting-started/installation.md).
...
```bash
python -m pip install -U "hedron>=1.0.1,<1.1"
```
...
```bash
python -m pip install -U "hedron>=1.0.1,<1.1"
python -m pip install "hedron[maps]>=1.0.1,<1.1"
python -m pip install "hedron[data]>=1.0.1,<1.1"
python -m pip install "hedron[charts]>=1.0.1,<1.1"
```
...
python -m pip install "hedron[data]>=1.0.1,<1.1"
python -m pip install "hedron[charts]>=1.0.1,<1.1"
python -m pip install "hedron[elements]>=1.0.1,<1.1"
```
...
- Pin `hedron>=1.0.1,<1.1`. Charts remain on `hedron-charts>=0.2.3,<0.3`.
