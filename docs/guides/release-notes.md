...
- Pin `hedron>=0.38.0,<0.39` and charts satellite `hedron-charts>=0.2.0,<0.3`.

```bash
python -m pip install -U "hedron>=0.38.0,<0.39"
python -m pip install "hedron[charts]>=0.38.0,<0.39"
...
- Historical cut pin `hedron>=0.37.0,<0.38`. Install the current tip with the pin under
  **0.38.0** above.
...
`fastapi-workbench>=1.0.0,<2.0`. Prefer the current tip pin under **0.38.0** above.
...
Install the current tip with the pin under **0.38.0** above (historical cut pin was
`>=0.28.2,<0.29`).
