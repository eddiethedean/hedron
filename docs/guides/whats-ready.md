...
**Canonical maturity snapshot for Hedron 0.29.x** (last published `v0.29.0`). Other
...
For a pinned internal admin/CRUD app on 0.29.x (`hedron>=0.29.0,<0.30`), you can use:
...
`hedron[charts]>=0.29.0,<0.30` and `hedron-sample-kit>=0.1.10,<0.2`.
...
| Charts | **Supported** Matplotlib/static on Beta package | Install `hedron[charts]>=0.29.0,<0.30`; Matplotlib/static is Supported; Plotly / Altair remain **Experimental** ([compatibility](../COMPATIBILITY.md#charts-and-sample-kit-compatibility-floor)) |
...
!!! note "Package train vs capability"
    ...
    Flagship packages are **Beta** maturity — pin `hedron>=0.29.0,<0.30`. The table above is
...
=== "uv (recommended)"
    ...
    ```bash
    uvx --from "hedron>=0.29.0,<0.30" hedron new my-app
    cd my-app && uv sync
...
Pin `hedron>=0.29.0,<0.30` for the current published train.

Extras: `"hedron[data]>=0.29.0,<0.30"`, `"hedron[extras]>=0.29.0,<0.30"`,
`"hedron[jinja]>=0.29.0,<0.30"`, `"hedron[dev]>=0.29.0,<0.30"`,
`"hedron[notebook]>=0.29.0,<0.30"` (Alpha satellite),
`"hedron[mcp]>=0.29.0,<0.30"` (Alpha satellite),
`"hedron[gradio]>=0.29.0,<0.30"` (Alpha satellite),
`"hedron[charts]>=0.29.0,<0.30"`, `"hedron[workbench]>=0.29.0,<0.30"`,
