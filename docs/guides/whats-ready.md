...
**Canonical maturity snapshot for Hedron 0.40.x** (last published `v0.40.0`). Other
...
For a pinned internal admin/CRUD app on 0.40.x (`hedron>=0.40.0,<0.41`), you can use:
...
| Public element author kit | **Supported** contracts (`hedron new element`, plugin registration) | [Plugin authoring](plugin-authoring.md) · [What’s new in 0.40](whats-new-0.40.md) |
...
| Charts | **Supported** first-party and Matplotlib/static paths on Beta package | Install `hedron[charts]>=0.40.0,<0.41`; `ChartSpec` / `hedron-chart` and Matplotlib/static are Supported; Plotly / Altair remain **Experimental** ([Chart API](../api/CHART.md)) |
...
!!! note "Package train vs capability"
    ...
    Flagship packages are **Beta** maturity — pin `hedron>=0.40.0,<0.41`. The table above is
...
=== "uv (recommended)"
    ...
    ```bash
    uvx --from "hedron>=0.40.0,<0.41" hedron new my-app
    cd my-app && uv sync
...
=== "pip"
    ...
    ```bash
    pip install "hedron>=0.40.0,<0.41" "uvicorn[standard]"
    python -m hedron new my-app
    cd my-app && pip install -e .
    uvicorn app:app --reload
    ```

Pin `hedron>=0.40.0,<0.41` for the current published train.

Extras: `"hedron[data]>=0.40.0,<0.41"`, `"hedron[extras]>=0.40.0,<0.41"`,
`"hedron[jinja]>=0.40.0,<0.41"`, `"hedron[dev]>=0.40.0,<0.41"`,
`"hedron[notebook]>=0.40.0,<0.41"` (tooling / localhost),
`"hedron[mcp]>=0.40.0,<0.41"` (Beta Supported inventory),
`"hedron[gradio]>=0.40.0,<0.41"` (Beta satellite; pin `hedron-gradio>=0.2.0,<0.3`),
`"hedron[charts]>=0.40.0,<0.41"`, `"hedron[workbench]>=0.40.0,<0.41"`,
