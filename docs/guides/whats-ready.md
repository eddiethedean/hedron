...
**Canonical maturity snapshot for Hedron 0.39.x** (last published `v0.39.0`). Other
...
For a pinned internal admin/CRUD app on 0.39.x (`hedron>=0.39.0,<0.40`), you can use:
...
`hedron[charts]>=0.39.0,<0.40` and `hedron-sample-kit>=0.1.10,<0.2`.
...
| Charts | **Supported** first-party and Matplotlib/static paths on Beta package | Install `hedron[charts]>=0.39.0,<0.40`; `ChartSpec` / `hedron-chart` and Matplotlib/static are Supported; Plotly / Altair remain **Experimental** ([Chart API](../api/CHART.md)) |
...
!!! note "Package train vs capability"
    ...
    Flagship packages are **Beta** maturity — pin `hedron>=0.39.0,<0.40`. The table above is
...
=== "uv (recommended)"
    ...
    ```bash
    uvx --from "hedron>=0.39.0,<0.40" hedron new my-app
    cd my-app && uv sync
...
Pin `hedron>=0.39.0,<0.40` for the current published train.

Extras: `"hedron[data]>=0.39.0,<0.40"`, `"hedron[extras]>=0.39.0,<0.40"`,
`"hedron[jinja]>=0.39.0,<0.40"`, `"hedron[dev]>=0.39.0,<0.40"`,
`"hedron[notebook]>=0.39.0,<0.40"` (tooling / localhost),
`"hedron[mcp]>=0.39.0,<0.40"` (Beta Supported inventory),
`"hedron[gradio]>=0.39.0,<0.40"` (Beta satellite; pin `hedron-gradio>=0.2.0,<0.3`),
`"hedron[charts]>=0.39.0,<0.40"`, `"hedron[workbench]>=0.39.0,<0.40"`,
`"hedron[posit]>=0.39.0,<0.40"`,
