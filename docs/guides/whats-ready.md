...
| Charts | **Supported** first-party and Matplotlib/static paths on Beta package | Install `hedron[charts]>=0.51.0,<0.52`; `ChartSpec` / `hedron-chart` and Matplotlib/static are Supported; Plotly / Altair remain **Experimental** ([Chart API](../api/CHART.md)) |
...
!!! note "Package train vs capability"
    ...
    Flagship packages are **Beta** maturity — pin `hedron>=0.51.0,<0.52`. The table above is
...
=== "uv (recommended)"
    ...
    ```bash
    uvx --from "hedron>=0.51.0,<0.52" hedron new my-app
    cd my-app && uv sync
...
=== "pip"
    ...
    ```bash
    pip install "hedron>=0.51.0,<0.52" "uvicorn[standard]"
    python -m hedron new my-app
    cd my-app && pip install -e .
    uvicorn app:app --reload
    ```

Pin `hedron>=0.51.0,<0.52`. Public-index notes: [Installation](../getting-started/installation.md).

Extras: `"hedron[data]>=0.51.0,<0.52"`, `"hedron[extras]>=0.51.0,<0.52"`,
`"hedron[jinja]>=0.51.0,<0.52"`, `"hedron[dev]>=0.51.0,<0.52"`,
`"hedron[notebook]>=0.51.0,<0.52"` (tooling / localhost),
`"hedron[mcp]>=0.51.0,<0.52"` (Beta Supported inventory),
`"hedron[gradio]>=0.51.0,<0.52"` (Beta satellite; pin `hedron-gradio>=0.2.0,<0.3`),
`"hedron[charts]>=0.51.0,<0.52"`, `"hedron[maps]>=0.51.0,<0.52"`, `"hedron[workbench]>=0.51.0,<0.52"`,
`"hedron[posit]>=0.51.0,<0.52"`,
