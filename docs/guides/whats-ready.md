...
**Canonical maturity snapshot for Hedron 0.38.x** (last published `v0.38.0`). Other
...
For a pinned internal admin/CRUD app on 0.38.x (`hedron>=0.38.0,<0.39`), you can use:
...
!!! note "Package train vs capability"
    ...
    Flagship packages are **Beta** maturity — pin `hedron>=0.38.0,<0.39`. The table above is
...
=== "uv (recommended)"
    ...
    ```bash
    uvx --from "hedron>=0.38.0,<0.39" hedron new my-app
    cd my-app && uv sync
...
=== "pip"
    ...
    ```bash
    pip install "hedron>=0.38.0,<0.39" "uvicorn[standard]"
    python -m hedron new my-app
    cd my-app && pip install -e .
    uvicorn app:app --reload
    ```

Pin `hedron>=0.38.0,<0.39` for the current published train.

Extras: `"hedron[data]>=0.38.0,<0.39"`, `"hedron[extras]>=0.38.0,<0.39"`,
`"hedron[jinja]>=0.38.0,<0.39"`, `"hedron[dev]>=0.38.0,<0.39"`,
`"hedron[notebook]>=0.38.0,<0.39"` (tooling / localhost),
`"hedron[mcp]>=0.38.0,<0.39"` (Beta Supported inventory),
`"hedron[gradio]>=0.38.0,<0.39"` (Beta satellite; pin `hedron-gradio>=0.2.0,<0.3`),
`"hedron[charts]>=0.38.0,<0.39"`, `"hedron[workbench]>=0.38.0,<0.39"`,
`"hedron[posit]>=0.38.0,<0.39"`,
