...
`hedron[charts]>=0.37.0,<0.38` and `hedron-sample-kit>=0.1.10,<0.2`.
...
| Charts | **Supported** Matplotlib/static on Beta package | Install `hedron[charts]>=0.37.0,<0.38`; Matplotlib/static is Supported; Plotly / Altair remain **Experimental** ([compatibility](../COMPATIBILITY.md#charts-and-sample-kit-compatibility-floor)) |
...
!!! note "Package train vs capability"
    ...
    Flagship packages are **Beta** maturity — pin `hedron>=0.37.0,<0.38`. The table above is
...
=== "uv (recommended)"
    ...
    ```bash
    uvx --from "hedron>=0.37.0,<0.38" hedron new my-app
    cd my-app && uv sync
...
=== "pip"
    ...
    ```bash
    pip install "hedron>=0.37.0,<0.38" "uvicorn[standard]"
    python -m hedron new my-app
    cd my-app && pip install -e .
    uvicorn app:app --reload
    ```

Pin `hedron>=0.37.0,<0.38` for the current published train.

Extras: `"hedron[data]>=0.37.0,<0.38"`, `"hedron[extras]>=0.37.0,<0.38"`,
`"hedron[jinja]>=0.37.0,<0.38"`, `"hedron[dev]>=0.37.0,<0.38"`,
`"hedron[notebook]>=0.37.0,<0.38"` (tooling / localhost),
`"hedron[mcp]>=0.37.0,<0.38"` (Beta Supported inventory),
`"hedron[gradio]>=0.37.0,<0.38"` (Beta satellite; pin `hedron-gradio>=0.2.0,<0.3`),
`"hedron[charts]>=0.37.0,<0.38"`, `"hedron[workbench]>=0.37.0,<0.38"`,
`"hedron[posit]>=0.37.0,<0.38"`,
