...
**Canonical maturity snapshot for Hedron 0.45.x** (last published `v0.45.0`). Other
...
For a pinned internal admin/CRUD app on 0.45.x (`hedron>=0.45.0,<0.46`), you can use:
...
CSRF (`standard`/`strict`), Flask/Django adapters, and polling for
job status.
...
`hedron[charts]>=0.45.0,<0.46` and `hedron-sample-kit>=0.1.10,<0.2`.
...
| Charts | **Supported** first-party and Matplotlib/static paths on Beta package | Install `hedron[charts]>=0.45.0,<0.46`; `ChartSpec` / `hedron-chart` and Matplotlib/static are Supported; Plotly / Altair remain **Experimental** ([Chart API](../api/CHART.md)) |
...
!!! note "Package train vs capability"
    ...
    Flagship packages are **Beta** maturity — pin `hedron>=0.45.0,<0.46`. The table above is
...
=== "uv (recommended)"
    ...
    ```bash
    uvx --from "hedron>=0.45.0,<0.46" hedron new my-app
    cd my-app && uv sync
...
Pin `hedron>=0.45.0,<0.46` for the current published train.

Extras: `"hedron[data]>=0.45.0,<0.46"`, `"hedron[extras]>=0.45.0,<0.46"`,
`"hedron[jinja]>=0.45.0,<0.46"`, `"hedron[dev]>=0.45.0,<0.46"`,
`"hedron[notebook]>=0.45.0,<0.46"` (tooling / localhost),
`"hedron[mcp]>=0.45.0,<0.46"` (Beta Supported inventory),
`"hedron[gradio]>=0.45.0,<0.46"` (Beta satellite; pin `hedron-gradio>=0.2.0,<0.3`),
`"hedron[charts]>=0.45.0,<0.46"`, `"hedron[workbench]>=0.45.0,<0.46"`,
`"hedron[posit]>=0.45.0,<0.46"`,
