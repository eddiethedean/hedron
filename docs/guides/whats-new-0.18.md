...
!!! note "Current train is 0.54"

    Pin `hedron>=0.53.0,<0.54` for new apps (checkout tip; PyPI still `>=0.54.0,<0.55` while deferred). The pin below is historical for this train only.
    See [What’s new in 0.51](whats-new-0.51.md).

!!! note "Historical phase"

    This page describes **0.18**. The current published train is **0.54.x** (last `v0.54.0`). Pin `hedron>=0.54.0,<0.55` from checkout (PyPI still `>=0.54.0,<0.55` while deferred).
...
Prefer the current 0.54 train for new apps; stay on a historical upper-bound pin
