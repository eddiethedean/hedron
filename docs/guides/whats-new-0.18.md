...
!!! note "Current train is 0.60"
    ...
    Pin `hedron>=0.53.0,<0.54` for new apps (checkout tip; current PyPI pin `>=0.60.2,<0.61`). The pin below is historical for this train only.
    See [What’s new in 0.51](whats-new-0.51.md).

!!! note "Historical phase"

    This page describes **0.18**. The current repository train is **0.60.x** (`v0.60.0` in-tree and on PyPI). Pin `hedron>=0.60.2,<0.61` from PyPI.
...
Prefer the current 0.60 train for new apps; stay on a historical upper-bound pin
