...
Product principles for evaluators and contributors. The full numbered list also lives in
the repository foundations corpus (maintainer-facing).
...
5. **Explicit boundaries.** Props, inputs, actions, data changes, events, and integration results have explicit contracts.
...
9. **Secure by default.** Dangerous behavior requires a visible, explicit opt-in.
...
18. **Escape hatches earn stability.** Native HTML, attributes, CSS, Web Components, and explicit responses remain available.
...
[Architecture](../ARCHITECTURE.md) · [Why Hedron](why-hedron.md) · [What’s ready](whats-ready.md) ·
