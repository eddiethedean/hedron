...
This workshop targets the Hedron **0.67.x** train (`hedron>=1.0.1,<1.1`) and Python
...
- Confirm that the environment can install `hedron>=1.0.1,<1.1` and optional packages
...
python3.11 -m pip install "hedron>=1.0.1,<1.1"
hedron --version
```
...
python3.11 -m venv .venv
source .venv/bin/activate
python3.11 -m pip install -e . "hedron-posit>=1.0.1,<1.1"
```

Add `"hedron-posit>=1.0.1,<1.1",` to the `dependencies` list in `pyproject.toml` so a fresh
...
Stop the server. Add `"hedron-posit>=1.0.1,<1.1",` to the `dependencies` list in
...
| `ModuleNotFoundError: hedron_posit` | Posit adapter was not installed | Run `python3.11 -m pip install "hedron-posit>=1.0.1,<1.1"` and add it to `pyproject.toml` |
