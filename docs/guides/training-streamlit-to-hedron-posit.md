...
This workshop targets Hedron **0.48.x** (`hedron>=0.48.0,<0.49`) and Python **3.11–3.14**. Before scheduling it,
...
- Confirm that the environment can install `hedron>=0.48.0,<0.49` and optional packages
...
python3 --version
uv --version
uvx --from "hedron>=0.48.0,<0.49" hedron --version
```
...
```bash
uvx --from "hedron>=0.48.0,<0.49" hedron new hello-hedron
cd hello-hedron
uv sync
uv add "hedron[posit]>=0.48.0,<0.49"
```
...
```bash
uvx --from "hedron>=0.48.0,<0.49" hedron migrate streamlit \
...
| `ModuleNotFoundError: hedron_posit` | Posit extra was not added | Run `uv add "hedron[posit]>=0.48.0,<0.49"` |
