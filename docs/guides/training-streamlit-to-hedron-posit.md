...
This workshop targets Hedron **0.47.x** (`hedron>=0.47.0,<0.48`) and Python **3.11–3.14**. Before scheduling it,
...
- Confirm that the environment can install `hedron>=0.47.0,<0.48` and optional packages
...
python3 --version
uv --version
uvx --from "hedron>=0.47.0,<0.48" hedron --version
```
...
```bash
uvx --from "hedron>=0.47.0,<0.48" hedron new hello-hedron
cd hello-hedron
uv sync
uv add "hedron[posit]>=0.47.0,<0.48"
```
...
```bash
uvx --from "hedron>=0.47.0,<0.48" hedron migrate streamlit \
...
| `ModuleNotFoundError: hedron_posit` | Posit extra was not added | Run `uv add "hedron[posit]>=0.47.0,<0.48"` |
