...
- Confirm that the environment can install `hedron>=0.45.0,<0.46` and optional packages
...
python3 --version
uv --version
uvx --from "hedron>=0.45.0,<0.46" hedron --version
```
...
```bash
uvx --from "hedron>=0.45.0,<0.46" hedron new hello-hedron
cd hello-hedron
uv sync
uv add "hedron[posit]>=0.45.0,<0.46"
```
...
```bash
uvx --from "hedron>=0.45.0,<0.46" hedron migrate streamlit \
...
| `ModuleNotFoundError: hedron_posit` | Posit extra was not added | Run `uv add "hedron[posit]>=0.45.0,<0.46"` |
