...
- Confirm that the environment can install `hedron>=0.43.0,<0.44` and optional packages
...
python3 --version
uv --version
uvx --from "hedron>=0.43.0,<0.44" hedron --version
```
...
```bash
uvx --from "hedron>=0.43.0,<0.44" hedron new hello-hedron
cd hello-hedron
uv sync
...
```bash
uvx --from "hedron>=0.43.0,<0.44" hedron migrate streamlit \
...
| `ModuleNotFoundError: hedron_posit` | Posit extra was not added | Run `uv add "hedron[posit]>=0.43.0,<0.44"` |
