...
This workshop targets the Hedron **1.0.x** train (`hedron>=1.0.0`) and Python
...
- Confirm that the environment can install `hedron>=1.0.0` and optional packages
...
python3.11 -m pip install "hedron>=1.0.0"
hedron --version
```
...
| 1 | Preserves the `hedron>=1.0.0` requirement and deployment dependency list |
