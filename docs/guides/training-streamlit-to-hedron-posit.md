...
- Confirm that the environment can install `hedron>=0.42.0,<0.43` and optional packages
...
python3 --version
uv --version
uvx --from "hedron>=0.42.0,<0.43" hedron --version
```
...
```bash
uvx --from "hedron>=0.42.0,<0.43" hedron new hello-hedron
cd hello-hedron
uv sync
...
```bash
uvx --from "hedron>=0.42.0,<0.43" hedron migrate streamlit \
...
| `ModuleNotFoundError: hedron_posit` | Posit extra was not added | Run `uv add "hedron[posit]>=0.42.0,<0.43"` |
