...
- Confirm that the environment can install `hedron>=0.40.0,<0.41` and optional packages
...
python3 --version
uv --version
uvx --from "hedron>=0.40.0,<0.41" hedron --version
```
...
```bash
uvx --from "hedron>=0.40.0,<0.41" hedron new hello-hedron
cd hello-hedron
uv sync
...
```bash
uvx --from "hedron>=0.40.0,<0.41" hedron migrate streamlit \
...
| `ModuleNotFoundError: hedron_posit` | Posit extra was not added | Run `uv add "hedron[posit]>=0.40.0,<0.41"` |
