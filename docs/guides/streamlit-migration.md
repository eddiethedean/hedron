...
**0.25.1+**, install `hedron[charts]>=0.27.0,<0.28` for Matplotlib charts; or keep
...
```bash
uv add "hedron[data]>=0.27.0,<0.28" "uvicorn[standard]"
```
...
This enforces `hedron-charts>=0.1.7,<0.2`; see
...
| `st.line_chart` | `LineChart` (`hedron[charts]>=0.27.0,<0.28`) | Provide a title and accessible description; a table fallback remains useful. |
...
| Chart installation resolves an older core | The lower bound allowed a satellite before `0.1.6` | Install `hedron[charts]>=0.27.0,<0.28` in a clean environment |
