...
**0.38.x**, install `hedron[charts]>=0.48.0,<0.49` for first-party or Matplotlib charts; or keep
...
```bash
uv add "hedron[data]>=0.48.0,<0.49" "uvicorn[standard]"
```
...
| `st.line_chart` | `LineChart` (`hedron[charts]>=0.48.0,<0.49`) | Provide a title and accessible description; a table fallback remains useful. |
...
| Chart installation resolves an older core | The lower bound allowed a satellite before `0.1.6` | Install `hedron[charts]>=0.48.0,<0.49` in a clean environment |
