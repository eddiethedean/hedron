...
**0.25.1+**, install `hedron[charts]>=0.28.1,<0.29` for Matplotlib charts; or keep
...
```bash
uv add "hedron[data]>=0.28.1,<0.29" "uvicorn[standard]"
```
...
This enforces `hedron-charts>=0.1.8,<0.2`; see
...
| `st.line_chart` | `LineChart` (`hedron[charts]>=0.28.1,<0.29`) | Provide a title and accessible description; a table fallback remains useful. |
...
| Chart installation resolves an older core | The lower bound allowed a satellite before `0.1.6` | Install `hedron[charts]>=0.28.1,<0.29` in a clean environment |
