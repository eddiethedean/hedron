...
```bash
uvx --from "hedron>=1.0.1,<1.1" hedron migrate streamlit \
  streamlit_app.py \
  --analyze-only \
  --format text
```
...
| `st.line_chart` | `LineChart` (`hedron[charts]>=1.0.1,<1.1`) | Provide a title and accessible description; a table fallback remains useful. |
...
| `DataTable` cannot be imported | The data extra is not installed | Install `hedron[data]>=1.0.1,<1.1` in the same environment as the app |
...
| Chart installation resolves an older core | The lower bound allowed a satellite before `0.1.6` | Install `hedron[charts]>=1.0.1,<1.1` in a clean environment |
