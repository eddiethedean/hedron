...
**0.25.1+**, install `hedron[charts]>=0.37.0,<0.38` for Matplotlib charts; or keep
...
```bash
uv add "hedron[charts]>=0.37.0,<0.38"
```
...
| `st.line_chart` | `LineChart` (`hedron[charts]>=0.37.0,<0.38`) | Provide a title and accessible description; a table fallback remains useful. |
...
| Chart installation resolves an older core | The lower bound allowed a satellite before `0.1.6` | Install `hedron[charts]>=0.37.0,<0.38` in a clean environment |
