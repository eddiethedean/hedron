...
**0.38.x**, install `hedron[charts]>=0.39.0,<0.40` for first-party or Matplotlib charts; or keep
...
```bash
uv add "hedron[data]>=0.39.0,<0.40" "uvicorn[standard]"
```
...
| `st.line_chart` | `LineChart` (`hedron[charts]>=0.39.0,<0.40`) | Provide a title and accessible description; a table fallback remains useful. |
...
| Chart installation resolves an older core | The lower bound allowed a satellite before `0.1.6` | Install `hedron[charts]>=0.39.0,<0.40` in a clean environment |
