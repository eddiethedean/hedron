...
**0.38.x**, install `hedron[charts]>=0.44.0,<0.45` for first-party or Matplotlib charts; or keep
...
```bash
uv add "hedron[charts]>=0.44.0,<0.45"
```
...
| `st.line_chart` | `LineChart` (`hedron[charts]>=0.44.0,<0.45`) | Provide a title and accessible description; a table fallback remains useful. |
...
| Chart installation resolves an older core | The lower bound allowed a satellite before `0.1.6` | Install `hedron[charts]>=0.44.0,<0.45` in a clean environment |
