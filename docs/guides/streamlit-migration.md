...
**0.25.1+**, install `hedron[charts]>=0.26.0,<0.27` for Matplotlib charts; or keep
...
```bash
uv add "hedron[charts]>=0.26.0,<0.27"
```
...
| `st.line_chart` | `LineChart` (`hedron[charts]>=0.26.0,<0.27`) | Provide a title and accessible description; a table fallback remains useful. |
...
| Chart installation resolves an older core | The lower bound allowed a satellite before `0.1.6` | Install `hedron[charts]>=0.26.0,<0.27` in a clean environment |
