...
**0.38.x**, install `hedron[charts]>=0.43.0,<0.44` for first-party or Matplotlib charts; or keep
...
```bash
uv add "hedron[charts]>=0.43.0,<0.44"
```
...
| `st.line_chart` | `LineChart` (`hedron[charts]>=0.43.0,<0.44`) | Provide a title and accessible description; a table fallback remains useful. |
...
| Chart installation resolves an older core | The lower bound allowed a satellite before `0.1.6` | Install `hedron[charts]>=0.43.0,<0.44` in a clean environment |
