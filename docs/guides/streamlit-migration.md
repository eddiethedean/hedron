...
```bash
uvx --from "hedron>=0.63.0,<0.64" hedron migrate streamlit \
  streamlit_app.py \
  --analyze-only \
  --format text
```
...
```bash
uv add "hedron[data]>=0.63.0,<0.64" "uvicorn[standard]"
```
...
| `st.line_chart` | `LineChart` (`hedron[charts]>=0.63.0,<0.64`) | Provide a title and accessible description; a table fallback remains useful. |
...
| `DataTable` cannot be imported | The data extra is not installed | Install `hedron[data]>=0.63.0,<0.64` in the same environment as the app |
...
| Chart installation resolves an older core | The lower bound allowed a satellite before `0.1.6` | Install `hedron[charts]>=0.63.0,<0.64` in a clean environment |
