...
**0.38.x**, install `hedron[charts]>=0.40.0,<0.41` for first-party or Matplotlib charts; or keep
...
```bash
uv add "hedron[data]>=0.40.0,<0.41" "uvicorn[standard]"
```

To include charts, install the compatible satellite through the flagship extra:

```bash
uv add "hedron[charts]>=0.40.0,<0.41"
```
...
| `st.line_chart` | `LineChart` (`hedron[charts]>=0.40.0,<0.41`) | Provide a title and accessible description; a table fallback remains useful. |
...
| Chart installation resolves an older core | The lower bound allowed a satellite before `0.1.6` | Install `hedron[charts]>=0.40.0,<0.41` in a clean environment |
