...
```bash
uv add "hedron[data]>=0.59.0,<0.60" "uvicorn[standard]"
```
...
| `st.line_chart` | `LineChart` (`hedron[charts]>=0.59.0,<0.60`) | Provide a title and accessible description; a table fallback remains useful. |
...
| `DataTable` cannot be imported | The data extra is not installed | Install `hedron[data]>=0.59.0,<0.60` in the same environment as the app |
...
| Chart installation resolves an older core | The lower bound allowed a satellite before `0.1.6` | Install `hedron[charts]>=0.59.0,<0.60` in a clean environment |
