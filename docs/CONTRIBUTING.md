...
| `evidence` | `evidence` — Evidence bundle, dep audit, release-gate check for current train, `verify_pkg_26.py` | Yes, unless **docs-only**; also on release |
| `release` (commit CI) | `packaging` — Packaging rehearsal (`verify_pkg_26`) | After `evidence` succeeds (skipped when docs-only) |
