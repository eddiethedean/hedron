...
**Package maturity:** Beta · **Train:** `0.50.x` (published `v0.50.0`) · pin `>=0.50.0,<0.51`<br>
...
```bash
pip install "hedron[extras]>=0.50.0,<0.51"
# or
pip install "hedron-extras>=0.50.0,<0.51"
# feature-scoped:
pip install "hedron-extras[data_explorer]>=0.50.0,<0.51"
# experimental UI (requires an explicit opt-in):
pip install "hedron[experimental-ui]>=0.50.0,<0.51"
