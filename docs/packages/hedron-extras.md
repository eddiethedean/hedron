...
**Package maturity:** Beta · **Train:** `0.58.x` (in-tree tip `v0.58.0`) · pin `>=0.58.0,<0.59` in-tree (PyPI `>=0.56.0,<0.59` ) (PyPI still `>=0.56.0,<0.59` )<br>
...
```bash
pip install "hedron[extras]>=0.56.0,<0.59"
# or
pip install "hedron-extras>=0.56.0,<0.59"
# feature-scoped:
pip install "hedron-extras[data_explorer]>=0.56.0,<0.59"
# experimental UI (requires an explicit opt-in):
pip install "hedron[experimental-ui]>=0.56.0,<0.59"
