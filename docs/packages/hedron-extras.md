...
**Package maturity:** Beta · **Train:** `0.49.x` (published `v0.49.1`) · pin `>=0.49.1,<0.50`<br>
...
```bash
pip install "hedron[extras]>=0.49.1,<0.50"
# or
pip install "hedron-extras>=0.49.1,<0.50"
# feature-scoped:
pip install "hedron-extras[data_explorer]>=0.49.1,<0.50"
# experimental UI (requires an explicit opt-in):
pip install "hedron[experimental-ui]>=0.49.1,<0.50"
