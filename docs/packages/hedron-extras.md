...
**Package maturity:** Beta · **Train:** `0.60.x` (`v0.60.0` in-tree; PyPI `v0.59.0` until upload) · pin `>=0.59.0,<0.60`<br>
...
```bash
pip install "hedron[extras]>=0.59.0,<0.60"
# or
pip install "hedron-extras>=0.59.0,<0.60"
# feature-scoped:
pip install "hedron-extras[data_explorer]>=0.59.0,<0.60"
# experimental UI (requires an explicit opt-in):
pip install "hedron[experimental-ui]>=0.59.0,<0.60"
