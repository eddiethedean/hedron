...
**Package maturity:** Beta · **Train:** `0.38.x` (published `v0.38.0`) · pin `>=0.38.0,<0.39`  
...
```bash
pip install "hedron[extras]>=0.38.0,<0.39"
# or
pip install "hedron-extras>=0.38.0,<0.39"
# feature-scoped:
pip install "hedron-extras[data_explorer]>=0.38.0,<0.39"
# experimental UI (requires an explicit opt-in):
pip install "hedron[experimental-ui]>=0.38.0,<0.39"
