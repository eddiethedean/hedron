...
**Package maturity:** Beta · **Train:** `0.45.x` (published `v0.45.0`) · pin `>=0.46.0,<0.47`  
...
```bash
pip install "hedron[extras]>=0.46.0,<0.47"
# or
pip install "hedron-extras>=0.46.0,<0.47"
# feature-scoped:
pip install "hedron-extras[data_explorer]>=0.46.0,<0.47"
# experimental UI (requires an explicit opt-in):
pip install "hedron[experimental-ui]>=0.46.0,<0.47"
