...
| [hedron-charts](hedron-charts.md) | `hedron[charts]` | First-party charts and visualization adapters; `>=0.2.0,<0.3` on the 0.57 train |
...
```bash
pip install "hedron[data,dev,posit]>=0.56.0,<0.58"
```

Checkout tip is `v0.57.0` (`>=0.57.0,<0.58` in-tree); PyPI first-run stays `>=0.56.0,<0.58` until the 0.57.0 upload lands.
