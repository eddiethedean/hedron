...
**Package maturity:** Beta · **Train:** `0.61.x` (`v0.61.0` in-tree and on PyPI) · pin `>=0.61.0,<0.62`
...
```bash
pip install "hedron[conformance]>=0.61.0,<0.62"
# or
pip install "hedron-conformance>=0.61.0,<0.62"
```

Checkout tip `v0.61.0` uses `uv sync`; applications install `>=0.61.0,<0.62` from PyPI.
