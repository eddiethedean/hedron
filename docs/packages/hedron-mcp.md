...
Living Hedron train `0.52.x` (checkout tip `v0.53.0`; flagship extra install from PyPI
stays `>=0.52.0,<0.53` while deferred). Disabled and empty until resources and tools are
...
```bash
pip install "hedron[mcp]>=0.52.0,<0.53"
# or
