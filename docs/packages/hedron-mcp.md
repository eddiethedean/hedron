...
Living Hedron train `0.56.x` (checkout tip `v0.56.0`; flagship extra install from PyPI
stays `>=0.54.0,<0.55` while deferred). Disabled and empty until resources and tools are
...
```bash
pip install "hedron[mcp]>=0.54.0,<0.55"
# or
