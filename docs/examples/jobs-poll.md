...
Enqueue durable work and **poll** status with `TaskFlow` (Supported path). SSE/WebSocket
job helpers are Experimental — prefer this recipe.
...
pip install "hedron>=0.60.2,<0.61" "uvicorn[standard]"
...
- `TaskFlow` submit form + scoped status polling that stops on terminal states
- Explicit `JobScope` + `authorize_submit` (no inferred identity)
- In-memory backend for local demos only

## Advanced — explicit enqueue / Poll

Lower to `enqueue_durable`, `job_status_response`, and `Poll` when you need custom status
URLs. See [JOBS.md](../api/JOBS.md) and [Celery / RQ](../guides/jobs-celery-rq.md).

Source: [`examples/jobs-poll`](https://github.com/eddiethedean/hedron/tree/main/examples/jobs-poll)
(may still show the Advanced spelling until the example tree is regenerated).
