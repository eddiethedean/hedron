...
**0.44** train. Prefer [session auth](session-auth.md) and
...
compose. Outside the workspace, install `hedron[charts]>=0.44.0,<0.45` for the compatible
chart satellite; see
...
| DataEditor / Auto / charts | dashboard + `/charts/*` (`hedron[charts]>=0.44.0,<0.45` or the monorepo source) | Chart panel refresh (sim) |
