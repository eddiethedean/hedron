...
**0.56** train. Prefer [session auth](session-auth.md) and
...
compose. Outside the workspace, install `hedron[charts]>=0.56.0,<0.57` from PyPI while
registry upload is deferred (checkout tip is `0.56.0`; see
...
| DataEditor / Auto / charts | dashboard + `/charts/*` (`hedron[charts]>=0.56.0,<0.57` or the monorepo source) | Chart panel refresh (sim) |
