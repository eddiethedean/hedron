...
**0.41** train. Prefer [session auth](session-auth.md) and
...
compose. Outside the workspace, install `hedron[charts]>=0.41.0,<0.42` for the compatible
chart satellite; see
...
| DataEditor / Auto / charts | dashboard + `/charts/*` (`hedron[charts]>=0.41.0,<0.42` or the monorepo source) | Chart panel refresh (sim) |
