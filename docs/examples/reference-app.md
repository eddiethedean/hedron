...
**0.59** train. Prefer [session auth](session-auth.md) and
...
compose. Outside the workspace, install `hedron[charts]>=0.59.0,<0.60` from PyPI
(published tip / living tip is `0.59.0`; see
...
| DataEditor / Auto / charts | dashboard + `/charts/*` (`hedron[charts]>=0.59.0,<0.60` or the monorepo source) | Chart panel refresh (sim) |
