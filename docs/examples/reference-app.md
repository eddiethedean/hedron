...
**0.62** train. Prefer [session auth](session-auth.md) and
...
compose. Outside the workspace, install `hedron[charts]>=0.62.0,<0.63` from PyPI
(the verified repository tip is `0.62.0`, and the public PyPI release is also `0.62.0`; see
...
| DataEditor / Auto / charts | dashboard + `/charts/*` (`hedron[charts]>=0.62.0,<0.63` or the monorepo source) | Chart panel refresh (sim) |
