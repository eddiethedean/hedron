...
**0.49** train. Prefer [session auth](session-auth.md) and
...
compose. Outside the workspace, install `hedron[charts]>=0.49.0,<0.50` for the compatible
chart satellite; see
...
| DataEditor / Auto / charts | dashboard + `/charts/*` (`hedron[charts]>=0.49.0,<0.50` or the monorepo source) | Chart panel refresh (sim) |
