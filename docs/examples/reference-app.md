...
**0.57** train. Prefer [session auth](session-auth.md) and
...
compose. Outside the workspace, install `hedron[charts]>=0.56.0,<0.58` from PyPI while
...
| DataEditor / Auto / charts | dashboard + `/charts/*` (`hedron[charts]>=0.56.0,<0.58` or the monorepo source) | Chart panel refresh (sim) |
