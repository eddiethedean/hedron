...
**1.1.x** train in this repository. Prefer [session auth](session-auth.md) and
...
compose. Outside the workspace, install `hedron[charts]>=1.0.0` from PyPI (see
...
| DataEditor / Auto / charts | dashboard + `/charts/*` (`hedron[charts]>=1.0.0` or the monorepo source) | Chart panel refresh (sim) |
