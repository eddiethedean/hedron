...
**0.52** train. Prefer [session auth](session-auth.md) and
...
compose. Outside the workspace, install `hedron[charts]>=0.51.0,<0.52` from PyPI while
registry upload is deferred (checkout tip is `0.52.0`).
chart satellite; see
...
| DataEditor / Auto / charts | dashboard + `/charts/*` (`hedron[charts]>=0.51.0,<0.52` or the monorepo source) | Chart panel refresh (sim) |
