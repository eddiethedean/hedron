...
**0.58** train. Prefer [session auth](session-auth.md) and
...
compose. Outside the workspace, install `hedron[charts]>=0.58.0,<0.59` from PyPI
(published tip / living tip is `0.58.0`; see
...
| DataEditor / Auto / charts | dashboard + `/charts/*` (`hedron[charts]>=0.58.0,<0.59` or the monorepo source) | Chart panel refresh (sim) |
