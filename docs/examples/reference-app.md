...
compose. Outside the workspace, install `hedron[charts]>=0.52.0,<0.53` from PyPI while
registry upload is deferred (checkout tip is `0.53.0`; see
...
| DataEditor / Auto / charts | dashboard + `/charts/*` (`hedron[charts]>=0.52.0,<0.53` or the monorepo source) | Chart panel refresh (sim) |
