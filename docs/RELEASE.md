...
live under `docs/archive/`. The published in-tree train is `v0.50.0`; Git tag and PyPI
...
5. The tag `v0.50.0` does not already exist locally or on the remote.
...
bash scripts/ci_checks.sh evidence --python 3.12 --gate-version 0.50.0
bash scripts/ci_checks.sh browser --python 3.12
uv run python scripts/check_release_gate.py 0.50.0
...
`v0.50.0` as published in adopter docs/`docs/release.toml`):

```bash
git fetch --tags origin
git rev-parse v0.50.0 >/dev/null 2>&1 && { echo "tag exists; stop"; exit 1; }
git tag -a v0.50.0 -m "Hedron 0.50.0"
git push origin v0.50.0
```

Pushing `v0.50.0` runs `.github/workflows/release.yml`, which re-runs CI, publishes
...
After a successful upload, set `docs/release.toml` `pypi_version = "0.50.0"`,
...
## Template: next patch (`0.50.1`)

Bump the coordinated train to `0.50.1`, set `docs/release.toml` accordingly, then:
