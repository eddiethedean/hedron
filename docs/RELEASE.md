...
bash scripts/ci_checks.sh evidence --python 3.12 --gate-version 0.49.0
bash scripts/ci_checks.sh browser --python 3.12
uv run python scripts/check_release_gate.py 0.49.0
...
`v0.49.0` as published in adopter docs/`docs/release.toml`):

```bash
git fetch --tags origin
git rev-parse v0.49.0 >/dev/null 2>&1 && { echo "tag exists; stop"; exit 1; }
git tag -a v0.49.0 -m "Hedron 0.49.0"
git push origin v0.49.0
```

Pushing `v0.49.0` runs `.github/workflows/release.yml`, which re-runs CI, publishes
