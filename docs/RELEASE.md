...
- If `git rev-parse v0.24.0` **fails** (tag missing locally/remotely), run the **Cut
  steps** below once from green `main`.
- If `v0.24.0` **already exists** on the remote (and PyPI serves `0.24.0`), do **not**
  retag or overwrite the release — use the **0.24.x patch** template for later fixes.
...
Commands for the `0.24.0` publish. Skip tagging when `v0.24.0` already exists (see **Tag
rule**). For later patches, replace with `0.24.1` / `v0.24.1` (see patch template below).
...
4. Tag and push **only if** `v0.24.0` is still missing (trusted workflow publishes when
   configured):

```bash
git rev-parse v0.24.0 >/dev/null 2>&1 && echo "tag exists — do not retag" && exit 1
