...
live under `docs/archive/`. The published in-tree train is `v0.51.2`. PyPI currently
serves `hedron` `0.51.0` (Git tag `v0.51.0`) until the `v0.51.2` upload lands.
...
5. The tag `v0.51.2` does not already exist locally or on the remote.
6. Keep `registry_status = "deferred"` and `pypi_version = "0.51.0"` until PyPI
   actually serves `0.51.2`. First-run copy-paste must keep using `pypi_pin_*`.
...
uv run python scripts/check_release_gate.py 0.51.2
...
`v0.51.0` is already tagged and published. For this patch:
...
Pushing `v0.51.2` runs `.github/workflows/release.yml`, which re-runs CI, publishes
...
Do not retag `v0.50.3`, `v0.50.2`, `v0.50.1`, `v0.50.0`, `v0.49.1`, `v0.49.0`,
`v0.51.0`, or `v0.51.1`.
