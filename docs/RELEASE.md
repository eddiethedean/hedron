...
This is the living maintainer runbook for the `0.39.x` train. Historical cut records
live under `docs/archive/`. The last published release is `v0.39.0`; the next planned
patch is `v0.39.1`.
...
3. `docs/acceptance/release-gate-0.39.toml` remains Verified and the 0.39 package verifier
   passes.
...
bash scripts/ci_checks.sh evidence --python 3.12 --gate-version 0.39.0
bash scripts/ci_checks.sh browser --python 3.12
uv run python scripts/check_release_gate.py 0.39.0
uv run python scripts/verify_pkg_39.py
```

## Tag and publish

After reviewing the complete version/changelog diff (tip honesty already treats
`v0.39.0` as published in adopter docs/`docs/release.toml`):

```bash
git fetch --tags origin
git rev-parse v0.39.0 >/dev/null 2>&1 && { echo "tag exists; stop"; exit 1; }
git tag -a v0.39.0 -m "Hedron 0.39.0"
git push origin v0.39.0
```

## Template: next patch (`0.39.1`)

Bump the coordinated train to `0.39.1`, set `docs/release.toml` accordingly, then:

```bash
bash scripts/ci_checks.sh evidence --python 3.12 --gate-version 0.39.1
uv run python scripts/check_release_gate.py 0.39.1
git tag -a v0.39.1 -m "Hedron 0.39.1"
