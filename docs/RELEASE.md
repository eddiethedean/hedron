...
`v0.42.0` as published in adopter docs/`docs/release.toml`):

```bash
git fetch --tags origin
git rev-parse v0.40.0 >/dev/null 2>&1 && { echo "tag exists; stop"; exit 1; }
git tag -a v0.40.0 -m "Hedron 0.40.0"
git push origin v0.40.0
```

## Template: next patch (`0.40.1`)

Bump the coordinated train to `0.40.1`, set `docs/release.toml` accordingly, then:

```bash
bash scripts/ci_checks.sh evidence --python 3.12 --gate-version 0.40.1
uv run python scripts/check_release_gate.py 0.40.1
git tag -a v0.40.1 -m "Hedron 0.40.1"
