...
This is the maintainer runbook for the published `0.60.x` train. Historical cut records
live under `docs/archive/`. The verified repository release is `v0.60.0`; PyPI publication is
deferred until the coordinated upload.
...
3. `docs/acceptance/release-gate-0.60.toml` is Verified with zero Deferred gates and
   `scripts/verify_pkg_59.py` passes. Historical packets `scripts/verify_pkg_58.py`,
...
bash scripts/ci_checks.sh evidence --python 3.12 --gate-version 0.60.0
bash scripts/ci_checks.sh browser --python 3.12
uv run python scripts/check_release_gate.py 0.60.0
