...
`fastapi-workbench` is currently `1.0.2`. The removed `hedron-workbench` package is not part of
...
uv run python scripts/check_published_quickstart.py 1.0.2 --dist-dir dist --attempts 1
...
2. Create and push the coordinated Hedron tag (for example, `v1.0.2`). The coordinated workflow
