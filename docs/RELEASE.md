...
`docs/archive/`. The current repository release is `v0.62.0`, published on PyPI.
...
3. `docs/acceptance/release-gate-0.62.toml` is Verified with zero Deferred gates and
   `scripts/check_062.py --gate CONTRACT-062 --verify` passes for the published release. Historical packets `scripts/verify_pkg_61.py`,
...
bash scripts/ci_checks.sh evidence --python 3.12 --gate-version 0.62.0
bash scripts/ci_checks.sh browser --python 3.12
uv run python scripts/check_release_gate.py 0.62.0
uv run python scripts/check_062.py --gate CONTRACT-062 --verify
