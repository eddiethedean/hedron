...
3. `docs/acceptance/release-gate-0.38.toml` remains Verified and the 0.38 package verifier
   passes.
...
bash scripts/ci_checks.sh evidence --python 3.12 --gate-version 0.38.0
bash scripts/ci_checks.sh browser --python 3.12
uv run python scripts/check_release_gate.py 0.38.0
uv run python scripts/verify_pkg_38.py
```
...
git tag -a v0.38.0 -m "Hedron 0.38.0"
git push origin v0.38.0
```

## Template: next patch (`0.38.1`)

Bump the coordinated train to `0.38.1`, set `docs/release.toml` accordingly, then:

```bash
bash scripts/ci_checks.sh evidence --python 3.12 --gate-version 0.38.1
uv run python scripts/check_release_gate.py 0.38.1
git tag -a v0.38.1 -m "Hedron 0.38.1"
