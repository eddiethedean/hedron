...
live under `docs/archive/`. The published in-tree train is `v0.49.1`; Git tag and PyPI
...
5. The tag `v0.49.1` does not already exist locally or on the remote.
...
`v0.49.1` as published in adopter docs/`docs/release.toml`):

```bash
git fetch --tags origin
git rev-parse v0.49.1 >/dev/null 2>&1 && { echo "tag exists; stop"; exit 1; }
git tag -a v0.49.1 -m "Hedron 0.49.1"
git push origin v0.49.1
```

Pushing `v0.49.1` runs `.github/workflows/release.yml`, which re-runs CI, publishes
...
`v0.47.0`, `v0.48.0`, or a later `v0.49.0` placeholder.

After a successful upload, set `docs/release.toml` `pypi_version = "0.49.1"`,
...
## Template: next patch (`0.49.2`)

Bump the coordinated train to `0.49.2`, set `docs/release.toml` accordingly, then:

```bash
bash scripts/ci_checks.sh evidence --python 3.12 --gate-version 0.49.2
uv run python scripts/check_release_gate.py 0.49.2
git tag -a v0.49.2 -m "Hedron 0.49.2"
git push origin v0.49.2
```
