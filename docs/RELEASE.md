...
This is the living maintainer runbook for the `0.52.x` train. Historical cut records
live under `docs/archive/`. The published in-tree train is `v0.52.0` (cut-ready;
**do not tag yet**). PyPI currently serves `hedron` `0.51.0` (Git tag `v0.51.0`) until
the `v0.52.0` upload lands.
...
3. `docs/acceptance/release-gate-0.51.toml` remains Verified and
   `scripts/verify_pkg_51.py` still passes as a historical packet. The living packet is
   `scripts/verify_pkg_52.py` (omit `--allow-planned`).
...
5. The tag `v0.52.0` does not already exist locally or on the remote.
6. Keep `registry_status = "deferred"` and `pypi_version = "0.51.0"` until PyPI
   actually serves `0.52.0`. First-run copy-paste must keep using `pypi_pin_*`.
...
bash scripts/ci_checks.sh evidence --python 3.12 --gate-version 0.52.0
bash scripts/ci_checks.sh browser --python 3.12
uv run python scripts/check_release_gate.py 0.52.0
...
`v0.51.0` is already tagged and published. For this in-tree cut, **do not tag yet**.
When ready:

```bash
git fetch --tags origin
git rev-parse v0.52.0 >/dev/null 2>&1 && { echo "tag exists; stop"; exit 1; }
git tag -a v0.52.0 -m "Hedron 0.52.0"
git push origin v0.52.0
```

Pushing `v0.52.0` runs `.github/workflows/release.yml`, which re-runs CI, publishes
...
Do not retag `v0.51.2`, `v0.51.1`, `v0.51.0`, `v0.50.3`, `v0.50.2`, `v0.50.1`,
`v0.50.0`, `v0.49.1`, or `v0.49.0`.
