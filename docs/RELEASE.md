...
This is the living maintainer runbook for the `0.32.x` train. Historical cut records
...
3. `docs/acceptance/release-gate-0.32.toml` remains Verified and the 0.32 package verifier
   passes.
...
bash scripts/ci_checks.sh evidence --python 3.12 --gate-version 0.32.0
bash scripts/ci_checks.sh browser --python 3.12
uv run python scripts/check_release_gate.py 0.32.0
...
`v0.32.0` as published in adopter docs/`docs/release.toml`):

```bash
git fetch --tags origin
git rev-parse v0.32.0 >/dev/null 2>&1 && { echo "tag exists; stop"; exit 1; }
git tag -a v0.32.0 -m "Hedron 0.32.0"
git push origin v0.32.0
```

## Template: next patch (`0.32.1`)

Bump the coordinated train to `0.32.1`, set `docs/release.toml` accordingly, then:

```bash
bash scripts/ci_checks.sh evidence --python 3.12 --gate-version 0.32.1
uv run python scripts/check_release_gate.py 0.32.1
git tag -a v0.32.1 -m "Hedron 0.32.1"
