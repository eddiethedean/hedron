...
This is the maintainer runbook for the `0.63.x` train. Historical cut records live under
`docs/archive/`. The current repository and PyPI release is `v0.63.0`.
...
3. `docs/acceptance/release-gate-0.63.toml` is Verified with zero Deferred gates and
   `scripts/check_063.py --gate CONTRACT-063 --verify` passes for the published release. Historical packets
...
bash scripts/ci_checks.sh evidence --python 3.12 --gate-version 0.63.0
bash scripts/ci_checks.sh browser --python 3.12
uv run python scripts/check_release_gate.py 0.63.0
