...
3. `docs/acceptance/release-gate-0.35.toml` remains Verified and the 0.35 package verifier
   passes.
...
bash scripts/ci_checks.sh evidence --python 3.12 --gate-version 0.35.0
bash scripts/ci_checks.sh browser --python 3.12
uv run python scripts/check_release_gate.py 0.35.0
uv run python scripts/verify_pkg_35.py
```
...
Bump the coordinated train to `0.33.1`, set `docs/release.toml` accordingly, then:

```bash
bash scripts/ci_checks.sh evidence --python 3.12 --gate-version 0.33.1
uv run python scripts/check_release_gate.py 0.33.1
git tag -a v0.33.1 -m "Hedron 0.33.1"
