...
live under `docs/archive/`. The published in-tree train is `v0.50.1`. PyPI currently
serves `hedron` `0.48.0` (Git tag `v0.48.0`) until the `v0.50.1` upload lands.
...
6. Keep `registry_status = "deferred"` and `pypi_version = "0.48.0"` until PyPI
   actually serves `0.50.1`.
...
`v0.47.0`, `v0.48.0`, `v0.49.0`, `v0.49.1`, or `v0.50.0`.

## After a successful upload

Set `docs/release.toml` `pypi_version = "0.50.1"`, `pypi_pin_floor` / `pypi_pin_ceiling`
to the in-tree pin, and `registry_status = "uploaded"`, then drop first-run
“tag/PyPI deferred” language so adopter docs match the index.
