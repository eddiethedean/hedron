...
This is the living maintainer runbook for the `0.48.x` train. Historical cut records
live under `docs/archive/`. The published in-tree train is `v0.48.0`; Git tag and PyPI
remain **deferred**. The next planned phase is `v0.49.0`. PyPI currently serves
...
3. `docs/acceptance/release-gate-0.47.toml` remains Verified and `scripts/verify_pkg_47.py`
   still passes as a historical packet. The living packet is `scripts/verify_pkg_48.py`
   (omit `--allow-planned`).
...
5. The tag `v0.48.0` does not already exist locally or on the remote.
...
bash scripts/ci_checks.sh evidence --python 3.12 --gate-version 0.48.0
bash scripts/ci_checks.sh browser --python 3.12
uv run python scripts/check_release_gate.py 0.48.0
...
`v0.48.0` as published in adopter docs/`docs/release.toml`):

```bash
git fetch --tags origin
git rev-parse v0.48.0 >/dev/null 2>&1 && { echo "tag exists; stop"; exit 1; }
git tag -a v0.48.0 -m "Hedron 0.48.0"
git push origin v0.48.0
```

Pushing `v0.48.0` runs `.github/workflows/release.yml`, which re-runs CI, publishes
...
After a successful upload, set `docs/release.toml` `pypi_version = "0.48.0"`,
