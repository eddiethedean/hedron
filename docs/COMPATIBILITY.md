...
`hedron-core>=0.36.0,<0.37`. The flagship `hedron[charts]>=0.36.0,<0.37` extra enforces
that chart floor.
...
| Matplotlib | `>=3.8,<4` via `hedron-charts[matplotlib]` | Install with `hedron[charts]>=0.36.0,<0.37`; Matplotlib/static charts are the conservative default. |
