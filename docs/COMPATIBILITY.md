...
`hedron-core>=0.52.0,<0.53`. The flagship `hedron[charts]>=0.52.0,<0.53` extra enforces
that chart floor.
...
| Matplotlib | `>=3.8,<4` via `hedron-charts[matplotlib]` | Install with `hedron[charts]>=0.52.0,<0.53`; Matplotlib/static charts are the conservative default. |
