...
stay on the `0.49.x` train (Published as in-tree `v0.49.0`). Independent Beta packages
...
`hedron-core>=0.49.0,<0.50`. The flagship `hedron[charts]>=0.49.0,<0.50` extra enforces
that chart floor.
...
| Matplotlib | `>=3.8,<4` via `hedron-charts[matplotlib]` | Install with `hedron[charts]>=0.49.0,<0.50`; Matplotlib/static charts are the conservative default. |
