...
`hedron-core>=0.31.0,<0.32`. The flagship `hedron[charts]>=0.31.0,<0.32` extra enforces
that chart floor.
...
| Matplotlib | `>=3.8,<4` via `hedron-charts[matplotlib]` | Install with `hedron[charts]>=0.31.0,<0.32`; Matplotlib/static charts are the conservative default. |
