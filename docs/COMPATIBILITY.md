...
**Status:** Accepted for the **0.36.x** train (**Published**; tip
PyPI/git = `v0.36.0`)
**Reviewed:** 2026-08-13
...
stay on the `0.36.x` train (Published as `v0.37.0`). Independent Beta packages
...
`hedron-core>=0.37.0,<0.38`. The flagship `hedron[charts]>=0.37.0,<0.38` extra enforces
that chart floor.
...
| Matplotlib | `>=3.8,<4` via `hedron-charts[matplotlib]` | Install with `hedron[charts]>=0.37.0,<0.38`; Matplotlib/static charts are the conservative default. |
