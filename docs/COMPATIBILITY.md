...
stay on the `0.36.x` train (Published as `v0.38.0`). Independent Beta packages
...
`hedron-core>=0.38.0,<0.39`. The flagship `hedron[charts]>=0.38.0,<0.39` extra enforces
that chart floor.
...
| Matplotlib | `>=3.8,<4` via `hedron-charts[matplotlib]` | Install with `hedron[charts]>=0.38.0,<0.39`; Matplotlib/static charts are the conservative default. |
