...
**Status:** Accepted for the **0.59.x** train, with published tip `v0.59.0` on PyPI.
**Reviewed:** 2026-08-18
...
stay on the living `0.59.x` train (published tip `v0.59.0` on PyPI). Independent Beta packages
...
`hedron-core>=0.59.0,<0.60`. The flagship `hedron[charts]>=0.59.0,<0.60` extra enforces
that chart floor.
...
| Matplotlib | `>=3.8,<4` via `hedron-charts[matplotlib]` | Install with `hedron[charts]>=0.59.0,<0.60`; Matplotlib/static charts are the conservative default. |
