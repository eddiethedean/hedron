...
`hedron-core>=0.54.0,<0.55`. The flagship `hedron[charts]>=0.54.0,<0.55` extra enforces
that chart floor.
...
| Matplotlib | `>=3.8,<4` via `hedron-charts[matplotlib]` | Install with `hedron[charts]>=0.54.0,<0.55`; Matplotlib/static charts are the conservative default. |
