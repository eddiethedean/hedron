...
**Status:** Accepted for the **0.59.x** train, with in-tree tip `v0.59.0`; PyPI serves `v0.58.0`
**Reviewed:** 2026-08-18
...
stay on the living `0.59.x` train (in-tree tip `v0.59.0`; PyPI tip `v0.58.0`). Independent Beta packages
...
`hedron-core>=0.58.0,<0.60`. The flagship `hedron[charts]>=0.58.0,<0.60` extra enforces
that chart floor.
...
| Matplotlib | `>=3.8,<4` via `hedron-charts[matplotlib]` | Install with `hedron[charts]>=0.58.0,<0.60`; Matplotlib/static charts are the conservative default. |
