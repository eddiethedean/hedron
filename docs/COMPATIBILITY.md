...
**Status:** Accepted for the **0.41.x** train (**Published**; tip
PyPI/git = `v0.41.0`)
**Reviewed:** 2026-08-14
...
stay on the `0.41.x` train (Published as `v0.41.0`). Independent Beta packages
...
`hedron-core>=0.41.0,<0.42`. The flagship `hedron[charts]>=0.41.0,<0.42` extra enforces
that chart floor.
...
| Matplotlib | `>=3.8,<4` via `hedron-charts[matplotlib]` | Install with `hedron[charts]>=0.41.0,<0.42`; Matplotlib/static charts are the conservative default. |
