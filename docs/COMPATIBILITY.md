...
`hedron-core>=0.56.0,<0.57`. The flagship `hedron[charts]>=0.56.0,<0.57` extra enforces
that chart floor.
...
| Matplotlib | `>=3.8,<4` via `hedron-charts[matplotlib]` | Install with `hedron[charts]>=0.56.0,<0.57`; Matplotlib/static charts are the conservative default. |
