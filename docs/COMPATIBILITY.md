...
**Status:** Accepted for the **0.48.x** train (**Published**; tip
PyPI/git = `v0.48.0`)
**Reviewed:** 2026-08-17
...
stay on the `0.48.x` train (Published as in-tree `v0.48.0`). Independent Beta packages
...
`hedron-core>=0.48.0,<0.49`. The flagship `hedron[charts]>=0.48.0,<0.49` extra enforces
that chart floor.
...
| Matplotlib | `>=3.8,<4` via `hedron-charts[matplotlib]` | Install with `hedron[charts]>=0.48.0,<0.49`; Matplotlib/static charts are the conservative default. |
