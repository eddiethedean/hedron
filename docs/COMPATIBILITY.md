...
**Status:** Accepted for the **0.34.x** train (**Published**; tip
PyPI/git = `v0.34.0`)
**Reviewed:** 2026-08-11
...
stay on the `0.34.x` train (Published as `v0.34.0`). Independent Beta packages
...
`hedron-core>=0.35.0,<0.36`. The flagship `hedron[charts]>=0.35.0,<0.36` extra enforces
that chart floor.
...
| Matplotlib | `>=3.8,<4` via `hedron-charts[matplotlib]` | Install with `hedron[charts]>=0.35.0,<0.36`; Matplotlib/static charts are the conservative default. |
