...
**Status:** Accepted for the **0.51.x** train (**Published** as `v0.51.0` on PyPI)
**Reviewed:** 2026-08-18
...
stay on the `0.51.x` train (Published as `v0.51.0` on PyPI). Independent Beta packages
...
`hedron-core>=0.51.0,<0.52`. The flagship `hedron[charts]>=0.51.0,<0.52` extra enforces
that chart floor.
...
| Matplotlib | `>=3.8,<4` via `hedron-charts[matplotlib]` | Install with `hedron[charts]>=0.51.0,<0.52`; Matplotlib/static charts are the conservative default. |
