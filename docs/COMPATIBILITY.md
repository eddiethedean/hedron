...
**Status:** Accepted for the **0.60.x** train, with in-tree tip `v0.60.0`; latest PyPI `v0.60.0`.
**Reviewed:** 2026-08-18
...
stay on the living `0.60.x` train (in-tree tip `v0.60.0`; latest PyPI `v0.60.0`). Independent Beta packages
...
`hedron-core>=0.60.0,<0.61`. The flagship `hedron[charts]>=0.60.0,<0.61` extra enforces
that chart floor.
...
| Matplotlib | `>=3.8,<4` via `hedron-charts[matplotlib]` | Install with `hedron[charts]>=0.60.0,<0.61`; Matplotlib/static charts are the conservative default. |
