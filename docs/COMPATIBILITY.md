...
**Status:** Accepted for the **0.23.0** train (**Published** as `v0.23.0`; last published
PyPI/git = `v0.23.0`)
**Reviewed:** 2026-08-08
...
`hedron-extras`)
stay on the `0.23.x` train (Published as `0.23.0`; last published
`v0.23.0`). Alpha packages `hedron-charts`, `hedron-gradio`,
`hedron-sample-kit`, `hedron-native`, `hedron-notebook`, and `hedron-mcp`
version independently (`0.1.x`) and declare `hedron-core>=0.23.0,<0.24`.
...
Compatibility claims for the **0.23.0** train require clean-install, package, FastAPI
