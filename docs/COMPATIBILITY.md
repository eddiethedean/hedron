...
**Status:** Accepted for the **0.62.x** train, with published tip `v0.62.0` on PyPI.
**Reviewed:** 2026-08-18
...
stay on the published `0.62.x` train (`v0.62.0` on PyPI). Independent Beta packages
...
`hedron-core>=0.62.0,<0.63`. The flagship `hedron[charts]>=0.62.0,<0.63` extra enforces
that chart floor.
...
| Matplotlib | `>=3.8,<4` via `hedron-charts[matplotlib]` | Install with `hedron[charts]>=0.62.0,<0.63`; Matplotlib/static charts are the conservative default. |
