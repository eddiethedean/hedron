...
**Status:** Accepted for the **0.58.x** train, with in-tree tip `v0.58.1`; PyPI serves `v0.58.0`
**Reviewed:** 2026-08-18
...
stay on the living `0.58.x` train (in-tree tip `v0.58.1`; PyPI tip `v0.58.0`). Independent Beta packages
...
`hedron-core>=0.58.0,<0.59`. The flagship `hedron[charts]>=0.58.0,<0.59` extra enforces
that chart floor.
...
| Matplotlib | `>=3.8,<4` via `hedron-charts[matplotlib]` | Install with `hedron[charts]>=0.58.0,<0.59`; Matplotlib/static charts are the conservative default. |
