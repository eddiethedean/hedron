...
`hedron-core>=0.50.0,<0.51`. The flagship `hedron[charts]>=0.50.0,<0.51` extra enforces
that chart floor.
...
| Matplotlib | `>=3.8,<4` via `hedron-charts[matplotlib]` | Install with `hedron[charts]>=0.50.0,<0.51`; Matplotlib/static charts are the conservative default. |
