...
`hedron-core>=0.39.0,<0.40`. The flagship `hedron[charts]>=0.39.0,<0.40` extra enforces
that chart floor.
...
| Matplotlib | `>=3.8,<4` via `hedron-charts[matplotlib]` | Install with `hedron[charts]>=0.39.0,<0.40`; Matplotlib/static charts are the conservative default. |
