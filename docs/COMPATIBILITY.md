...
**Status:** Accepted for the **0.50.x** train (**Published** as `v0.50.3` in-tree; latest on PyPI `v0.50.1`)
**Reviewed:** 2026-08-18
...
stay on the `0.50.x` train (Published as `v0.50.3` in-tree; latest on PyPI `v0.50.1`). Independent Beta packages
...
coordinated 0.50 train. `hedron-elements` is Beta for the declared Supported
inventory only.

### Charts and sample-kit compatibility floor

`hedron-charts>=0.2.0,<0.3` and `hedron-sample-kit>=0.1.10,<0.2` are compatible with
`hedron-core>=0.50.1,<0.51`. The flagship `hedron[charts]>=0.50.1,<0.51` extra enforces
that chart floor.
...
| Matplotlib | `>=3.8,<4` via `hedron-charts[matplotlib]` | Install with `hedron[charts]>=0.50.1,<0.51`; Matplotlib/static charts are the conservative default. |
...
| External cache | **Redis** `>=7.0` server; client `redis` `>=5,<6`. Serialization: JSON UTF-8 with cache key prefix `h1:c:` (tags `h1:c:tag:`), disjoint from job records `h1:job:`. Failures raise and surface via readiness without caching poisoned values. Shared Redis clients must keep cache and job prefixes non-overlapping. Conformance uses `fakeredis` in unit CI and Redis in ops topology. |
...
Compatibility claims for the **0.50.x** train require clean-install, package, FastAPI
