...
**Status:** Accepted for the **0.56.x** train, published as `v0.56.0` on PyPI
**Reviewed:** 2026-08-18
...
`hedron-core>=0.56.0,<0.58`. The flagship `hedron[charts]>=0.56.0,<0.58` extra enforces
that chart floor.
...
| Matplotlib | `>=3.8,<4` via `hedron-charts[matplotlib]` | Install with `hedron[charts]>=0.56.0,<0.58`; Matplotlib/static charts are the conservative default. |
