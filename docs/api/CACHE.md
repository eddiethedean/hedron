...
|---|---|---|
| `ttl` | `int` / duration | Time-to-live seconds (API-specific) |
| `scope` | `str` | Logical scope (`tenant`, `private`, …) affecting key dimensions |
...
- Keys include function identity, declared public arguments, implementation version, and
  required tenant/user/locale/permission dimensions.
