...
| | |
|---|---|
| Import | `from hedron_core.builtins import SseTrigger` |
...
```python
from hedron_core.builtins import SseTrigger, Text
...
`SseTrigger` participates in interaction markup. Pair it with an explicit `@app.action` POST (and CSRF) when the control mutates state.
