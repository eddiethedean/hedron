...
public PyPI release, `hedron-django>=0.56.0,<0.57`, until the 0.56 upload lands.
Pin from PyPI `hedron>=0.56.0,<0.57` (or `hedron-django>=0.56.0,<0.57`).
...
uvx --from "hedron>=0.56.0,<0.57" hedron new my-django-app --django
...
```bash
pip install "hedron-django>=0.56.0,<0.57" "django>=5.2,<6"
```
