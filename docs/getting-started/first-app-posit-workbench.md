...
mkdir -p ~/projects
cd ~/projects
uvx --from "hedron>=0.60.2,<0.61" hedron new my-workbench-app
cd my-workbench-app
...
A repository checkout reports **`0.60.2`**.
...
python -m pip install "hedron>=0.60.2,<0.61" "hedron-posit>=0.60.2,<0.61" "uvicorn[standard]>=0.30"
