...
mkdir -p ~/projects
cd ~/projects
uvx --from "hedron>=0.37.0,<0.38" hedron new my-workbench-app
cd my-workbench-app
...
Expect the path to end in `my-workbench-app` and the version to be on the current `0.37.x` train
(`0.37.0` or a later patch).
...
python -m pip install "hedron>=0.37.0,<0.38" "hedron-posit>=0.37.0,<0.38" "uvicorn[standard]>=0.30"
