...
mkdir -p ~/projects
cd ~/projects
uvx --from "hedron>=0.47.0,<0.48" hedron new my-workbench-app
cd my-workbench-app
...
Expect the path to end in `my-workbench-app` and the version to be **`0.46.0`**.
...
python -m pip install "hedron>=0.47.0,<0.48" "hedron-posit>=0.47.0,<0.48" "uvicorn[standard]>=0.30"
