...
mkdir -p ~/projects
cd ~/projects
uvx --from "hedron>=0.62.0,<0.63" hedron new my-workbench-app
cd my-workbench-app
...
Expect the path to end in `my-workbench-app` and the PyPI version to be **`0.62.0`**.
...
python -m pip install "hedron>=0.62.0,<0.63" "hedron-posit>=0.62.0,<0.63" "uvicorn[standard]>=0.30"
