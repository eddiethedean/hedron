...
mkdir -p ~/projects
cd ~/projects
uvx --from "hedron>=0.39.0,<0.40" hedron new my-workbench-app
cd my-workbench-app
...
Expect the path to end in `my-workbench-app` and the version to be on the current `0.39.x` train
...
python -m pip install "hedron>=0.39.0,<0.40" "hedron-posit>=0.39.0,<0.40" "uvicorn[standard]>=0.30"
