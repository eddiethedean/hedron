...
mkdir -p ~/projects
cd ~/projects
uvx --from "hedron>=0.50.1,<0.51" hedron new my-workbench-app
cd my-workbench-app
...
Expect the path to end in `my-workbench-app` and the version to be **`0.50.1`**.
...
python -m pip install "hedron>=0.50.1,<0.51" "hedron-posit>=0.50.1,<0.51" "uvicorn[standard]>=0.30"
