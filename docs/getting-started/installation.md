...
!!! note "Corporate proxy / air-gapped installs"
    ...
    extras onto that index; pin `hedron>=0.30.0,<0.31`. Offline: download wheels on a
    connected host (`pip download "hedron>=0.30.0,<0.31"`) and `pip install --no-index
...
| Symptom | Fix |
|---|---|
| `hedron: command not found` | Use `python -m hedron …`, `uvx --from "hedron>=0.30.0,<0.31" …`, or see [FAQ](../guides/faq.md#hedron-command-not-found) / [Troubleshooting](../guides/troubleshooting.md#hedron-command-not-found) |
...
| Wrong / old version | `pip install -U "hedron>=0.30.0,<0.31"` — [Troubleshooting](../guides/troubleshooting.md#wrong-or-unexpected-version) |
...
| Need charts | Install `hedron[charts]>=0.30.0,<0.31` — [Compatibility](../COMPATIBILITY.md#charts-and-sample-kit-compatibility-floor) |
...
```bash
pip install "hedron[data]>=0.30.0,<0.31"          # example
```
...
!!! note "`hedron[browser]` needs Playwright browsers"
    ...
    ```bash
    pip install "hedron[browser]>=0.30.0,<0.31"
...
=== "uv"
    ...
    cd my-hedron-app
    uv add "hedron>=0.30.0,<0.31" "uvicorn[standard]"
    ```
...
=== "pip (macOS/Linux)"
    ...
    python -m pip install "hedron>=0.30.0,<0.31" "uvicorn[standard]"
    ```
