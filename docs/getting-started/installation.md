...
The latest published PyPI release and repository checkout are **`0.63.0`**. The `v0.63.0` tag
and PyPI upload are complete.

| You are… | Install |
|---|---|
| Building an app from PyPI | `hedron>=0.63.0,<0.64` |
| Working in this repository | `uv sync` (editable **0.63.0**) |
...
!!! note "Corporate proxy / air-gapped installs"
    ...
    extras onto that index; pin `hedron>=0.63.0,<0.64`.
    Offline: download wheels on a connected host (`pip download "hedron>=0.63.0,<0.64"`)
...
=== "uv"
    ...
    cd my-hedron-app
    uv add "hedron>=0.63.0,<0.64" "uvicorn[standard]"
    ```
...
=== "pip (macOS/Linux)"
    ...
    python -m pip install "hedron>=0.63.0,<0.64" "uvicorn[standard]"
    ```
...
Expect **`0.63.0`** when installed from PyPI and **`0.63.0`** from this source checkout.
...
This checkout is **`0.63.0`**; public application installs remain **`0.63.0`**. Use
`hedron>=0.63.0,<0.64` for applications.
...
!!! note "`hedron[browser]` needs Playwright browsers"
    ...
    ```bash
    pip install "hedron[browser]>=0.63.0,<0.64"
...
| `hedron: command not found` | Use `python -m hedron …`, `uvx --from "hedron>=0.63.0,<0.64" …`, or see [FAQ](../guides/faq.md#hedron-command-not-found) / [Troubleshooting](../guides/troubleshooting.md#hedron-command-not-found) |
...
| Wrong / old version | Upgrade: `pip install -U "hedron>=0.63.0,<0.64"` — [Troubleshooting](../guides/troubleshooting.md#wrong-or-unexpected-version) |
...
| Need charts | Install `hedron[charts]>=0.63.0,<0.64` — [Compatibility](../COMPATIBILITY.md#charts-and-sample-kit-compatibility-floor) |
