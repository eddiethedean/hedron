...
=== "uv (recommended CLI)"
    ...
    Or install the CLI once with `uv tool install "hedron>=0.15.0"`, then `hedron new …`.
...
| `pyproject.toml` | Project deps (`hedron>=0.15.0`) and `[tool.hedron]` |
...
| Symptom | Fix |
|---|---|
| `hedron: command not found` | Use `python -m hedron …`, `uvx --from "hedron>=0.15.0" …`, or see [FAQ](../guides/faq.md#hedron-command-not-found) / [Troubleshooting](../guides/troubleshooting.md#hedron-command-not-found) |
...
| Wrong / old version | `pip install -U "hedron>=0.15.0"` — [Troubleshooting](../guides/troubleshooting.md#wrong-or-unexpected-version) |
...
=== "uv"
    ...
    cd my-hedron-app
    uv add "hedron>=0.15.0" "uvicorn[standard]"
    ```
