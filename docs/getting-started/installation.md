...
!!! note "Corporate proxy / air-gapped installs"
    ...
    extras onto that index; pin `hedron>=0.45.0,<0.46`. Offline: download wheels on a
    connected host (`pip download "hedron>=0.45.0,<0.46"`) and `pip install --no-index
...
Expect **`0.45.0`** or a newer **`0.45.x`** patch on this train. Last published
PyPI/git is **`v0.45.0`**. Pin with `hedron>=0.45.0,<0.46` for
production.
...
```bash
pip install "hedron[data]>=0.45.0,<0.46"          # example
```

Charts and the sample plugin have explicit compatibility floors:

```bash
pip install "hedron[charts]>=0.45.0,<0.46"
...
!!! note "`hedron[browser]` needs Playwright browsers"
    ...
    ```bash
    pip install "hedron[browser]>=0.45.0,<0.46"
...
=== "uv"
    ...
    cd my-hedron-app
    uv add "hedron>=0.45.0,<0.46" "uvicorn[standard]"
    ```
