...
!!! note "Corporate proxy / air-gapped installs"
    ...
    extras onto that index; pin `hedron>=0.36.0,<0.37`. Offline: download wheels on a
    connected host (`pip download "hedron>=0.36.0,<0.37"`) and `pip install --no-index
...
Expect **`0.36.0`** or a newer **`0.36.x`** patch on this train. Last published
PyPI/git is **`v0.36.0`**. Pin with `hedron>=0.36.0,<0.37` for
production.
...
| Symptom | Fix |
|---|---|
| `hedron: command not found` | Use `python -m hedron …`, `uvx --from "hedron>=0.36.0,<0.37" …`, or see [FAQ](../guides/faq.md#hedron-command-not-found) / [Troubleshooting](../guides/troubleshooting.md#hedron-command-not-found) |
...
| Wrong / old version | `pip install -U "hedron>=0.36.0,<0.37"` — [Troubleshooting](../guides/troubleshooting.md#wrong-or-unexpected-version) |
...
| Need charts | Install `hedron[charts]>=0.36.0,<0.37` — [Compatibility](../COMPATIBILITY.md#charts-and-sample-kit-compatibility-floor) |
...
```bash
pip install "hedron[data]>=0.36.0,<0.37"          # example
```
...
!!! note "`hedron[browser]` needs Playwright browsers"
    ...
    ```bash
    pip install "hedron[browser]>=0.36.0,<0.37"
...
=== "uv"
    ...
    cd my-hedron-app
    uv add "hedron>=0.36.0,<0.37" "uvicorn[standard]"
    ```
