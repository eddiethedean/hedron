...
The latest published PyPI release and repository checkout are **`0.60.2`**.
The `v0.60.2` Git tag and PyPI publication are complete.
Application commands on this page use the published pin; contributors working in this
checkout should use `uv sync`.

| You are… | Install |
|---|---|
| Building an app from PyPI | `hedron>=0.60.2,<0.61` |
| Working in this repository | `uv sync` (editable **0.60.2**) |
...
!!! note "Corporate proxy / air-gapped installs"
    ...
    extras onto that index; pin `hedron>=0.60.2,<0.61`.
    Offline: download wheels on a connected host (`pip download "hedron>=0.60.2,<0.61"`)
...
=== "uv"
    ...
    cd my-hedron-app
    uv add "hedron>=0.60.2,<0.61" "uvicorn[standard]"
    ```
...
Expect **`0.60.2`** when installed from PyPI or from the source checkout.
...
This checkout and public application installs are **`0.60.2`**. Use
`hedron>=0.60.2,<0.61` for applications.
...
!!! note "`hedron[browser]` needs Playwright browsers"
    ...
    ```bash
    pip install "hedron[browser]>=0.60.2,<0.61"
...
| `hedron: command not found` | Use `python -m hedron …`, `uvx --from "hedron>=0.60.2,<0.61" …`, or see [FAQ](../guides/faq.md#hedron-command-not-found) / [Troubleshooting](../guides/troubleshooting.md#hedron-command-not-found) |
...
| Wrong / old version | Upgrade: `pip install -U "hedron>=0.60.2,<0.61"` — [Troubleshooting](../guides/troubleshooting.md#wrong-or-unexpected-version) |
...
| Need charts | Install `hedron[charts]>=0.60.2,<0.61` — [Compatibility](../COMPATIBILITY.md#charts-and-sample-kit-compatibility-floor) |
