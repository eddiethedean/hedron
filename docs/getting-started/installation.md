...
| You are… | Install |
|---|---|
| Building an app from PyPI | `hedron>=0.60.0,<0.61` |
...
!!! note "Corporate proxy / air-gapped installs"
    ...
    extras onto that index; pin `hedron>=0.60.0,<0.61`.
    Offline: download wheels on a connected host (`pip download "hedron>=0.60.0,<0.61"`)
...
=== "uv"
    ...
    cd my-hedron-app
    uv add "hedron>=0.60.0,<0.61" "uvicorn[standard]"
    ```
...
`hedron>=0.60.0,<0.61` for applications.
...
!!! note "`hedron[browser]` needs Playwright browsers"
    ...
    ```bash
    pip install "hedron[browser]>=0.60.0,<0.61"
...
| `hedron: command not found` | Use `python -m hedron …`, `uvx --from "hedron>=0.60.0,<0.61" …`, or see [FAQ](../guides/faq.md#hedron-command-not-found) / [Troubleshooting](../guides/troubleshooting.md#hedron-command-not-found) |
...
| Wrong / old version | Upgrade: `pip install -U "hedron>=0.60.0,<0.61"` — [Troubleshooting](../guides/troubleshooting.md#wrong-or-unexpected-version) |
...
| Need charts | Install `hedron[charts]>=0.60.0,<0.61` — [Compatibility](../COMPATIBILITY.md#charts-and-sample-kit-compatibility-floor) |
