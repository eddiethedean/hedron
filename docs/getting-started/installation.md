...
| Symptom | Fix |
|---|---|
| `hedron: command not found` | Use `python -m hedron …`, `uvx --from "hedron>=0.22.0,<0.23" …`, or see [FAQ](../guides/faq.md#hedron-command-not-found) / [Troubleshooting](../guides/troubleshooting.md#hedron-command-not-found) |
...
| Wrong / old version | `pip install -U "hedron>=0.22.0,<0.23"` — [Troubleshooting](../guides/troubleshooting.md#wrong-or-unexpected-version) |
...
```bash
pip install "hedron[data]>=0.22.0,<0.23"          # example
...
=== "uv"
    ...
    cd my-hedron-app
    uv add "hedron>=0.22.0,<0.23" "uvicorn[standard]"
    ```
