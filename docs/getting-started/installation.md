...
!!! note "Corporate proxy / air-gapped installs"
    ...
    extras onto that index; pin `hedron>=0.35.0,<0.36`. Offline: download wheels on a
    connected host (`pip download "hedron>=0.35.0,<0.36"`) and `pip install --no-index
...
PyPI/git is **`v0.34.0`**. Pin with `hedron>=0.35.0,<0.36` for
production.
...
| Symptom | Fix |
|---|---|
| `hedron: command not found` | Use `python -m hedron …`, `uvx --from "hedron>=0.35.0,<0.36" …`, or see [FAQ](../guides/faq.md#hedron-command-not-found) / [Troubleshooting](../guides/troubleshooting.md#hedron-command-not-found) |
...
| Wrong / old version | `pip install -U "hedron>=0.35.0,<0.36"` — [Troubleshooting](../guides/troubleshooting.md#wrong-or-unexpected-version) |
...
| Need charts | Install `hedron[charts]>=0.35.0,<0.36` — [Compatibility](../COMPATIBILITY.md#charts-and-sample-kit-compatibility-floor) |
...
```bash
pip install "hedron[data]>=0.35.0,<0.36"          # example
```
...
!!! note "`hedron[browser]` needs Playwright browsers"
    ...
    ```bash
    pip install "hedron[browser]>=0.35.0,<0.36"
...
=== "uv"
    ...
    cd my-hedron-app
    uv add "hedron>=0.35.0,<0.36" "uvicorn[standard]"
    ```
