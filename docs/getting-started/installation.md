...
This documentation describes the **0.53.x** train. The living in-tree tip is **`v0.53.0`**;
**`v0.52.0` is on PyPI today**. Git tag and PyPI upload for **0.53.0** remain **deferred**
until the cut lands —
use registry-resolvable pins below unless you are developing Hedron itself.

| You are… | Install |
|---|---|
| Building an app from PyPI | `hedron>=0.54.0,<0.55` |
| Working in this repository | `uv sync` (editable **0.53.0**) |
...
!!! note "Corporate proxy / air-gapped installs"
    ...
    extras onto that index; pin `hedron>=0.54.0,<0.55`.
    Offline: download wheels on a connected host (`pip download "hedron>=0.54.0,<0.55"`)
...
This checkout is **0.53.0**. Application installs from PyPI use
`hedron>=0.54.0,<0.55`.
...
```bash
pip install "hedron[data]>=0.54.0,<0.55"
pip install "hedron[charts]>=0.54.0,<0.55"
...
!!! note "`hedron[browser]` needs Playwright browsers"
    ...
    ```bash
    pip install "hedron[browser]>=0.54.0,<0.55"
...
| Symptom | Fix |
|---|---|
| `hedron: command not found` | Use `python -m hedron …`, `uvx --from "hedron>=0.54.0,<0.55" …`, or see [FAQ](../guides/faq.md#hedron-command-not-found) / [Troubleshooting](../guides/troubleshooting.md#hedron-command-not-found) |
...
| Wrong / old version | Upgrade: `pip install -U "hedron>=0.54.0,<0.55"` — [Troubleshooting](../guides/troubleshooting.md#wrong-or-unexpected-version) |
...
| Need charts | Install `hedron[charts]>=0.54.0,<0.55` — [Compatibility](../COMPATIBILITY.md#charts-and-sample-kit-compatibility-floor) |
