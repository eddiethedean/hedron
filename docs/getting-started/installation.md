...
The repository contains the **0.56.x** train, while the latest installable PyPI release is
**0.56.0**. The 0.56 upload is deferred. Application commands on this page therefore use
the registry-resolvable pin; contributors working in this checkout should use `uv sync`.

| You are… | Install |
|---|---|
| Building an app from PyPI | `hedron>=0.56.0,<0.57` |
| Working in this repository | `uv sync` (editable **0.56.0**) |
...
!!! note "Corporate proxy / air-gapped installs"
    ...
    extras onto that index; pin `hedron>=0.56.0,<0.57`.
    Offline: download wheels on a connected host (`pip download "hedron>=0.56.0,<0.57"`)
...
=== "uv"
    ...
    cd my-hedron-app
    uv add "hedron>=0.56.0,<0.57" "uvicorn[standard]"
    ```
...
```bash
pip install "hedron[data]>=0.56.0,<0.57"
...
!!! note "`hedron[browser]` needs Playwright browsers"
    ...
    ```bash
    pip install "hedron[browser]>=0.56.0,<0.57"
...
| Symptom | Fix |
|---|---|
| `hedron: command not found` | Use `python -m hedron …`, `uvx --from "hedron>=0.56.0,<0.57" …`, or see [FAQ](../guides/faq.md#hedron-command-not-found) / [Troubleshooting](../guides/troubleshooting.md#hedron-command-not-found) |
...
| Wrong / old version | Upgrade: `pip install -U "hedron>=0.56.0,<0.57"` — [Troubleshooting](../guides/troubleshooting.md#wrong-or-unexpected-version) |
...
| Need charts | Install `hedron[charts]>=0.56.0,<0.57` — [Compatibility](../COMPATIBILITY.md#charts-and-sample-kit-compatibility-floor) |
