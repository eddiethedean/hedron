...
The repository contains the **0.58.1** in-tree patch cut; PyPI serves **`v0.58.0`**.
The Git tag/PyPI upload for 0.58.1 is deferred.
...
| You are… | Install |
|---|---|
| Building an app from PyPI | `hedron>=0.58.0,<0.59` |
| Working in this repository | `uv sync` (editable **0.58.1**) |
...
!!! note "Corporate proxy / air-gapped installs"
    ...
    extras onto that index; pin `hedron>=0.58.0,<0.59`.
    Offline: download wheels on a connected host (`pip download "hedron>=0.58.0,<0.59"`)
...
Expect **`0.58.0`** from PyPI and **`0.58.1`** from the repository checkout.
...
This checkout is **0.58.1**. Application installs from PyPI use
`hedron>=0.58.0,<0.59`.
...
```bash
pip install "hedron[data]>=0.58.0,<0.59"
pip install "hedron[charts]>=0.58.0,<0.59"
...
!!! note "`hedron[browser]` needs Playwright browsers"
    ...
    ```bash
    pip install "hedron[browser]>=0.58.0,<0.59"
