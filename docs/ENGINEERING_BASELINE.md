...
- Pyright runs strict type checking on public packages. `hedron-core` and `hedron` additionally
  have a warning-fatal, zero-diagnostic ratchet; documented narrow exceptions require justification.
...
| Job | Coverage |
|---|---|
| `quality` | ruff format + check, workspace pyright, warning-fatal strict Pyright for `hedron-core` + `hedron`, tip+2 `verify_pkg_*` (older packets on evidence), satellite import + symbol-tier gates, wheel build + clean-install smoke, STATUS/ROADMAP mirror check, docs train SSOT / recipe / sim checks, relative markdown link check, `mkdocs build --strict` |
...
`hedron-core` and `hedron` have reached zero Pyright errors and warnings. The shared quality
suite preserves that baseline with:

```bash
bash scripts/ci_checks.sh typing --python 3.12
```

The command runs Pyright with `--warnings` over both complete package trees. Pyright's warning
severity therefore remains useful for incremental workspace cleanup while any warning in these
two packages is still CI-fatal. Commit CI and release CI call the same quality suite.
