...
**Status:** Accepted; kept current with the **0.25.2** patch candidate on `main`
...
| `hedron` | `hedron` | `hedron-core`, FastAPI; Starlette through FastAPI | `v0.2.0` (current train `0.25.0`) |
...
| `hedron-charts` | `hedron_charts` | `hedron-core`; chart backends remain extras; also `hedron[charts]` | `v0.6.0` (Alpha line `0.1.x`, tip `0.1.6`) |
...
| `hedron-jinja` | `hedron_jinja` | `hedron-core`, Jinja; also `hedron[jinja]` | `v0.9.0` / train with `0.25.0` |
...
**Publish note:** the coordinated workspace is **`0.25.2`** (release candidate; not tagged);
the last published PyPI/git train remains **`v0.25.1`** — see [STATUS](STATUS.md).
