...
**Status:** Accepted; kept current with the **0.11.0** published train on `main`
...
| `hedron` | `hedron` | `hedron-core`, FastAPI; Starlette through FastAPI | `v0.2.0` (current published train `0.11.0`) |
...
| `hedron-jinja` | `hedron_jinja` | `hedron-core`, Jinja; also `hedron[jinja]` | `v0.9.0` / train with `0.11.0` |
...
**Publish note:** the coordinated published train is **`0.11.0`** (`v0.11.0`) for native
Flask/Django depth — see [STATUS](STATUS.md).
