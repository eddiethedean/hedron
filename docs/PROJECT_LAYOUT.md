...
**Status:** Accepted; kept current with the **0.24.0** train on `main`
...
| `hedron` | `hedron` | `hedron-core`, FastAPI; Starlette through FastAPI | `v0.2.0` (current train `0.24.0`) |
...
| `hedron-jinja` | `hedron_jinja` | `hedron-core`, Jinja; also `hedron[jinja]` | `v0.9.0` / train with `0.24.0` |
...
**Publish note:** the coordinated train is **`0.24.0`** (**Published** as `v0.24.0`; last
published PyPI/git = `v0.24.0`) for the production security floor — see [STATUS](STATUS.md).
