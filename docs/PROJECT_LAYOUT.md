...
**Status:** Accepted; kept current with the **0.20.0** train on `main`
...
| `hedron` | `hedron` | `hedron-core`, FastAPI; Starlette through FastAPI | `v0.2.0` (current train `0.20.0`) |
...
| `hedron-charts` | `hedron_charts` | `hedron-core`; chart backends remain extras; also `hedron[charts]` | `v0.6.0` (Alpha line `0.1.x`, tip `0.1.5`) |
...
| `hedron-jinja` | `hedron_jinja` | `hedron-core`, Jinja; also `hedron[jinja]` | `v0.9.0` / train with `0.20.0` |
...
**Publish note:** the coordinated train on `main` is **`0.20.0`** (Ready to cut; last
published PyPI/git = `v0.19.0`) for the production security floor — see [STATUS](STATUS.md).
