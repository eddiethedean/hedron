...
**Status:** Accepted; kept current with the **0.10.1** published train on `main`
...
| `hedron` | `hedron` | `hedron-core`, FastAPI; Starlette through FastAPI | `v0.2.0` (current published train `0.10.1`) |
...
| `hedron-jinja` | `hedron_jinja` | `hedron-core`, Jinja; also `hedron[jinja]` | `v0.9.0` / train with `0.10.1` |
...
**Publish note:** the coordinated published train is **`0.10.1`** (`v0.10.1`) for live interaction—
