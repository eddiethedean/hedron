...
**Status:** Maintainer reference for the published **1.1.x** release train.
...
| `hedron` | `hedron` | `hedron-core`, FastAPI; Starlette through FastAPI | `v0.2.0` (current train `1.1.x`) |
...
| `hedron-posit` | `hedron_posit` | `hedron`, `fastapi-workbench`; unified Posit facade; also `hedron[posit]` | Beta `1.1.x` |
...
**Publish note:** the coordinated **`v1.1.1`** train is tagged and published on PyPI — see
[STATUS](STATUS.md).
