...
Security fixes land on the **current published train** (`0.56.x`). Older `0.x` lines should
upgrade; there is **no multi-year LTS**. Best-effort triage for the immediately previous
minor (`0.55.x`) continues through approximately **2027-08-20** — after that, upgrade
is required. There is **no contractual patch SLA**.

The living in-tree tip is **`v0.56.0`**; **`v0.54.0` remains on PyPI** while the Git tag and
PyPI upload for 0.56.0 are **deferred** (`registry_status = deferred`). First-run pins from
PyPI use `>=0.54.0,<0.55` until a newer wheel lands. Checkout tip is `v0.56.0`.

| Version | Supported |
|---|---|
| `0.56.x` | Yes (current published train — pin `>=0.56.0,<0.57`; published `v0.56.0`) |
| `0.55.x` | Best-effort security triage through approximately 2027-08-20; upgrade to `0.56.x` |
| `0.54.x` | Best-effort security triage through approximately 2027-08-20; upgrade to `0.55.x` / `0.56.x` |
| `0.53.x` | Best-effort security triage through approximately 2027-08-20; upgrade to `0.54.x` / `0.56.x` |
...
4. Fixes land on the current published train (`0.56.x`); see Supported versions above.
