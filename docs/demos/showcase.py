"""The full Hedron Showcase demo, rendered offline with ``hedron-sim``."""
...
def _metric(label: str, value: str, delta: str, *, tone: str = "up"):
    return html.dl(
        html.dt(label),
        html.dd(
            html.strong(value, data={"metric-value": "true"}),
            html.small(delta, class_=f"showcase-delta showcase-delta--{tone}"),
        ),
...
def _pipeline_card(region, *, refreshed: bool = False):
    return html.section(
        html.div(
            html.div(
                html.span("Pipeline", class_="hedron-app-kicker"),
                html.h3("Data release pipeline"),
                html.p("One declarative flow from source to production."),
            ),
            html.span("Live" if refreshed else "Running", class_="hedron-status"),
            class_="showcase-card-heading",
        ),
        html.ol(
            _step("Ingest", "1,284 sources connected", "complete"),
            _step("Validate", "Zero schema drift detected", "complete"),
            _step("Transform", "18 of 24 partitions applied", "current"),
            _step("Publish", "Awaiting release approval", "pending"),
...
def _approval_card(region, *, approved: bool = False):
    ...
    return html.section(
        html.div(
            html.div(
                html.span("Release gate", class_="hedron-app-kicker"),
                html.h3("v1.0.5-rc1"),
                html.p("Production promotion requires an explicit owner action."),
            ),
...
def _run_rows(filter_name: str) -> Iterable[object]:
    ...
    for name, status, duration, count, tone in rows:
        if filter_name == "attention" and tone not in {"danger", "warning"}:
            continue
        yield html.tr(
            html.td(name),
...
def _runs_card(region, filter_name: str = "all"):
    ...
    return html.section(
        html.div(
            html.div(
                html.span("Operations", class_="hedron-app-kicker"),
                html.h3("Recent runs"),
            ...
            html.div(
                ...
                html.button(
                    "Needs attention",
                    type="button",
                    class_="showcase-filter showcase-filter--active"
                    if filter_name == "attention"
                    else "showcase-filter",
                    **_hx(hx_get="/runs/attention", hx_target=region.selector, hx_swap="outerHTML"),
                ),
...
def _activity_card():
    return html.section(
        html.div(
            html.span("Audit trail", class_="hedron-app-kicker"),
            html.h3("Recent activity"),
            html.p("Important changes, visible by default."),
        ...
        html.ol(
            ...
            html.li(
                html.time("09:18"),
                html.div(
                    html.strong("Workspace upgraded"),
                    html.span("Northstar moved to Scale"),
                ),
            ),
            html.li(
                html.time("08:55"),
                html.div(
                    ...
                    html.span("Webhook latency returned to baseline"),
                ),
            ),
...
def _component_inventory(region):
    return html.section(
        html.div(
            html.div(
                html.span("Built-in vocabulary", class_="hedron-app-kicker"),
                html.h3("What you just saw"),
                html.p("The page is composed from typed Python components and explicit routes."),
            ),
            html.button(
                "Inspect surface map",
                type="button",
        ...
        html.div(
            html.span("AppShell", class_="showcase-chip"),
            html.span("Metric", class_="showcase-chip"),
            html.span("Card", class_="showcase-chip"),
            html.span("Table", class_="showcase-chip"),
            html.span("Progress", class_="showcase-chip"),
            html.span("Timeline", class_="showcase-chip"),
            html.span("Typed action", class_="showcase-chip showcase-chip--accent"),
...
def build_showcase_demo() -> str:
    """Return the full-feature Hedron Showcase as a no-server docs island."""
    app = SimApp(title="Hedron Showcase", demo_id="showcase-dashboard")
    pipeline = app.region("showcase-pipeline", description="Pipeline status")
    approval = app.region("showcase-approval", description="Release approval")
    runs = app.region("showcase-runs", description="Recent runs")
    inventory = app.region("showcase-inventory", description="Component inventory")
        ...
        return Page(
            html.div(
                html.aside(
                    ...
                    html.span("SHOWCASE", class_="showcase-nav-label"),
                    html.nav(
                        html.a(
                            "Overview",
                            href=SafeUrl.parse("#showcase-overview", purpose=UrlPurpose.NAVIGATION),
                            class_="showcase-nav-link showcase-nav-link--active",
                        ),
                        html.a(
                            "Deployments",
                            href=SafeUrl.parse("#showcase-runs", purpose=UrlPurpose.NAVIGATION),
                            class_="showcase-nav-link",
                        ),
                        html.a(
                            "Components",
                            href=SafeUrl.parse(
                                "#showcase-inventory", purpose=UrlPurpose.NAVIGATION
                            ),
                            class_="showcase-nav-link",
                        ),
                ...
                html.main(
                    html.header(
                        html.div(
                            html.span("HEDRON / SHOWCASE", class_="hedron-app-kicker"),
                            html.h2("Command center"),
                            html.p(
                                "A complete server-rendered workspace composed from Python values."
                            ),
                        ),
                        html.div(
                    ...
                    html.div(
                        ...
                        html.span(
                            "Everything is operating normally. The transform pipeline is the "
                            "only active change."
                        ),
                    ...
                    html.div(
                        ...
                        _metric("Successful runs", "98.7%", "+2.1%"),
                        _metric("Open incidents", "7", "−3"),
                    ...
                    _component_inventory(inventory),
                    id="showcase-overview",
                    class_="hedron-app-main showcase-main",
                ),
                class_="hedron-demo hedron-app-shell showcase-shell",
                data={"hedron-theme": "showcase"},
            ),
            title="Hedron Showcase",
        )

    @app.fragment("/pipeline/refresh", region=pipeline)
            ...
            html.section(
                html.div(
                    html.div(
                        ...
                        html.h3("The architecture stays inspectable"),
                        html.p(
                            "Routes, fragments, actions, and components remain ordinary "
                            "Python contracts."
                        ),
                    ),
                    html.span("7 surfaces", class_="hedron-status"),
                    class_="showcase-card-heading",
                ),
                html.div(
                    html.span("Page", class_="showcase-chip showcase-chip--accent"),
                    html.span("View", class_="showcase-chip"),
                    html.span("Action", class_="showcase-chip"),
                    html.span("Fragment region", class_="showcase-chip"),
    ...
    return wrap_browser_chrome(
        ...
        url="127.0.0.1:8000",
        caption=(
            "Offline Hedron Showcase — try Refresh pipeline, filter Recent runs, "
            "approve the release, "
            "and inspect the component surface map."
        ),
    )
