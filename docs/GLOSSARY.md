...
**Pin** — An install constraint with an upper bound, for example `hedron>=0.52.0,<0.53`.
...
**Published** — A cut that is on the current train (`v0.53.0` in-tree; `v0.52.0` on PyPI). Pin the matching
...
adopters pin together. “Living train” means the current published line (`0.53.x`).
