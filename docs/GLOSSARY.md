...
**Pin** — An install constraint with an upper bound, for example `hedron>=0.61.0,<0.62`.
...
**Published** — A cut available on the current train (`v0.61.0` in-tree and on PyPI). Pin the matching
...
adopters pin together. “Living train” means the current published line (`0.61.x`).
