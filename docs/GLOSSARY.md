...
**Pin** — An install constraint with an upper bound, for example `hedron>=0.56.0,<0.59`.
...
**Published** — A cut that is on the current train (`v0.58.0` in-tree; PyPI serves `v0.56.0`). Pin the matching
...
adopters pin together. “Living train” means the current published line (`0.58.x`).
