...
| D-037 | Accepted | Phase 0.7 external cache and durable job conformance targets Redis (`redis` client, JSON `h1:` keys). FastAPI `BackgroundTasks` remains non-durable (D-020). Official HTMX SSE extension is Deferred to phase 0.10; bounded polling is the Supported job-status transport until that gate passes. |
| D-038 | Accepted | Hedron has no scheduled 1.0 phase or arbitrary API-freeze terminus. Capability phases continue on the `0.x` line (`0.N` → `v0.N.0`), with stability and compatibility promises attached to documented contracts rather than inferred from a major version. The former deferred backlog is assigned to 0.9 native framework depth, 0.10 live interaction/navigation, 0.11 authoring/tooling, 0.12 data/visualization scale, 0.13 advanced async/observability, and 0.14 portable runtimes/acceleration. Every phase retains evidence, migration, and published-artifact gates. This supersedes D-031/D-032 and the 1.0/freeze portions of D-035. |
...
| D-040 | Accepted | Hedron adopts Jinja as an optional trusted-application template integration in a separate `hedron-jinja` distribution. Python components remain canonical; Jinja owns template composition while Hedron owns typed component bindings, rendering, metadata, diagnostics, assets, HTMX policy, and adapters. Component aliases are explicit and static, strict escaping/checking is the default, and no sandbox claim is made for untrusted template authors. RFC-0031 governs the phase 0.11 implementation and phases 0.11–0.13 HDN migration/removal. This supersedes D-039's open design hold and RFC-0030 while preserving HDN's experimental classification. |
