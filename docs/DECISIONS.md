...
| D-037 | Accepted | Phase 0.7 external cache and durable job conformance targets Redis (`redis` client, JSON `h1:` keys). FastAPI `BackgroundTasks` remains non-durable (D-020). Official HTMX SSE was Deferred through 0.9; phase 0.10 promotes pinned `htmx-ext-sse` as Supported live observation while bounded polling remains the required Supported fallback (RFC-0032 / JOB-006). |
