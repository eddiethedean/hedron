...
def _chrome(title: str, current: str, *content: object) -> Page:
    return Page(
        ...
        AppShell(
            ...
            brand=Brand("Data Mover", href="/", mark_text="H"),
            env_badge=Text(ENVIRONMENT, role="label"),
            account=AccountSummary("ops@example.test", detail="Operator"),
            ...
            app_footer=AppFooter("Hedron owns every stylesheet on this path."),
            content_width="wide",
            panel_id=PANEL_ID,
...
def pipelines() -> Page:
    ...
    return _chrome(
        ...
        Status("Pipeline transform in progress", variant="compact", tone="info"),
        Identity("Dana Approver", detail="Release owner"),
        ResourceList(
            ResourceRow("nightly-warehouse", href="/", description="Primary warehouse sync"),
            ResourceRow("crm-backfill", description="CRM historical load"),
            label="Active transfers",
        ),
        Card(
            FileUpload(label="Upload mapping CSV", hint="UTF-8 CSV up to 5 MB", status="Ready"),
            title="Mapping upload",
        ),
        SplitView(
