...
- Align the Flask adapter with the coordinated 1.1.1 release train.
...
- Add canonical `view` and `action` adapter aliases.

## [0.67.0] — 2026-08-27

### Changed
- Aligned the Flask adapter and coordinated package metadata with the Phase 0.67 release train.
...
### Added
- 0.64 presentation and HTMX lifecycle compatibility for Flask hosts.
...
- Capability/idempotency action enforcement remains FastAPI-owned in 0.55
  (`PARITY-055`: Flask capabilities/replay unsupported). Layout components such as
...
### Added
- Phase 0.45 portable catalog facts; Flask is not a TypeSchema producer.
