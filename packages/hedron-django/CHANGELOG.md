...
- Align the Django adapter with the coordinated 1.1.1 release train.
...
### Changed
- Aligned the Django adapter and coordinated package metadata with the Phase 0.67 release train.
...
### Added
- 0.64 presentation and HTMX lifecycle compatibility for Django hosts.
...
### Changed
- Adopted the 0.62 coordinated core train while retaining the ordinary HTML fallback boundary.
...
### Fixed
- ``history_restore`` render mode matches the FastAPI host (PAGE vs FRAGMENT).
...
### Fixed
- Django ``include_feature`` now fails closed for unmaterialized DataWorkspace
...
### Added
- Phase 0.45 portable catalog facts; Django is not a TypeSchema producer.
...
- Coordinated 0.29 train bump. Django adapter does not import hedron-workbench.
