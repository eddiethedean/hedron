...
- Enforce stream deadlines while awaiting each asynchronous token.
...
- Keep AppShell environment badge labels vertically centered when authored with `Text`.
...
### Added
- Unified action lifecycle, operation identity, bounded traces, server-authored async regions, and phase 0.61 surface contracts.
