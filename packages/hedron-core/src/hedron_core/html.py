...
from hedron_core._html_meta import ATTR_ALIASES, KNOWN_TAGS, URL_ATTRS, VOID_TAGS
        ...
        if lower in URL_ATTRS or lower.endswith("href") or lower.endswith("src"):
            if isinstance(value, SafeUrl):
                ...
                purpose = (
                    ...
                    if lower in {"src", "poster"}
                    else UrlPurpose.NAVIGATION
                )
                # Require explicit SafeUrl — do not silently wrap.
                raise error(
                    "HED-SEC-0003",
...
class _NativeElement:
    ...
    def __init__(self, tag: str, children: tuple[Any, ...], attrs: dict[str, Any]) -> None:
        ...
        self.attributes = _normalize_attrs(attrs)
