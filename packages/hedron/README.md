...
Requires Python 3.11–3.14. Pin the published `hedron>=0.60.2,<0.61` release from PyPI.
The in-tree `v0.60.2` cut is published on PyPI.
The fastest path uses
[`uv`](https://docs.astral.sh/uv/getting-started/installation/):

```bash
uvx --from "hedron>=0.60.2,<0.61" hedron new my-hedron-app
cd my-hedron-app
uv sync
...
The latest installable PyPI release is `hedron>=0.60.2,<0.61`. The repository checkout
and the repository provides the published `0.60.x` train. Use `uv sync` when working from a checkout.
...
**Package maturity:** Beta · **Repository train:** `0.60.x` · **Repository tip:** `0.60.2` · **Latest PyPI release:** `0.60.2` · **Application pin:** `>=0.60.2,<0.61`
...
```bash
uv add "hedron[data,dev]>=0.60.2,<0.61"
```

Charts require the fixed compatible floor:

```bash
uv add "hedron[charts]>=0.60.2,<0.61"
```
