...
def compose_lifespan(
    ...
    *,
    ...
    Startup is synchronous (path/manifest/plugin work) and runs before the
    optional ``user_lifespan``. Production mode requires a valid build
    manifest and disables runtime CSS compilation until shutdown.

    Args:
        user_lifespan: Optional nested lifespan composed around Hedron startup.
        production: Override production detection; ``None`` uses env/settings.
        build_dir: Build output directory (default ``HEDRON_BUILD_DIR`` or
            ``.hedron/build``).
        theme: Theme name stored on ``app.state.hedron_theme``.

    Returns:
        A FastAPI-compatible async context manager factory.
    """
