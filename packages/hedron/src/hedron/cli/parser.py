...
from hedron.cli.arguments import boolean_argument, run_command
...
def _cmd_style_check_dispatch(args: argparse.Namespace) -> int:
    if boolean_argument(args, "custom_css"):
...
def _register_catalog_commands(sub: _Subparsers) -> None:
    ...
    _ignored = routes_p.add_argument(
        "--document",
        action="store_true",
    ...
    _ignored = preview_p.add_argument("logical_id", help="Route logical id or name")
    ...
    _ignored = testgen_p.add_argument(
        "--profile",
        default="default",
...
def _register_inspect_commands(sub: _Subparsers) -> None:
    ...
    _ignored = inspect_p.add_argument(
        "component",
        help="Component name, logical id, 'interactions', 'features', or 'htmx-extensions'",
    )
    _ignored = inspect_p.add_argument("--json", action="store_true", help="Emit versioned JSON")
    _ignored = inspect_p.add_argument(
        "--static",
        nargs="?",
        const=".",
    ...
    _ignored = eject_p.add_argument(
        "component", help="Component name, logical id, or 'features:<bundle-id>'"
    )
    _ignored = eject_p.add_argument(
        "--out",
        "--output",
    ...
    _ignored = explain_p.add_argument(
        "target",
...
def _register_scaffold_commands(sub: _Subparsers) -> None:
    ...
    _ignored = build_p.add_argument("--project", default=None)
    _ignored = build_p.add_argument(
        "--dev", action="store_true", help="Use readable development names"
    )
    build_p.set_defaults(func=cmd_build)

    new_p = sub.add_parser("new", help="Scaffold a Hedron application or element")
    _ignored = new_p.add_argument("name", help="Project name, or 'element'")
    _ignored = new_p.add_argument(
        "element_name", nargs="?", default=None, help="Element package name"
    )
    _ignored = new_p.add_argument("--path", default=None, help="Destination directory")
    _ignored = new_p.add_argument("--force", action="store_true")
    _ignored = new_p.add_argument(
        "--flask",
        action="store_true",
...
def _register_check_commands(sub: _Subparsers) -> None:
    ...
    _ignored = check_p.add_argument("--project", default=None)
    ...
    _ignored = security_check_p.add_argument("--project", default=None)
    _ignored = security_check_p.add_argument(
        "--format",
        choices=("text", "json", "sarif"),
        default="text",
    )
    ...
    _ignored = security_check_p.add_argument(
        "--suppressions", default=None, help="JSON suppressions file"
    )
    _ignored = security_check_p.add_argument(
        "--baseline",
        default=None,
...
def _register_discovery_commands(sub: _Subparsers) -> None:
    ...
    _ignored = discover_p.add_argument(
        "--format",
        choices=("human", "json"),
        default="json",
...
def _register_package_commands(sub: _Subparsers) -> None:
    ...
    _ignored = package_doctor_p.add_argument(
        "path",
        nargs="?",
        default=".",
...
def _register_audit_commands(sub: _Subparsers) -> None:
    ...
    _ignored = conf_p.add_argument("--json", action="store_true", help="Emit JSON report")
...
def _register_runtime_commands(sub: _Subparsers) -> None:
    ...
    _ignored = dev_p.add_argument("--project", default=None)
    _ignored = dev_p.add_argument("--interval", type=float, default=0.5)
    _ignored = dev_p.add_argument("--once", action="store_true", help="Build once and exit")
    ...
    _ignored = run_p.add_argument("target", nargs="?", help="module:app or module:factory")
    _ignored = run_p.add_argument("--factory", action="store_true")
    _ignored = run_p.add_argument("--host")
    _ignored = run_p.add_argument("--port", type=int)
    _ignored = run_p.add_argument("--reload", action="store_true")
    _ignored = run_p.add_argument("--workers", type=int, default=1)
    _ignored = run_p.add_argument("--debug", action="store_true")
    _ignored = run_p.add_argument("--workbench", action="store_true")
    _ignored = run_p.add_argument(
        "--discover",
        action="store_true",
        help="Bind then discover the Workbench mount even without RS_SERVER_URL",
    )
    _ignored = run_p.add_argument("--workbench-mode", choices=("auto", "on", "off"), default="auto")
    _ignored = run_p.add_argument("--mount")
    _ignored = run_p.add_argument("--public-base-url")
    _ignored = run_p.add_argument("--forwarded-allow-ips")
    _ignored = run_p.add_argument("--allow-external-bind", action="store_true")
    _ignored = run_p.add_argument(
        "--topology",
        choices=(
            "auto",
...
def _register_theme_commands(sub: _Subparsers) -> None:
    ...
    _ignored = theme_check_p.add_argument(
        "--theme",
        action="append",
        default=None,
        help="Theme name to check (repeatable; default: every built-in theme)",
    )
    _ignored = theme_check_p.add_argument("--format", choices=("text", "json"), default="text")
    _ignored = theme_check_p.add_argument(
        "--severity",
        default="error",
    ...
    _ignored = theme_export_p.add_argument("--theme", dest="theme_name", default="folio")
    _ignored = theme_export_p.add_argument("--spec", default=None, help="ThemeSpec JSON input")
    _ignored = theme_export_p.add_argument(
        "--profile",
        choices=("core", "forms", "data", "workflow", "complete"),
        default="core",
    )
    _ignored = theme_export_p.add_argument(
        "--format", choices=("css", "json", "report"), default="json"
    )
    _ignored = theme_export_p.add_argument("--output", default=None)
    ...
    _ignored = theme_manifest_p.add_argument("--output", default=None)
    ...
    _ignored = theme_metadata_p.add_argument("--output", default=None)
    ...
    _ignored = theme_matrix_p.add_argument("--component", action="append", default=None)
    _ignored = theme_matrix_p.add_argument("--viewport", action="append", default=None)
    _ignored = theme_matrix_p.add_argument("--mode", action="append", default=None)
    _ignored = theme_matrix_p.add_argument("--accessibility-mode", action="append", default=None)
    _ignored = theme_matrix_p.add_argument("--output", default=None)
    ...
    _ignored = theme_contract_p.add_argument("--theme", dest="theme_name", default="folio")
    _ignored = theme_contract_p.add_argument("--spec", default=None, help="ThemeSpec JSON input")
    _ignored = theme_contract_p.add_argument("--stylesheet", default=None)
    _ignored = theme_contract_p.add_argument("--output", default=None)
    ...
    _ignored = theme_inspect_p.add_argument("--stylesheet", required=True)
    _ignored = theme_inspect_p.add_argument("--output", default=None)
    ...
    _ignored = style_check_group.add_argument(
        "--zero-app-css",
        dest="zero_app_css",
    ...
    _ignored = style_check_p.add_argument("--format", choices=("text", "json"), default="text")
    ...
    _ignored = style_inspect_p.add_argument("--format", choices=("human", "json"), default="human")
    ...
    _ignored = style_explain_p.add_argument(
        "surface", nargs="?", help="Public component.part surface"
    )
    _ignored = style_explain_p.add_argument("--property", default=None)
    _ignored = style_explain_p.add_argument(
        "--design",
        default=None,
    ...
    _ignored = style_preview_p.add_argument(
        "--design",
        default=None,
        help="Design or theme name (default: app theme or 'folio')",
    )
    ...
    _ignored = style_diff_p.add_argument("base", nargs="?", help="Base design/theme name")
    _ignored = style_diff_p.add_argument("candidate", nargs="?", help="Candidate design/theme name")
    _ignored = style_diff_p.add_argument(
        "--ejected-path",
        default=None,
    ...
    _ignored = style_update_p.add_argument("--check", action="store_true", required=True)
    _ignored = style_update_p.add_argument("--manifest", default=None)
    _ignored = style_update_p.add_argument("--format", choices=("human", "json"), default="human")
    ...
    _ignored = style_eject_p.add_argument("name", help="Design or theme name to eject")
    _ignored = style_eject_p.add_argument(
        "--group", default=None, help="Eject one typed design group"
    )
    _ignored = style_eject_p.add_argument("--recipe", default=None, help="Eject one named recipe")
    _ignored = style_eject_p.add_argument(
        "--component",
        default=None,
    ...
    _ignored = style_eject_app_p.add_argument("--output", required=True)
    _ignored = style_eject_app_p.add_argument("--overwrite", action="store_true")
    ...
    _ignored = style_init_p.add_argument("--name", default="custom")
    _ignored = style_init_p.add_argument("--output", required=True)
    ...
    _ignored = style_package_p.add_argument("--spec", required=True)
    _ignored = style_package_p.add_argument("--output", required=True)
    _ignored = style_package_p.add_argument(
        "--profile",
        choices=("core", "forms", "data", "workflow", "complete"),
        default="core",
    )
    _ignored = style_package_p.add_argument("--license", action="append", default=None)
    _ignored = style_package_p.add_argument("--overwrite", action="store_true")
    ...
    _ignored = style_conform_p.add_argument("--spec", required=True)
    _ignored = style_conform_p.add_argument(
        "--profile",
        choices=("core", "forms", "data", "workflow", "complete"),
        default=None,
    )
...
def _register_migrate_commands(sub: _Subparsers) -> None:
    ...
    _ignored = build_api_parser(migrate_sub)
    _ignored = build_streamlit_parser(migrate_sub)
    _ignored = build_react_parser(migrate_sub)
...
def _register_upgrade_commands(sub: _Subparsers) -> None:
    ...
    _ignored = upgrade_p.add_argument(
        "--from", dest="from_version", required=True, help="Current train tip"
    )
    _ignored = upgrade_p.add_argument(
        "--to", dest="to_version", required=True, help="Target train tip"
    )
    _ignored = upgrade_p.add_argument(
        "--baseline",
        default=None,
    ...
    _ignored = upgrade_p.add_argument("--out", default=None, help="Write JSON report to a file")
    _ignored = upgrade_p.add_argument(
        "--allow-definite",
        action="store_true",
