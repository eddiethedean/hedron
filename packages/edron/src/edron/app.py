...
from typing import Annotated, Protocol, cast, get_origin
...
from edron.promotion import CapabilityPromotion
from edron.simulation import AppSimulation
from hedron_core.app_state import request_json
...
class _AsyncPageRenderer(Protocol):
    def __call__(self, **kwargs: object) -> Awaitable[object]: ...


class _ProjectionMapping(Protocol):
...
class App:
    ...
    def __init__(
        ...
        *,
        ...
        if not isinstance(cast(object, raw_title), str) or not raw_title.strip():
        ...
        _ignored = install_connections(self.hedron, self._resources)
        self._resource_specs: dict[str, Resource] = {}
        self._pages: dict[str, object] = {}
        self._fragments: dict[int, object] = {}
        ...
        self._promotions: dict[str, object] = {}
        self._data_workspaces: dict[str, object] = {}
    ...
    def from_hedron(cls, app: hedron.Hedron, *, title: str | None = None) -> App:
        raw_app: object = app
        if not isinstance(cast(object, raw_app), hedron.Hedron):
        ...
        if isinstance(existing_resources, ConnectionRegistry):
            ...
            _ignored = install_connections(app, instance._resources)
    ...
    def native_surface(self, surface: object) -> object:
        """Resolve a registered Edron surface to its exact native projection."""
        return self._native_surface(surface)

    def simulation(self, *, fixtures: Mapping[str, object] | None = None) -> AppSimulation:
    ...
    def interaction(self, value: hedron.Interaction) -> hedron.Interaction:
        ...
        if not isinstance(cast(object, raw_value), hedron.Interaction):
            raise TypeError("interaction expects a native hedron.Interaction")
        self._ensure_open()
        self._interactions[value.fingerprint] = value
        return value

    def browser_plan(self, *demands: object, assets: Sequence[str] = ()) -> object:
    ...
    def resource(
        ...
        *,
        ...
        if spec.scope == "application":
            _ignored = spec.register(self._resources)
        else:
            ...
            request_provider: Callable[..., object]
            if inspect.iscoroutinefunction(spec.factory):

                async def async_request_resource() -> object:
                ...
                def sync_request_resource() -> object:
                    value = spec.factory()
    ...
    @staticmethod
    def _unwrap_annotations(annotation: object) -> object:
        if get_origin(annotation) is Annotated:
            return type_arguments(annotation)[0]
        return annotation

    @staticmethod
    def _dependency_signature(
        signature: inspect.Signature, dependencies: Sequence[object]
    ) -> inspect.Signature:
    ...
    def _without_self(signature: inspect.Signature) -> inspect.Signature:
        ...
        if params and params[0].name == "self":
            _ignored = params.pop(0)
        return signature.replace(parameters=params)

    def _page_dependencies(self, page_type: type[Page]) -> list[Dependency[object]]:
        return [
            value for value in class_namespace(page_type).values() if isinstance(value, Dependency)
        ]

    def _instantiate(self, page_type: type[Page], dependency_values: Mapping[str, object]) -> Page:
        try:
            instance = page_type()
    ...
    def _dependency_values(
        self, page_type: type[Page], kwargs: dict[str, object]
    ) -> dict[str, object]:
    ...
    def _call_with_kwargs(
        fn: Callable[..., object], instance: Page, kwargs: dict[str, object]
    ) -> object:
    ...
    def function_page(
        ...
        *,
        ...
        def register(render_fn: Callable[..., object]) -> Callable[..., object]:
            ...
            if inspect.iscoroutinefunction(render_fn):
                ...
                async def async_page_render(instance: Page, **kwargs: object) -> object:
                    return await cast(_AsyncPageRenderer, cast(object, render_fn))(**kwargs)

                page_render = async_page_render
            else:

                def sync_page_render(instance: Page, **kwargs: object) -> object:
    ...
    def page_function(self, *args: object, **kwargs: object) -> object:
    ...
    def _register_page(
        ...
        *,
        ...
        if (
            not isinstance(cast(object, raw_page_type), type)
            or not issubclass(cast(object, raw_page_type), Page)
        ...
        page_namespace = class_namespace(page_type)
        if "__init__" in page_namespace:
            raise RegistrationError(
                "Page classes must not define __init__",
                code="EDRON_PAGE_INIT",
                source=source_location(page_type),
            )
        render = page_namespace.get("render")
        ...
        declared_dependencies: list[Dependency[object]] = [
            cast(Dependency[object], item) for item in dependencies if isinstance(item, Dependency)
        ]
        page_dependencies: list[Dependency[object]] = [
        ...
        def endpoint(**kwargs: object) -> object:
            ...
            with frame_context(frame):
                ...
                return hedron_core_page(
                    *cast(object, instance)._resolved_output(), title=title if show_title else None
                )
        ...
        if inspect.iscoroutinefunction(render):
            ...
            async def async_endpoint(**kwargs: object) -> object:
                dependency_values = self._dependency_values(page_type, kwargs)
                ...
                with frame_context(frame):
                    ...
                    if result is not None:
                        instance.include(result)
                    return hedron_core_page(
                        *cast(object, instance)._resolved_output(),
        ...
        for member_name, member in class_namespace(page_type).items():
            if isinstance(member, Fragment):
                ...
                self._register_action(
                    cast(Action[object, object], member),
                    page_type,
                    path,
                    member_name,
                    callable_dependencies=[
    ...
    @staticmethod
    def _native_dependency(value: object) -> object:
        return value.native() if isinstance(value, Dependency) else value

    def _register_fragment(
        self,
        definition: Fragment[object],
        page_type: type[Page],
        page_path: str,
        member_name: str,
        *,
        callable_dependencies: Sequence[object],
        route_dependencies: Sequence[object],
    ) -> None:
        ...
        def endpoint(**kwargs: object) -> object:
            ...
            with frame_context(frame):
                ...
                if result is not None:
                    instance.include(result)
                return hedron_core_fragment(*cast(object, instance)._resolved_output())
        ...
        cast(object, definition)._native = native
        self._fragments[id(definition)] = native

    def _register_action(
        self,
        definition: Action[object, object],
        page_type: type[Page],
        page_path: str,
        member_name: str,
        *,
        ...
        for index, parameter in enumerate(parameters):
            annotation = self._unwrap_annotations(parameter_annotation(parameter))
    ...
    def _mount_fragment(
        self, definition: Fragment[object], arguments: Mapping[str, object]
    ) -> None:
    ...
    def _native_surface(self, surface: object) -> object:
        ...
        if isinstance(surface, Action):
            return self._actions.get(id(cast(object, surface)))
    ...
    def source_map(self) -> dict[str, object]:
        ...
        for record in self._pages.values():
            ...
            for _member_name, member in class_namespace(page_type).items():
                if not isinstance(member, (Fragment, Action)):
                    continue
                if len(entries) >= MAX_SOURCE_MAP_ENTRIES:
                    truncated = True
                    break
                internal_member = cast(object, member)
    ...
    def explain(self) -> dict[str, object]:
        ...
        visual_interactions: list[dict[str, object]] = []
        truncated = False
        for record in self._pages.values():
        ...
        for bundle in self._bundles.values():
            if len(visual_interactions) >= MAX_EXPLANATION_SURFACES:
                truncated = True
                break
            provider = dynamic_attribute(bundle, "provider", "")
            if provider not in {"hedron-charts", "hedron-maps"}:
                continue
            projections_value = dynamic_attribute(bundle, "projections", ())
            projections = (
                cast(Sequence[object], projections_value)
                if isinstance(projections_value, Sequence)
                and not isinstance(projections_value, (str, bytes))
                else ()
            )
            ...
            visual_interactions.append(
                {
                    ...
                    "logical_id": dynamic_attribute(bundle, "logical_id"),
                    "provider": provider,
                    "provider_version": dynamic_attribute(bundle, "provider_version"),
                    "projection": (
                        cast(_ProjectionMapping, projection).as_mapping()
        ...
        return {
            ...
            "promotions": [
                ...
                for promotion in sorted(
                    (cast(CapabilityPromotion, item) for item in self._promotions.values()),
    ...
    def _resolve_action(self, value: object) -> object:
        ...
        if isinstance(value, Action):
            return self._actions.get(id(cast(object, value)))
        return value

    @staticmethod
    def _bind_action(native: object, arguments: Mapping[str, object]) -> object:
        if not arguments:
            return native
    ...
    def _action_button(self, label: str, action: object, **kwargs: object) -> object:
    ...
    def _action_form(self, action: object, *, model: object = None, **kwargs: object) -> object:
        native = self._resolve_action(action)
    ...
    def include(self, feature: object) -> object:
    ...
    def include_package(
        ...
        *,
        ...
        if not isinstance(cast(object, raw_package), FeaturePackage):
    ...
    def navigation_target(self, target: object) -> NavigationTarget:
    ...
    def _include_visual_interaction(self, interaction: object) -> object:
    ...
    def data_workspace(
        ...
        *,
        ...
        async def save(request: Request) -> JSONResponse:
            try:
                payload_value = await request_json(request)
    ...
    def styles(self, name: str, source: str | Path, **kwargs: object) -> object:
...
def hedron_core_page(*nodes: object, title: str | None = None) -> object:
    from hedron_core.builtins.document import Page as NativePage

    return NativePage(*nodes, title=title)


def hedron_core_fragment(*nodes: object) -> object:
